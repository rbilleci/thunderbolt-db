...
async fn sqlx_supported_subset_smoke_with_unsupported_recovery(
    ...
    let unsupported = sqlx::raw_sql("COPY sqlx_people FROM STDIN WITH CSV HEADER")
        .execute(&mut conn)
        .await
        .expect_err("COPY FROM STDIN WITH CSV HEADER remains explicitly unsupported");
    let database_error = unsupported
        .as_database_error()
        .expect("COPY FROM STDIN WITH CSV HEADER unsupported error should be a database error");
