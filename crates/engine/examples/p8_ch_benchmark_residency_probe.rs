...
fn run_probe(args: &Args) -> Result<(), Box<dyn Error>> {
    ...
    markdown.push_str(&format!("- resident_bytes: {}\n", snapshot.resident_bytes));
    markdown.push_str("- configured_production_tiers: 25pct/125pct of 24GiB VRAM\n");
    ...
    markdown.push_str("\n## decision\n");
    markdown.push_str("The scheduled-worker baseline is a calibration tier because the 25% VRAM target estimates hundreds of millions of retained rows and is not defensible for one short cron slice. The harness preserves the simplified 25/125% tier plan: 25% is the under-resident trust gate, and 125% is the over-resident memory-pressure/fallback gate.\n");
