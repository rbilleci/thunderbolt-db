...
/// Sub-frames per wave publish (`GPU_DB_INTENT_LANE_SUBFRAMES`, default 1).
/// Splitting a wave's WAL payload into N contiguous-seq frames multiplies the
/// FUA fence queue depth generated by the SAME traffic — this drive's FUA
/// latency is bimodal (~1.6ms/fence below ~qd8, 0.68ms at qd16+), so low-load
/// deployments raise this to push the drive into fast mode (the ack then
/// waits on N pipelined fast fences instead of one slow one).
