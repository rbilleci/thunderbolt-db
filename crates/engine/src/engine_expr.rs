...
fn mandatory_int4_equalities(expr: &ResidentExpr, out: &mut Vec<(usize, i32)>) {
    match expr {
        ResidentExpr::Binary {
            op: ResidentBinaryOp::And,
            lhs,
...
mod zone_map_prune_tests {
    ...
    /// The load-bearing regression guard for the audit's DO-NOT-SHIP: on a MIXED-type table the filter
