...
/// Outcome of [`Engine::try_append_open_shard_mega`] (see its doc).
pub(crate) enum MegaAppendOutcome {
    /// Shape/basis/index not eligible — run the classic two-stage path.
    NotEligible,
    /// The launch failed; the index entry was dropped and nothing was published.
    /// The classic retry overwrites the same headroom and converges.
    DeviceFailed,
    /// The pass ran: per-row verdicts (0 inserted / 1 key present / 2 overflow).
    Applied { verdicts: Vec<u32> },
}
...
impl Engine {
    ...
    /// MEGA-FUSE (task #11): the covered-INSERT wave's PROBE-FIRST fused validate+insert —
    /// ONE launch probes the shard's PK device index per row, CAS-inserts winners, scatters
    /// column values, stamps created_by (the NEVER-VISIBLE sentinel for non-winners — their
    /// slots are burned, hidden from every snapshot) and row-ids, and publishes the device
    /// row-count header. Returns per-row verdicts (0 inserted / 1 key already present /
    /// 2 probe overflow). `NotEligible` => the caller runs the classic two-stage path
    /// (separate validate launch + apply coalescer), byte-identical to the flag being off.
    ///
    /// Load-bearing subtleties:
    /// * Any non-winner pins the shard's `max_created_by` high-water to `Index::MAX`: the
    ///   hwm fast path lets readers at `s >= hwm` skip the visibility filter entirely, and a
    ///   sentinel-stamped slot must NEVER ride that skip.
    /// * The host PK-cache mirror extends only for ALL-WINNER waves (a loser's key is already
    ///   in the mirror; re-adding would corrupt it) — any-loser waves leave the mirror stale
    ///   and the (ptr,row_count)-keyed basis rebuilds on the next probe.
    /// * Losers burn their slots: the device index-cache basis advances by the FULL k.
    pub(crate) fn try_append_open_shard_mega(
        &self,
        table: &str,
        new_rows: &[Vec<SqlValue>],
        stamps: &[Index],
        ...
        use MegaAppendOutcome as O;
        let k = new_rows.len();
        if k == 0 || stamps.len() != k || !self.device_write_locate_enabled() {
            return O::NotEligible;
        }
        let pressured_gpus = self
            .router
            .runtime()
            ...
            .memory_pressured_gpu_ids
            .clone();
        let (shard_id, capacity, row_count, shard_int4_names, shard_int8_names, gpu_id) = {
            let shards = self.read_state.residency.shards.load();
            let Some(table_shards) = shards.get(table) else {
                return O::NotEligible;
            };
        ...
        let column_types: Vec<SqlType> = catalog_table.columns.iter().map(|c| c.ty).collect();
        let column_count = column_types.len();
        if shard_int4_names.len() + shard_int8_names.len() != column_count
            || !shard_int8_names.is_empty()
            || new_rows.iter().any(|row| row.len() != column_count)
        {
            return O::NotEligible; // int4-only covered shape; arity drift declines
        }
        // Rollover is the classic path's job.
        if row_count.checked_add(k).map_or(true, |end| end > capacity) {
            return O::NotEligible;
        }
        let Some(shard_device_memory) = self
            .read_state
        ...
        let chunk_offsets: Vec<u64> = chunks
            ...
            .map(|chunk| chunk.byte_offset)
            .collect();
        let Ok(base_row_u32) = u32::try_from(row_count) else {
            return O::NotEligible;
        };
        let Some(created_by_region) =
            self.get_or_alloc_created_by_region(table, shard_id, capacity, gpu_id)
        else {
            return O::NotEligible;
        };
        ...
        // EXACTLY ONE live cached device index at the current basis — the probe IS the
        // validate, so no index means no verdicts (classic path handles it).
        let device_ptr = shard_device_memory.device_ptr();
        let mut live: Vec<(usize, Arc<CudaResidentDeviceMemory>, u32, u32)> = Vec::new();
        {
            let cache = self
        ...
        let (col_idx, index, table_mask, hash_shift) = live.pop().expect("len 1");
        let table_size = (table_mask as u64) + 1;
        if ((row_count + k) as u64).saturating_mul(2) > table_size {
            // Past the builder's load rule: drop so the next probe rebuilds at the grown
            // size, and let the classic path take this wave.
            self.read_state
                .residency
                .shard_pk_device_index
                .lock()
        ...
        let values: Vec<i32> = column_values.iter().flatten().copied().collect();
        let col_dests: Vec<u64> = chunk_offsets.iter().map(|off| device_ptr + off).collect();
        let request = gpu_db_execution::FusedApplyRequest {
            col_dests: &col_dests,
            values: &values,
            stamps,
            ...
            header_dest: device_ptr,
            header_value: (row_count + k) as u64,
        };
        let _hold_index_alive = &index; // the launch reads the index buffer
        let verdicts = match shard_device_memory.submit_i32_mega_fuse(&request) {
            Ok(verdicts) if verdicts.len() == k => verdicts,
            Ok(_) | Err(_) => {
                // A failed/short launch may have partially mutated the index — drop the
                // entry (rebuild on next probe). Values/stamps live in headroom the classic
                // retry overwrites at the same base; never publish here.
                self.read_state
                    .residency
                    .shard_pk_device_index
                    .lock()
                    .unwrap_or_else(|poisoned| poisoned.into_inner())
                    .remove(&(table.to_string(), shard_id, col_idx));
                return O::DeviceFailed;
            }
        };
        let overflow = verdicts.iter().any(|&v| v == 2);
        // SUFFIX TRIM: losers at the TAIL of the wave never publish — their slots stay
        // invisible headroom (the next append overwrites them, sentinel stamps included)
        // and the shard stays on the version-free fast path. Only INTERIOR losers (a dup
        // mid-wave followed by winners at positional slots) burn published slots and pin
        // the hwm to MAX (permanently versioned until rollover/VACUUM — rare^2, documented).
        let suffix_losers = verdicts.iter().rev().take_while(|&&v| v != 0).count();
        let published = k - suffix_losers;
        let interior_losers = verdicts[..published].iter().filter(|&&v| v != 0).count();
        let all_winners = interior_losers == 0 && suffix_losers == 0;
        if suffix_losers > 0 {
            // The kernel already published base+k into the device row-count header word —
            // correct it to the trimmed count (the same 8-byte header chunk the unfused
            // encoder writes last). Failure leaves the header HIGH over invisible,
            // sentinel-stamped headroom: readers bound to the HOST row_count never see it,
            // and the next wave's launch republishes — converges, so best-effort.
            let mut header = Vec::with_capacity(8);
            header.extend_from_slice(&((row_count + published) as u64).to_le_bytes());
            let _ = shard_device_memory.append_owned_chunks(vec![CudaOwnedDeviceMemoryChunk {
                byte_offset: 0,
                bytes: header,
            }]);
        }
        {
            ...
            if let Some(entry) = cache.get_mut(&(table.to_string(), shard_id, col_idx)) {
                if entry.resident_device_ptr == device_ptr && entry.row_count == row_count {
                    if overflow {
                        entry.device_index = None; // DECLINED is monotone
                    }
                    // Winners' index entries point at their POSITIONAL slots within
                    // [base, base+published); interior losers burn theirs.
                    entry.row_count = row_count + published;
                }
            }
        }
        if all_winners {
        ...
        // Zone map over WINNER rows only (losers are hidden or trimmed).
        let new_min_max: Vec<(i32, i32)> = (0..column_count)
            .filter(|&c| {
                matches!(
                    column_types[c],
            ...
            .map(|c| {
                new_rows
                    ...
                    .zip(verdicts.iter())
                    .filter(|(_, &v)| v == 0)
                    .fold((i32::MAX, i32::MIN), |(lo, hi), (row, _)| {
                        let v = sql_value_as_int4(&row[c]);
                        (lo.min(v), hi.max(v))
                    })
            })
            .collect();
        let winner_count = verdicts.iter().filter(|&&v| v == 0).count();
        // hwm: a SENTINEL slot inside the published range pins the shard permanently
        // versioned (the hwm fast path must never skip the filter over one); suffix-only
        // losers never publish, so the hwm stays the real winner max.
        let stamps_hwm = if interior_losers > 0 {
            Index::MAX
        } else {
            stamps
                .iter()
        ...
        let num_i32_cols = shard_int4_names.len();
        let appended_bytes = (published * num_i32_cols * std::mem::size_of::<i32>()) as u64;
        if published > 0 {
            self.read_state.residency.with_shards_mut(|shards| {
                if let Some(table_shards) = shards.get_mut(table) {
                    if let Some(open) = table_shards.last_mut() {
                        open.row_count += published;
                        open.max_created_by = open.max_created_by.max(stamps_hwm);
                        open.resident_bytes = open.resident_bytes.saturating_add(appended_bytes);
                        if winner_count > 0 {
                            for (stat, (lo, hi)) in open
                                .resident_device_int4_column_stats
    ...
    /// Mega-fuse waves that actually ran (the flag's non-vacuity telemetry).
    pub fn mega_fuse_hits(&self) -> u64 {
        self.read_state
            .residency
            .mega_fuse_hits
            .load(std::sync::atomic::Ordering::Relaxed)
