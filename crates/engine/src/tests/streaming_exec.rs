...
fn gpu_streaming_grouped_compaction_and_over_cardinality_defer() {
    // S-E.3 compaction + the honest over-cardinality defer. (1) 150 true groups (1.8KB of partials,
    // UNDER the 2KB chunk target) over ~3 chunks: after chunk 2 the accumulator holds ~300 partial rows
    // (3.6KB, OVER target) so it MUST compact mid-scan via the device merge back to 150 — and still
    // produce exact counts. (2) an all-unique key (1500 groups, 18KB of true partials) cannot compact
    // below the budget -> the fold DEFERS to the CPU path (correct rows, no hit counted).
    let mut e = Engine::new_local();
    let mut seq = 0u64;
    if !gpu_available(&mut e, &mut seq) {
        return;
    ...
    let result = e
        ...
        .unwrap();
    let expected: Vec<Vec<SqlValue>> = (0..150)
        .map(|g| {
            let count = (0..N).filter(|i| i % 150 == g).count() as i64;
    ...
    assert_eq!(
        sorted_rows(&result),
        expected,
        "150-group counts exact THROUGH the mid-scan compaction"
    );
