...
impl MutationRequest {
    pub fn new(parsed: gpu_db_sql::ParsedCommand) -> Self {
        Self {
            ...
            principal: AuthorizationPrincipal::BootstrapPostgres,
            on_prepared: None,
        }
    }
    ...
    /// Carry connection-local authorization metadata without changing canonical SQL/WAL bytes.
    pub fn with_principal(mut self, principal: AuthorizationPrincipal) -> Self {
...
impl CopyMutationRequest {
    ...
    /// Carry connection-local authorization metadata without changing canonical COPY/WAL bytes.
    pub fn with_principal(mut self, principal: AuthorizationPrincipal) -> Self {
        self.principal = principal;
        self
    }
    ...
    fn into_parts(
        ...
        (self.copy, self.rows, self.target, self.principal)
    }
}
...
impl Engine {
    ...
    fn submit_copy(
        ...
        let (copy, rows, target, principal) = request.into_parts();
        let catalog = match self.transaction_snapshot_handle(txn_id) {
            Some(snapshot) => snapshot.transaction_catalog(),
            None => self.catalog_snapshot(),
        };
        self.authorize_command_at(
            &catalog,
            principal,
            &Command::Insert(self.relational_copy_insert(&copy, Vec::new())),
        )?;
    ...
    fn submit_statement(
        ...
        match command {
            ...
            command @ (Command::SequenceNextVal(_) | Command::SequenceSetVal(_)) => {
                reject_prepared_hook(on_prepared)?;
                self.execute_sequence_value_command(txn_id, &command, principal)
...
#[cfg(test)]
#[path = "engine_mutation_admission/tests.rs"]
mod tests;
