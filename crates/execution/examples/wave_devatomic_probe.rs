//! Wave-engine increment 1d-i — push the read ceiling: DEVICE-memory claim/completed atomics.
//!
//! The 1c audit pinpointed the ~10.5M req/s ceiling as the two HOST-MAPPED atomics (`claim`, `completed`)
//! crossing PCIe — 256 threads already saturated them. This moves both into DEVICE memory (GPU-local
//! atomics, ~ns not ~µs); the only host-mapped per-request write left is the result slot itself
//! (un-contended, different slot each → PCIe bandwidth, not serialized). Completion is signaled by the
//! LAST completer (the thread whose `completed` bump reaches `head`) setting a single host-mapped
//! `all_done` flag, so the host still gets a tight poll. Same GPU index as 1c.
//!
//! Also QUANTIFIES the slot→wire mapping cost (the "bare data plane" caveat): after the GPU drain, the
//! host maps every packed result slot into a neutral `Option<i32>` row and times it — the per-request
//! host work the real facade would do (here trivially parallelizable, unlike the old serial coalescer).
//!
//! Run: `cargo run --release -p gpu_db_execution --example wave_devatomic_probe`
...
/// Control block (host-mapped) = [doorbell@0, head@4, all_done@8]. Counters (DEVICE) = [claim@0,
/// completed@4]. Each thread: exit on doorbell/backstop; peek device-`claim` < head; device `atom.add`
/// to claim; hash-probe the index + gather; write the packed result (host-mapped); device `atom.add`
/// `completed`; if that reaches `head`, set host-mapped `all_done`. Lock-free, no barriers.
const PTX_SRC: &str = r#"
.version 6.0
.target sm_30
.address_size 64

.visible .entry gpu_db_wave_devatomic(
    .param .u64 ctrl,
    .param .u64 counters,
    .param .u64 req,
    .param .u64 res,
...
fn main() {
    ...
    let ctrl = ctrl_host as *mut u32; // [doorbell, head, all_done, _]
    ...
    let entry = CString::new("gpu_db_wave_devatomic").unwrap();
    ...
    assert_eq!(
        dev_counters[1], requests,
        "device completed counter != requests (got {})",
        dev_counters[1]
    );

    // Correctness (per-slot, atomic packed read).
    let mut found = 0u32;
    ...
    // Quantify the slot->wire mapping cost: map every packed slot into a neutral Option<i32> row
    // (the per-request host work the facade would do; embarrassingly parallel, unlike the old coalescer).
    let map_started = Instant::now();
    let mut neutral: Vec<Option<i32>> = Vec::with_capacity(requests as usize);
    for i in 0..requests as usize {
        let packed = unsafe { ptr::read_volatile(results.add(i)) };
        let done = (packed & 0xffff_ffff) as u32;
        neutral.push(if done == 1 {
            Some((packed >> 32) as i32)
        } else {
            None
        });
    }
    let map_elapsed = map_started.elapsed();
    std::hint::black_box(&neutral);
    ...
    let map_rate = requests as f64 / map_elapsed.as_secs_f64();
    println!(
        "# wave 1d-i DEVICE-atomic indexed point lookup  rows={rows} requests={requests} threads={} ({blocks}x256)",
        blocks * 256
    );
    ...
    println!(
        "  GPU drain {requests} INDEXED lookups in {drain_elapsed:?}  ->  {throughput:.0} req/s  \
         (vs 1c host-mapped-atomic: ~10.5M)"
    );
    println!(
        "  slot->wire map (host, single-thread) {map_elapsed:?}  ->  {map_rate:.0} rows/s  \
         (parallelizable; quantifies the bare-data-plane caveat)"
    );
    ...
    println!("OK: device-atomic indexed data plane correct, clean exit");
}
