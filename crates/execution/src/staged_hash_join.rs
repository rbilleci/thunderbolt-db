...
fn launch_cuda_hash_join_inner_i64(
    ...
    const PTX: &[u8] = include_bytes!("staged_hash_join.ptx");
    ...
    fill_ptx.push(0);
    let fill_fn = primary.cached_function(c"gpu_db_fill_i128", &fill_ptx)?;
...
fn launch_cuda_hash_join_inner_text_nn(
    ...
    fill_ptx.extend_from_slice(FILL_PTX);
    fill_ptx.push(0);
    let fill128_fn = primary.cached_function(c"gpu_db_fill_i128", &fill_ptx)?;
    let fill64_fn = primary.cached_function(c"gpu_db_fill_i64", &fill_ptx)?;
