...
fn copy_cuda_resident_i64_rows(
    resident: &CudaResidentDeviceMemory,
    byte_offset: u64,
    row_indices: &[u64],
) -> Result<Vec<i64>, CudaRuntimeProbeError> {
...
fn launch_cuda_resident_i64_compare_scalar_filter(
    resident: &CudaResidentDeviceMemory,
    byte_offset: u64,
    scalar: i64,
    scalar_on_left: bool,
...
fn launch_cuda_resident_i64_compare_columns_filter(
    resident: &CudaResidentDeviceMemory,
    a_byte_offset: u64,
    b_byte_offset: u64,
    comparison: u32,
...
fn launch_cuda_resident_i32_sum_at_indices(
    resident: &CudaResidentDeviceMemory,
    byte_offset: u64,
    indices: &[u32],
) -> Result<i64, CudaRuntimeProbeError> {
...
fn launch_cuda_resident_i64_sum_at_indices_i128(
    resident: &CudaResidentDeviceMemory,
    byte_offset: u64,
    indices: &[u32],
) -> Result<i128, CudaRuntimeProbeError> {
...
fn launch_cuda_resident_i64_minmax_at_indices(
    resident: &CudaResidentDeviceMemory,
    byte_offset: u64,
    indices: &[u32],
    is_max: bool,
...
/// non-empty.
fn launch_cuda_resident_i128_minmax_partials_at_indices(
    resident: &CudaResidentDeviceMemory,
    byte_offset: u64,
    indices: &[u32],
    is_max: bool,
...
/// must be non-empty.
fn launch_cuda_resident_i128_sum_partials_at_indices(
    resident: &CudaResidentDeviceMemory,
    byte_offset: u64,
    indices: &[u32],
) -> Result<i128, CudaRuntimeProbeError> {
...
fn launch_cuda_group_by_kernel_timed(
    ...
    for i in 0..nslots {
        if keys[i] != EMPTY {
            ...
            groups.push(GroupByI32Row { key: keys[i], count: counts[i], sum: sums[i], sum_hi: 0, min: 0, max: 0, min_hi: 0, max_hi: 0, min_uuid: [0u8; 16], max_uuid: [0u8; 16] });
        }
    }
    Ok((groups, best))
}
...
fn compact_mask_i32_to_indices(
    resident: &CudaResidentDeviceMemory,
    mask_device_ptr: u64,
    n: u64,
) -> Result<Vec<u32>, CudaRuntimeProbeError> {
