...
"#;
    ...
    fn gpu_radix_argsort_parallel_matches_serial_and_bitonic() {
        ...
        for keys in &cases {
            ...
            for &desc in &[false, true] {
                let (resident, off) = retain_keys(keys);
                let parallel = launch_cuda_order_by_sort_i64_radix(&resident, keys, desc)
                    .expect("typed production radix");
        ...
        for &n in &[131_073_usize, 200_000, 300_000] {
            ...
            for &desc in &[false, true] {
                let (resident, off) = retain_keys(&keys);
                let parallel = launch_cuda_order_by_sort_i64_radix(&resident, &keys, desc)
                    .expect("typed production radix large-n");
        ...
        for &n in &[100_usize, 1_000, 10_000, 100_000, 1_000_000, 10_000_000] {
            ...
            let radix_ms = bench(&|| {
                launch_cuda_order_by_sort_i64_radix(&resident, &keys, false).unwrap()
            });
