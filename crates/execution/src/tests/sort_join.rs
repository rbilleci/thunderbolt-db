...
fn cuda_payload_join_retains_nn_outer_coordinates_on_device() {
    ...
    let mut actual = coordinates
        .readback_for_test()
        .expect("coordinate readback");
    ...
    let short_mask = left
        .row_range_mask_u32(2, 0, 2)
        .expect("short relation mask");
    assert!(matches!(
        left.filter_join_coordinates(&coordinates, &[Some(&short_mask), None], &[None, None],),
    ...
    let sorted_values: Vec<Option<i32>> = sorted_bytes
        .chunks_exact(4)
        .zip(sorted_valid)
        .map(|(bytes, valid)| valid.then(|| i32::from_le_bytes(bytes.try_into().unwrap())))
        .collect();
    assert_eq!(
        sorted_values,
        vec![
...
#[test]
#[ignore = "requires a local NVIDIA driver and GPU"]
fn cuda_materialized_text_scan_rejects_bad_offsets_and_scales_past_one_block() {
    use std::sync::{Arc, Barrier};

    let runtime = CudaDriverRuntime::probe().expect("probe");

    // Exact overflow-sabotage shape from the RETIRE-003 audit: lengths derived from [1,0,1]
    // used to become [u64::MAX,1], whose host checked_add rejected. The device validator must
    // reject before allocation/copy and leave the shared context and pools reusable.
    let invalid = Arc::new(
        runtime
        ...
        handles.push(std::thread::spawn(move || {
            ...
            for _ in 0..8 {
                let coordinates = invalid
                    .identity_join_coordinates(2, None)
                    .expect("identity coordinates");
                assert!(invalid
                    .materialize_join_coordinates(
                        &coordinates,
                ...
                assert_eq!(
                    invalid
                        .count_i32_equal_from_payload(0, 1, 1, None)
                        .expect("context remains reusable after rejected text materialization"),
                    1
                );
            }
        }));
    ...
    for handle in handles {
        handle.join().expect("validator thread");
    }

    // More than four scan blocks, with empty and non-empty strings crossing every boundary.
    let row_count = 1_025_u32;
    let mut offsets = Vec::with_capacity(row_count as usize + 1);
    let mut text = Vec::new();
    let mut expected = Vec::with_capacity(row_count as usize);
    offsets.push(0_u64);
    for row in 0..row_count {
        let value = vec![b'a' + (row % 23) as u8; (row % 7) as usize];
        text.extend_from_slice(&value);
        offsets.push(text.len() as u64);
        expected.push(Some(String::from_utf8(value).unwrap()));
    }
    let bytes_offset = offsets.len() as u64 * 8;
    let mut payload = offsets
        .iter()
    ...
    let source = runtime
        ...
        .expect("multi-block text payload");
    let coordinates = source
        .identity_join_coordinates(row_count, None)
        .expect("multi-block coordinates");
    let materialized = source
        .materialize_join_coordinates(
            ...
            &[CudaMaterializeJoinColumn::Text {
                ...
                payload: &source,
                offsets_byte_offset: 0,
                bytes_byte_offset: bytes_offset,
                bytes_len: text.len() as u64,
                validity_bitmap_offset: None,
            }],
        )
        .expect("parallel text materialization");
    assert_eq!(materialized.row_count(), row_count);
    assert_eq!(materialized.columns()[0].text_bytes_len, text.len() as u64);
    let identity = materialized
        .memory()
        .identity_join_coordinates(row_count, None)
        .expect("materialized identity");
    let layout = materialized.columns()[0];
    assert_eq!(
        materialized
            .memory()
            .project_text_from_join_coordinates(
                ...
                layout.text_bytes_byte_offset.unwrap(),
                layout.text_bytes_len,
                Some(layout.validity_bitmap_offset),
            )
            .expect("multi-block text projection"),
        expected
    );

    let empty = runtime
        .retain_device_memory_copy(0, &0_u64.to_le_bytes())
        .expect("empty text payload");
    let empty_coordinates = empty
        .identity_join_coordinates(0, None)
        .expect("empty coordinates");
    let empty_run = empty
        .materialize_join_coordinates(
            &empty_coordinates,
            &[CudaMaterializeJoinColumn::Text {
        ...
        .expect("empty text materialization");
    assert_eq!(empty_run.row_count(), 0);
    assert_eq!(empty_run.columns()[0].text_bytes_len, 0);
}

#[test]
#[ignore = "requires a local NVIDIA driver and GPU"]
fn cuda_coordinate_scoped_derived_key_rejects_different_coordinates() {
    let runtime = CudaDriverRuntime::probe().expect("probe");
    let resident = runtime
        .retain_device_memory_copy(
            0,
    ...
    let first_mask = resident.row_range_mask_u32(4, 0, 2).expect("first mask");
    let first = resident
        .identity_join_coordinates(4, Some(&first_mask))
        .expect("first coordinates");
    let second_mask = resident.row_range_mask_u32(4, 2, 4).expect("second mask");
    let second = resident
        .identity_join_coordinates(4, Some(&second_mask))
        .expect("second coordinates");
    let key = resident
        ...
        .expect("coordinate-scoped derived key");
    assert!(matches!(
        resident.sort_join_coordinates_with_keys(
            &second,
            &[CudaJoinSortKey::Derived {
                relation: 0,
                values: key.group_view(),
                width: 4,
                descending: false,
                nulls_first: false,
            }],
...
fn cuda_hash_join_inner_text_matches_unique_build_key() {
    ...
    let bytes =
        |texts: &[&str]| -> Vec<Vec<u8>> { texts.iter().map(|t| t.as_bytes().to_vec()).collect() };
...
fn cuda_staged_hash_joins_validate_and_apply_validity_bitmaps() {
    ...
    macro_rules! assert_invalid {
        ($result:expr) => {
            assert!(
                matches!($result, Err(CudaRuntimeProbeError::InvalidInputLength(_))),
    ...
    let sorted_unique = |outcome: HashJoinOutcome| {
        ...
        let mut pairs: Vec<(u32, u32)> = build_idxs.into_iter().zip(probe_idxs).collect();
        pairs.sort_unstable();
        pairs
    };
    assert_eq!(
    ...
    assert_eq!(
        sorted_unique(
            resident
                .hash_join_inner_text(&[b"x", b"y"], &[b"x", b"y"], Some(&[0b01]), Some(&[0b11]),)
    ...
    assert_eq!(
        build.into_iter().zip(probe).collect::<Vec<_>>(),
        vec![(0, 1)]
    );
    let (build, probe) = resident
        .hash_join_inner_text_nn(&[b"z", b"z"], &[b"z", b"z"], Some(&[0b01]), Some(&[0b10]))
