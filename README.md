...
- Supported sources:
  ...
  - `MvccReadSource::FollowValueChainBranches { keys, plans }` (generic source-preserving branch helper; resolves multiple linear value-chain plans per visible seed row in branch-list order before moving to the next seed)
...
- Next obvious extension boundary:
  - widen the new nested-join surface again with explicit fan-in policies or richer source-provenance controls without regressing the same engine-facing contract and explicit fallback accounting on the eventual GPU-backed path.
