...
- CUDA hardware onboarding has driver-level launch paths in `CudaDriverRuntime`: `launch_smoke_add_one(...)` covers PTX module load, device allocation, kernel launch, synchronization, and D2H copy, while `filter_equal_u32_mask(...)` and `filter_equal_bytes_mask(...)` are now integrated into `CudaMvccExecutionBackend` for exact numeric/string `MvccReadFilter::ValueEquals`, key-prefix filters, and prefix-equivalent key ranges over supported full-scan/key-lookup shapes. Broader MVCC row encoding, general key-range predicates, native scan/visibility, and key lookup kernels remain explicit `GpuMvccReadParityGap` fallback work.
...
- Current device strategy:
  ...
  - CUDA transition primitives: `CudaDriverRuntime::filter_equal_u32_mask(...)` and `filter_equal_bytes_mask(...)` now prove real H2D -> kernel predicate -> D2H equality-mask paths for fixed-width and byte/string batches; exact `ValueEquals`, key-prefix filters, and prefix-equivalent key ranges over supported first-slice shapes can report GPU execution on CUDA-capable hardware, while broader MVCC row/general key-range predicate work remains on explicit fallback.
