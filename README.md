...
- CUDA hardware onboarding has driver-level launch paths in `CudaDriverRuntime`: `launch_smoke_add_one(...)` covers PTX module load, device allocation, kernel launch, synchronization, and D2H copy, while `filter_equal_u32_mask(...)` is now integrated into `CudaMvccExecutionBackend` for exact numeric `MvccReadFilter::ValueEquals` filters over supported full-scan/key-lookup shapes. Broader MVCC row encoding, string/key predicates, native scan/visibility, and key lookup kernels remain explicit `GpuMvccReadParityGap` fallback work.
