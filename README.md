...
- Next obvious extension boundary:
  - widen the mixed-subpath first/last offset surface from paired first/last helpers into first/last-to-explicit-ordinal offset helpers without regressing the same engine-facing contract and explicit fallback accounting on the eventual GPU-backed path.
