...
- Current device strategy:
  ...
  - CUDA transition primitives: `CudaDriverRuntime::filter_all_mask(...)`, `filter_equal_u32_mask(...)`, `filter_equal_bytes_mask(...)`, `filter_bytes_range_mask(...)`, and `mvcc_row_batch_lengths(...)` now prove real device mask generation plus row-batch H2D -> kernel -> D2H inspection paths; filterless reads, exact `ValueEquals`, key-prefix filters, general key ranges, and nested `All`/`Any` trees composed from those predicates over supported first-slice shapes can report GPU execution on CUDA-capable hardware, while native scan and visibility work remains on explicit fallback.
