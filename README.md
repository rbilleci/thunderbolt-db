...
- CUDA-transition prep now also includes a regression-proven first-slice backend handoff: full scan / key lookup plus simple filter shapes can execute through an alternate backend while unsupported shapes fall back through the CPU reference pipeline with the same tracked `GpuMvccReadParityGap` accounting
