...
- Current device strategy:
  ...
  - CUDA transition primitives: `CudaDriverRuntime::filter_equal_u32_mask(...)`, `filter_equal_bytes_mask(...)`, and `filter_bytes_range_mask(...)` now prove real H2D -> kernel predicate -> D2H mask paths for fixed-width, byte/string equality, and bytewise range batches; exact `ValueEquals`, key-prefix filters, general key ranges, and nested `All`/`Any` trees composed from those predicates over supported first-slice shapes can report GPU execution on CUDA-capable hardware, while broader MVCC row/native scan and visibility work remains on explicit fallback.
