...
- Next obvious extension boundary:
  - widen the same-subpath first/last-occurrence surface from exact offsets into inclusive offset-range helpers without regressing the same engine-facing contract and explicit fallback accounting on the eventual GPU-backed path.
