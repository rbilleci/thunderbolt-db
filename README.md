...
- Current device strategy:
  ...
  - CUDA transition primitives: `CudaDriverRuntime::filter_equal_u32_mask(...)` and `filter_equal_bytes_mask(...)` now prove real H2D → kernel predicate → D2H equality-mask paths for fixed-width and byte/string batches; exact `ValueEquals` filters over supported first-slice shapes can report GPU execution on CUDA-capable hardware, while broader MVCC row/key predicate work remains on explicit fallback.
