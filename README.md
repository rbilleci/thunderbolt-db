...
- P2 catalog work has started with an engine-owned `public` schema catalog. Created relational tables receive stable user relation OIDs starting at `16384`; columns receive stable ids, `attnum`, table OID references, and PostgreSQL-compatible type metadata for `int4` (OID `23`, fixed width `4`) and `text` (OID `25`, variable width). This metadata is now used by relational insert validation, row decoding, catalog-bound select projection/filter/order planning, and result column descriptions. The compatibility endpoint also has dynamic first-slice `pg_catalog.pg_class`/`pg_catalog.pg_attribute` introspection for supported session tables, including relation OIDs plus column `attnum`/`atttypid`/`attlen` metadata. The current limitation is explicit: catalog state is in-memory with the bootstrap engine, and broader psql `\d`/`information_schema` introspection is still pending.
