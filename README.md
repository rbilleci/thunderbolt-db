...
- P1 relational SQL bootstrap support is intentionally narrow and structured: `CREATE TABLE name (column INT|INTEGER|TEXT, ...)`, `INSERT INTO name (columns...) VALUES (...)`, and `SELECT {*|columns} FROM name [WHERE column = literal] [ORDER BY column [ASC|DESC]] [LIMIT n]`. Engine-facing relational reads lower through the MVCC read path with CPU reference execution and explicit `GpuMvccReadParityGap` fallback; the psql compatibility endpoint exercises the same parsed command shapes for real-client golden coverage. Joins, expressions, NULLs, updates/deletes, schema-qualified names, constraints, broad type coercion, and catalog introspection are still outside this P1 slice.
- P2 catalog work has started with an engine-owned `public` schema catalog. Created relational tables receive stable user relation OIDs starting at `16384`; columns receive stable ids, `attnum`, table OID references, and PostgreSQL-compatible type metadata for `int4` (OID `23`, fixed width `4`) and `text` (OID `25`, variable width). This metadata is now used by relational insert validation, row decoding, catalog-bound select projection/filter/order planning, result column descriptions, and bootstrap recovery from the durable WAL prefix. The compatibility endpoint also has dynamic first-slice `pg_catalog.pg_class`/`pg_catalog.pg_attribute` introspection for supported session tables, including relation OIDs plus column `attnum`/`atttypid`/`attlen` metadata. The current limitation is explicit: catalog state replays through the in-memory bootstrap WAL recovery helper, while cross-process persisted WAL/checkpoint recovery and broader psql `\d`/`information_schema` introspection remain pending.
- P3 wire-protocol work has started with a narrow real extended-query path in the compatibility endpoint. `Parse`, `Bind`, `Describe`, `Execute`, `Sync`, and `Close` now support session-local prepared statements and portals for text-format `int4`/`text` parameters and text-format results over the current relational `SELECT` subset; statement close cascades to dependent portals, portal close stays local, and bind parameter arity is checked against the prepared query placeholder/type contract. Unsupported binary formats, limited portal fetches, copy/function-call flows, and broader portal behavior still return explicit unsupported-feature errors. The psql golden suite includes a `\bind` / `\parse` / `\bind_named` scenario that reaches relational execution through the extended protocol.
- P4 SQL-to-GPU bridge work has started at the SQL result boundary. Relational `SELECT * FROM table` now lowers through the MVCC CUDA probe path and can report `executed_target = gpu(0)` with SQL-level CPU/GPU parity coverage on local NVIDIA hardware. SQL-side filter/order/projection/limit finalization is still host-side and is explicitly reported as `GpuMvccReadParityGap`; `RelationalSqlGpuBridgeReport::from_results(...)` summarizes SQL-level GPU execution and CPU fallback rates for the supported relational mix.
