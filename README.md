...
- P2 catalog work has started with an engine-owned `public` schema catalog. Created relational tables receive stable user relation OIDs starting at `16384`; columns receive stable ids, `attnum`, table OID references, and PostgreSQL-compatible type metadata for `int4` (OID `23`, fixed width `4`) and `text` (OID `25`, variable width). This metadata is now used by relational insert validation, row decoding, projection, and result column descriptions. The current limitation is explicit: catalog state is in-memory with the bootstrap engine and `pg_catalog` introspection for user tables is still pending.
