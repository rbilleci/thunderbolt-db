...
write_retained_phase_sample() {
  ...
  if [ -n "$phase_facts" ] && [ -s "$phase_facts" ]; then
    sed -n 's/^select_phase_json=//p' "$phase_facts" | tail -n "$sample_count" >"$sample_path"
  fi
...
REPORT
  ...
  engine_pgwire_query_metrics "$url" "$metrics_path" order_line_lookup_composite_int4_projection \
    "${lookup_key}|${lookup_item}|${lookup_qty}|${lookup_amount}" \
    "SELECT ol_o_id, ol_i_id, ol_quantity, ol_amount FROM order_line WHERE ol_o_id = $lookup_key AND ol_i_id = $lookup_item" \
    "$tmp_prefix" retained_engine_int4_composite_equality_multi_column_projection true none
  engine_pgwire_query_metrics "$url" "$metrics_path" order_line_lookup_composite \
    ...
    "$tmp_prefix" retained_engine_int4_text_composite_equality_projection true "$lookup_blocker"
...
25pct,engine_backed_pgwire_endpoint,psql/libpq,order_line_lookup_composite_int4_projection,1,scaled_smoke,none,retained_engine_int4_composite_equality_multi_column_projection,"wall_clock_throughput,p50_us,p95_us,p99_us,error_count,correctness_status,saturation_note"
25pct,engine_backed_pgwire_endpoint,psql/libpq,order_line_lookup_composite,1,scaled_smoke,$lookup_blocker,retained_engine_int4_text_composite_equality_projection,"wall_clock_throughput,p50_us,p95_us,p99_us,error_count,correctness_status,saturation_note"
CSV
  local concurrency
  for concurrency in 2 4 8 16 32 64 128; do
...
- concurrency_blocker: none_for_scaled_owner_thread_smoke
...
textless composite-key lookup \`SELECT ol_o_id, ol_i_id, ol_quantity, ol_amount
FROM order_line WHERE ol_o_id = <literal> AND ol_i_id = <literal>\` now reaches
a retained \`int4_composite_equality_multi_column_projection\` route with
zero-H2D evidence. The composite/text lookup \`SELECT ol_o_id, ol_i_id,
ol_quantity, ol_amount, ol_dist_info FROM order_line WHERE ol_o_id = <literal>
AND ol_i_id = <literal>\` now also reaches a retained
\`int4_equality_mixed_column_projection\` route with zero-H2D evidence by
materializing \`ol_dist_info\` from the resident text offset/byte layout.
...
The endpoint now builds selected matching row IDs with retained device-side
match-index compaction for accepted int4 equality filters, then reads back only
selected-row int4 values and selected-row text offset/byte spans from retained
device memory. That narrows the composite/text lookup D2H surface to compacted
matching row indices plus selected projection output for this equality lookup
family.

The remaining benchmark blocker is
\`full_25pct_identical_curves_require_operator_long_run\`: full 25% identical
default/tuned PostgreSQL/GPU curves still require an operator-approved long-run
window and artifact budget.
REPORT
  ...
  trap - RETURN
  cat "$report_path"
  echo "p8_ch_benchmark_engine_backed_pgwire_smoke=closed retained_lookup_shape=int4_equality_mixed_column_projection lookup_blocker=$lookup_blocker artifact=$report_path"
}
...
write_engine_backed_pgwire_concurrency_smoke() {
  ...
  local facts_path="$smoke_dir/endpoint-facts.txt"
  local max_sessions
  max_sessions="$(pgwire_required_engine_sessions "$targets" 6)"
  : >"$metrics_path"
...
REPORT
  ...
  GPU_DB_P8_ENGINE_PGWIRE_LISTEN="$listen" \
    ...
    GPU_DB_P8_ENGINE_PGWIRE_GPU_MICROBATCH_ROUTE_LANE_SCAN_POLICY="${GPU_DB_P8_ENGINE_PGWIRE_GPU_MICROBATCH_ROUTE_LANE_SCAN_POLICY:-fixed}" \
...
REPORT
  ...
  local expected_count lookup_key lookup_item lookup_qty lookup_amount tmp_prefix concurrency literal_variants literal_sql_list literal_expected_list literal_projection_sql_list literal_projection_expected_list literal_mixed_sql_list literal_mixed_expected_list literal_heterogeneous_sql_list literal_heterogeneous_expected_list
  expected_count="$rows"
  lookup_key=$(((rows + 1) / 2))
  lookup_item=$(((lookup_key % 100000) + 1))
  lookup_qty=$(((lookup_key % 50) + 1))
  ...
  literal_heterogeneous_sql_list=""
  literal_heterogeneous_expected_list=""
  ...
  for literal_idx in $(seq 1 "$literal_variants"); do
    ...
    if [ -n "$literal_sql_list" ]; then
      ...
      literal_heterogeneous_sql_list="${literal_heterogeneous_sql_list};;"
      literal_heterogeneous_expected_list="${literal_heterogeneous_expected_list};;"
    fi
    ...
    literal_heterogeneous_sql_list="${literal_heterogeneous_sql_list}SELECT ol_o_id, ol_i_id, ol_quantity, ol_amount FROM order_line WHERE ol_o_id = $literal_key;;SELECT ol_o_id FROM order_line WHERE ol_o_id = $literal_key;;SELECT ol_o_id, ol_dist_info FROM order_line WHERE ol_o_id = $literal_key"
    literal_heterogeneous_expected_list="${literal_heterogeneous_expected_list}${literal_key}|${literal_item}|${literal_qty}|${literal_amount};;${literal_key};;${literal_key}|${literal_dist}"
  done
...
CSV
  for concurrency in ${targets//,/ }; do
    ...
    engine_pgwire_concurrency_metric "$url" "$metrics_path" "$curve_path" \
      order_line_lookup_ol_o_id_heterogeneous_literal_batch \
      "${lookup_key}|${lookup_item}|${lookup_qty}|${lookup_amount}" \
      "SELECT ol_o_id, ol_i_id, ol_quantity, ol_amount FROM order_line WHERE ol_o_id = $lookup_key" \
      "$tmp_prefix" retained_engine_heterogeneous_literal_projection_batch true "$concurrency" \
      "$literal_heterogeneous_sql_list" "$literal_heterogeneous_expected_list"
...
- client_driver: \`${GPU_DB_CH_BENCH_ENGINE_PGWIRE_CLIENT_DRIVER:-tokio-postgres/simple-query}\`
...
- gpu_microbatch_route_lane_scan_policy: \`${GPU_DB_P8_ENGINE_PGWIRE_GPU_MICROBATCH_ROUTE_LANE_SCAN_POLICY:-fixed}\`
...
When select phase facts are enabled, the JSON metrics also attach retained
...
write_identical_pgwire_target_smoke() {
  ...
  if [ "$rows" -ge "$(rows_25pct)" ] && [ "${GPU_DB_CH_BENCH_ALLOW_FULL_IDENTICAL_PGWIRE_25PCT:-0}" != 1 ]; then
    GPU_DB_CH_BENCH_IDENTICAL_PGWIRE_CONCURRENCY_TARGETS="$targets" write_identical_pgwire_full_readiness
    return 0
  fi
...
source "$ROOT_DIR/scripts/lib/p8_ch_benchmark_protocol_boundary.sh"
...
write_identical_pgwire_load_stream() {
  local rows="$1"
  cat <<SQL
\set ON_ERROR_STOP on
CREATE TABLE order_line (
...
stream_identical_pgwire_load_to_psql() {
  local url="$1"
  local rows="$2"
  local out_path="$3"
  local err_path="$4"
...
write_identical_pgwire_full_readiness() {
  mkdir -p "$OUT_DIR/identical-pgwire-full-readiness"
  local readiness_dir="$OUT_DIR/identical-pgwire-full-readiness"
  local report_path="$readiness_dir/readiness.md"
  local json_path="$readiness_dir/readiness.jsonl"
  local estimated_rows
  estimated_rows="$(rows_25pct)"
  local retained_target_bytes=6442450944
  ...
  local raw_metric_budget_bytes=$((64 * 1024 * 1024))
  local required_disk_bytes=$((generated_table_bytes + wal_log_bytes + raw_metric_budget_bytes))
  ...
  local targets="${GPU_DB_CH_BENCH_IDENTICAL_PGWIRE_CONCURRENCY_TARGETS:-1,2,4,8,16,32,64,128}"
  local engine_port="${GPU_DB_CH_BENCH_ENGINE_PGWIRE_PORT:-55437}"
  local docker_port
  docker_port="$(pgsql_docker_port)"
  local status=blocked
  local blocker=full_25pct_identical_curves_require_operator_long_run
  local full_command="GPU_DB_CH_BENCH_ALLOW_FULL_IDENTICAL_PGWIRE_25PCT=1 GPU_DB_CH_BENCH_IDENTICAL_PGWIRE_ROWS=$estimated_rows GPU_DB_CH_BENCH_IDENTICAL_PGWIRE_CONCURRENCY_TARGETS=$targets scripts/run_p8_ch_benchmark_residency_probe.sh --identical-pgwire-target-smoke"
  local cleanup_command="scripts/run_p8_ch_benchmark_residency_probe.sh --pgsql-baseline-docker-down"

  cat >"$report_path" <<REPORT
# P8 Identical Pgwire Full 25% Readiness
...
- raw_metric_budget_bytes: $raw_metric_budget_bytes
- required_disk_bytes: $required_disk_bytes
- available_disk_bytes: $available_disk_bytes
- disk_preflight: $disk_preflight
- target_profiles: default_postgresql, tuned_postgresql, gpu_db_retained_endpoint
- default_postgresql_url: postgresql://postgres:REDACTED@127.0.0.1:$docker_port/gpu_db_p8_baseline
- tuned_postgresql_url: postgresql://postgres:REDACTED@127.0.0.1:$docker_port/gpu_db_p8_baseline
- gpu_db_retained_endpoint_url: postgresql://postgres@127.0.0.1:$engine_port/postgres?sslmode=disable
- concurrency_targets: \`$targets\`
- load_contract: streamed \`CREATE TABLE\` plus \`COPY FROM STDIN\`; no full \`load.sql\` artifact
- cleanup_command: \`$cleanup_command\`
- full_guard: \`GPU_DB_CH_BENCH_ALLOW_FULL_IDENTICAL_PGWIRE_25PCT=1\`
- full_guarded_command: \`$full_command\`
- expected_gpu_db_retained_facts: SQL-visible COPY rows committed through Engine WAL/MVCC, resident warmup, zero-H2D retained route, device-side match-index compaction, selected-row int4/text D2H readback
- remaining_125pct_blocker: missing_partitioned_over_resident_execution

This is an operator readiness artifact only. It does not execute the full
25% default PostgreSQL, tuned PostgreSQL, or GPU DB retained curves. The full
run remains blocked until Richard approves a long-run window and artifact
budget, and the command above is explicitly guarded.
REPORT

  cat >"$json_path" <<JSON
{"kind":"identical_pgwire_full_readiness","tier":"25pct","status":"$status","blocker":"$blocker","estimated_rows":$estimated_rows,"retained_target_bytes":$retained_target_bytes,"generated_table_bytes":$generated_table_bytes,"wal_log_bytes":$wal_log_bytes,"raw_metric_budget_bytes":$raw_metric_budget_bytes,"required_disk_bytes":$required_disk_bytes,"available_disk_bytes":$available_disk_bytes,"disk_preflight":"$disk_preflight","targets":["default_postgresql","tuned_postgresql","gpu_db_retained_endpoint"],"concurrency_targets":"$(json_escape "$targets")","load_contract":"streamed_create_table_plus_copy_from_stdin","full_load_sql_artifact":false,"full_guard":"GPU_DB_CH_BENCH_ALLOW_FULL_IDENTICAL_PGWIRE_25PCT=1","full_guarded_command":"$(json_escape "$full_command")","cleanup_command":"$(json_escape "$cleanup_command")","remaining_125pct_blocker":"missing_partitioned_over_resident_execution"}
JSON
  cat "$report_path"
  echo "p8_ch_benchmark_identical_pgwire_full_readiness=blocked reason=$blocker artifact=$report_path"
}
...
case "$mode" in
  ...
  --identical-pgwire-target-smoke)
    write_identical_pgwire_target_smoke
    ;;
  --identical-pgwire-full-readiness)
    write_identical_pgwire_full_readiness
    ;;
  ...
  --self-check)
    ...
    grep -q 'retained_metadata_count_observed=true' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'backend_count_value_matches=true' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'post_mutation_residency_invalidated=false' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'post_mutation_route_accepted=true' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'post_mutation_route_zero_h2d=true' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'post_mutation_metadata_count_observed=true' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'post_mutation_rows_visible=true' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'post_mutation_incremental_append_observed=true' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'post_mutation_incremental_shard_rollover_observed=true' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'post_mutation_inserted_row_visible=true' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'post_mutation_inserted_device_route_witness=true' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q 'post_mutation_inserted_route_shape=sharded_int4_equality_mixed_column_projection' "$tmp_dir/out/engine-backed-protocol-boundary/probe-facts.txt"
    grep -q '"post_mutation_residency_invalidated":false' "$tmp_dir/out/engine-backed-protocol-boundary/metrics.jsonl"
    grep -q '"post_mutation_route_accepted":true' "$tmp_dir/out/engine-backed-protocol-boundary/metrics.jsonl"
    grep -q '"retained_route_zero_h2d_telemetry":true' "$tmp_dir/out/engine-backed-protocol-boundary/metrics.jsonl"
    grep -q '"post_mutation_route_zero_h2d_telemetry":true' "$tmp_dir/out/engine-backed-protocol-boundary/metrics.jsonl"
    grep -q '"retained_count_source":"resident_shard_metadata"' "$tmp_dir/out/engine-backed-protocol-boundary/metrics.jsonl"
    grep -q '"post_mutation_rows_visible":true' "$tmp_dir/out/engine-backed-protocol-boundary/metrics.jsonl"
    grep -q '"post_mutation_incremental_append_observed":true' "$tmp_dir/out/engine-backed-protocol-boundary/metrics.jsonl"
    grep -q '"post_mutation_incremental_shard_rollover_observed":true' "$tmp_dir/out/engine-backed-protocol-boundary/metrics.jsonl"
    grep -q '"post_mutation_inserted_row_visible":true' "$tmp_dir/out/engine-backed-protocol-boundary/metrics.jsonl"
    grep -q '"post_mutation_inserted_device_route_witness":true' "$tmp_dir/out/engine-backed-protocol-boundary/metrics.jsonl"
    ...
    grep -q '"query":"order_line_lookup_ol_o_id_retained_projection".*"correctness_status":"pass".*"route_classification":"retained_engine_int4_equality_projection".*"retained_gpu_route":true' "$tmp_dir/out/engine-backed-pgwire-benchmark-smoke/metrics.jsonl"
    grep -q '"query":"order_line_lookup_ol_o_id_multi_column".*"correctness_status":"pass".*"route_classification":"retained_engine_int4_equality_multi_column_projection".*"retained_gpu_route":true' "$tmp_dir/out/engine-backed-pgwire-benchmark-smoke/metrics.jsonl"
    grep -q 'int4_composite_equality_multi_column_projection' "$tmp_dir/out/engine-backed-pgwire-benchmark-smoke/endpoint-facts.txt"
    grep -q 'int4_equality_mixed_column_projection' "$tmp_dir/out/engine-backed-pgwire-benchmark-smoke/engine-backed-pgwire-benchmark-smoke.md"
    ...
    GPU_DB_CH_BENCH_OUT_DIR="$tmp_dir/out" GPU_DB_CH_BENCH_IDENTICAL_PGWIRE_CONCURRENCY_TARGETS=1,2 \
      "$0" --identical-pgwire-full-readiness >"$tmp_dir/identical-readiness.out"
    grep -q 'full_25pct_identical_curves_require_operator_long_run' "$tmp_dir/out/identical-pgwire-full-readiness/readiness.md"
    grep -q '"full_load_sql_artifact":false' "$tmp_dir/out/identical-pgwire-full-readiness/readiness.jsonl"
