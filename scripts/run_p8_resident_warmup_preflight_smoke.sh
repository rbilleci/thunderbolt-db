...
grep -q '^entry.route.query_shape=sharded_count_all$' "$TMP_DIR/dry-run.out"
grep -q '^entry.route.h2d_bytes_if_resident=0$' "$TMP_DIR/dry-run.out"
grep -q '^entry.route.d2h_bytes_estimate=16$' "$TMP_DIR/dry-run.out"
...
grep -q '^entry.route.query_shape=sharded_count_all$' "$TMP_DIR/apply.out"
grep -q '^entry.route.h2d_bytes_if_resident=0$' "$TMP_DIR/apply.out"
grep -q '^entry.route.d2h_bytes_estimate=16$' "$TMP_DIR/apply.out"
...
grep -q '^entry.route.query_shape=sharded_count_all$' "$TMP_DIR/invalidated.out"
grep -q '^entry.route.d2h_bytes_estimate=24$' "$TMP_DIR/invalidated.out"
...
grep -q '^entry.route.accepted=none$' "$TMP_DIR/oversized.out"
