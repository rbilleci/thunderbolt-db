...
def classify(test_id: str) -> list[str]:
    ...
    if "psql_golden::" in test_id:
        ...
        if re.search(r"relational|create_table|insert|select|delete|update|column_defaults|column_default", test_id):
            buckets.append("sql.relational_foundation")
        if re.search(r"catalog|pg_catalog|introspection|schema|type|column_defaults|column_default|attrdef", test_id):
    ...
    if "gpu_db_protocol::" in test_id:
        ...
        if re.search(r"relational|create_table|insert|select|column_defaults|column_default", test_id):
            buckets.append("sql.relational_foundation")
    if "gpu_db_server::" in test_id and re.search(
        r"simple_query|frontend|protocol|session_reset|copy|truncate", test_id
    ):
        buckets.append("protocol.client_flows")
    ...
    if re.search(r"extended|bind|parse|portal|prepared|parameterized|cursor|fetch_count|gdesc", test_id):
        buckets.append("protocol.extended_query")
    if re.search(r"relational|create_table|insert|select|delete|update|truncate|column_defaults|column_default", test_id):
        buckets.append("sql.relational_foundation")
    ...
    if re.search(
        r"relational_catalog|catalog_helpers|catalog_schema|catalog_introspection|shared_catalog|pg_catalog|pg_type|pg_namespace|information_schema|type_metadata|database_metadata|pg_dump|column_id|relation_oid|create_index|catalog_index|column_default|attrdef",
        test_id,
    ):
        buckets.append("sql.catalog_schema_types")
