...
def classify(test_id: str) -> list[str]:
    ...
    if "psql_golden::" in test_id:
        ...
        if re.search(r"extended|bind|parse|portal|prepared|parameterized|cursor|fetch_count", test_id):
    ...
    if re.search(
        r"relational_index|access_path|equality_index|checkpoint_vacuum|checkpoint_control|prune_versions|retention",
        test_id,
    ):
        buckets.append("storage.indexing")
    if re.search(
        r"relational_catalog|catalog_helpers|catalog_schema|catalog_introspection|pg_catalog|pg_type|pg_namespace|information_schema|type_metadata|column_id|relation_oid",
        test_id,
    ):
        buckets.append("sql.catalog_schema_types")
