...
if [[ -z "$PYTHON_BIN" ]]; then
  ...
  if [[ -n "$PATH_PYTHON" ]] && python_has_pip "$PATH_PYTHON"; then
    ...
    echo "asyncpg smoke requires Python 3.14 with pip; set PYTHON_BIN to a compatible interpreter" >&2
    exit 1
  fi
elif ! python_has_pip "$PYTHON_BIN"; then
  echo "asyncpg smoke PYTHON_BIN must name an executable Python 3.14 interpreter with pip" >&2
  exit 1
fi

DEPS_DIR=$(mktemp -d "${TMPDIR:-/tmp}/gpu-db-asyncpg-deps.XXXXXX")
cleanup() {
  rm -rf "$DEPS_DIR"
}
trap cleanup EXIT
