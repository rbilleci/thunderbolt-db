...
- Raw frontend `CopyData` and `FunctionCall` frames are covered by helper-level protocol tests because PostgreSQL 16 `psql` does not emit them from stable golden-suite flows; that coverage pins the unsupported SQLSTATE, skip-until-`Sync`, skipped side-effect, and later recovery behavior.
...
- Named prepared-statement and named-portal duplicate handling, duplicate portal precedence over malformed payloads, missing raw `Bind` target statements with otherwise unsupported binary formats, missing named extended-`Close` targets, SQL `PREPARE` state isolation for extended `Close Statement`, and statement-close cascade invalidation of dependent portals before later Execute recovery are covered by helper-level protocol tests because PostgreSQL 16 `psql` does not emit stable named `Parse`/`Bind`/`Close` metacommand traffic for the golden suite; unnamed replacement remains covered in the same lifecycle tests.
- Invalid raw UTF-8 bind payloads and mismatched raw parameter/result-format code counts are covered by helper-level protocol tests because PostgreSQL 16 `psql` does not emit those malformed wire shapes from stable golden-suite metacommands.
