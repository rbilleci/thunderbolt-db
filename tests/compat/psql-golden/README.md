...
- Named prepared-statement and named-portal duplicate handling, missing named extended-`Close` targets, and SQL `PREPARE` state isolation for extended `Close Statement` are covered by helper-level protocol tests because PostgreSQL 16 `psql` does not emit stable named `Parse`/`Bind`/`Close` metacommand traffic for the golden suite; unnamed replacement remains covered in the same lifecycle tests.
