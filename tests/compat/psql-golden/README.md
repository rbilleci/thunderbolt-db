...
- Named prepared-statement and named-portal duplicate handling plus missing named extended-`Close` targets are covered by helper-level protocol tests because PostgreSQL 16 `psql` does not emit stable named `Parse`/`Bind`/`Close` metacommand traffic for the golden suite; unnamed replacement remains covered in the same lifecycle tests.
