...
CREATE TABLE truncate_restart_people (id SERIAL, name TEXT);
INSERT INTO truncate_restart_people (name) VALUES ('Ada'), ('Grace');
SELECT id, name FROM truncate_restart_people ORDER BY id;
TRUNCATE TABLE public.truncate_restart_people RESTART IDENTITY;
SELECT id, name FROM truncate_restart_people ORDER BY id;
SELECT last_value, is_called FROM public.truncate_restart_people_id_seq;
INSERT INTO truncate_restart_people (name) VALUES ('Linus');
SELECT id, name FROM truncate_restart_people ORDER BY id;
...
TRUNCATE TABLE truncate_teams CONTINUE IDENTITY;
