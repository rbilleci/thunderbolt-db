...
The transactional-DDL boundary stages every admitted `CREATE TABLE`, `CREATE VIEW`, and `CREATE OR REPLACE VIEW` in
the explicit transaction's single ordered operation stream, composes them with DML and typed resets, and carries the
resulting private catalog through Parse/Describe and the existing `engine_transaction_delta` claimant. Current binary
WAL binds each catalog ordinal; every catalog, DML, and reset statement's typed digest even when its relational effect
is empty or later shadowed; created-table OID/schema and allocator post-state; created and referenced sequence
identities; each view target's exact preimage/stable kind/OID/definition digest/postimage and complete transitive
source-relation closure; and every mixed operation's exact family and target before one atomic publication and
deterministic replay. View opcodes 12/13 are additive to the accepted opcode 4–11 representation. It adds no WAL or
