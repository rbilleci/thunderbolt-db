...
This is a factual inventory captured and updated on 2026-07-25 for the PRODUCT-001 consolidation. It does not
...
|---|---|---|---|
| `thunderbolt-db-server` (`gpu_db_server` bin) | `crates/server/src/main.rs` plus the bounded `lib`, `security`, `transport`, `cancellation`, `async_submit`, `wire_response`, `extended`, `copy`, `sql_prepared`, `sql_cursor`, and `sql_session` module owners | One session-owned `SharedEngine::submit` boundary plus effect-free prepare/describe over the real engine | Engine-backed simple query plus one shared Parse/Bind/Describe/Execute/Close portal lifecycle, transaction-private Parse/Describe, and statement-ordered table, stored-view, index, sequence lifecycle, DML, and typed table-reset composite transactions. The admitted sequence lifecycle family is `CREATE SEQUENCE`, `ALTER SEQUENCE ... RESTART`, `ALTER SEQUENCE ... RENAME TO ...`, ordered multi-target `DROP SEQUENCE [IF EXISTS]`, and `TRUNCATE ... RESTART IDENTITY`; every operation remains behind the same transaction/WAL/publication owner. Ordinary `nextval`, both `setval` forms, and omitted INSERT defaults on unchanged published identities use separate typed durable value transitions through that owner, while the later user envelope references their stable-OID outcomes and facade session state retains operation-specific `currval`. The target also provides bounded GPU scalar projection, atomic ordinary multi-statement Query messages, typed text/CSV COPY FROM/TO, and bounded SQL PREPARE/EXECUTE/DEALLOCATE plus DECLARE/FETCH/CLOSE session control. COPY FROM retains and revalidates its analyzed relation generation through CopyDone and enters the canonical transaction/WAL/publication boundary. An explicit local-dev trust profile and fail-closed production TLS/SCRAM-SHA-256 profile wrap the same dispatcher. Startup advertises the PostgreSQL 16 version number and standard-conforming strings among six `ParameterStatus` fields, then emits one random `BackendKeyData`; direct or TLS-contained exact-key CancelRequest connections silently signal only the current request generation. Typed SHOW/isolation and SET TRANSACTION state, text/binary codecs for `int2`/`int4`/`int8`/finite `numeric`/`bool`/`text`/`date`/`timestamp`/`uuid`, NUMERIC typmod, typed constraint/range SQLSTATEs, exact-once suspended portals, failed-transaction recovery/chain handling, and bounded pool cleanup remain session control around the same facade owner. Blocking and async COPY, queued async facade/metadata work, effect-free result encoding, extended Sync recovery, malformed/wrong/stale/idle keys, and post-cancel reuse share that one server-local registry. Every post-submission facade error and successful mutation/RETURNING outcome is preserved. PostgreSQL 16 psql 04/06/07, unchanged R2DBC autodetection, all 18 pg_dump/pg_restore formats and restore variants, and bounded pg_dumpall globals restore now use pinned, complete catalog candidate relations with typed GPU filters, joins, aggregate, final projection/gather, and ordering through this same boundary. Materialized-view lifecycle and every catalog family not named above remain outside this target and fail before database effects unless PLAN records their later acceptance. |

Cargo metadata reports exactly this one pgwire binary target. The superseded protocol listener, its
host-relational module tree, both direct-engine P8 protocol adapters, and their P8 benchmark launchers are deleted;
the historical P8 `order_line` workload is not a product or BENCH-001 harness.

The canonical target's TLS/SCRAM surface above passed its independent PRODUCT-001 audit on 2026-07-21. It wraps the
same dispatcher and adds no facade submission, WAL, sequence, or publication authority. The canonical target's
accepted keyed-cancellation slice resolves its
...
Exact source/restored sequence ACL and isolated inherited-default checks close the archive-fidelity boundary. The
current compatibility candidate additionally resolves SQL-visible role names, ACL terminal policy, psql
`format_type` cells, and complete `\df[+]` filtering/type/owner/language/comment/ACL formatting through typed GPU
operators. Relation-detail referenced-by and publication membership plus shared-object comments join complete raw
catalog candidates on the GPU, while `\gdesc` performs one final device-result readback without rebuilding and
reuploading a host relation. DML `RETURNING` requires a separate device SELECT verdict. No new execution or mutation
authority is added.
The accepted SQLSTATE/type-codec, named-client, and mixed-recovery slice extends that same dispatcher without adding
an execution, WAL, recovery, sequence, or publication owner. PostgreSQL text/binary values and NULLs cross the
neutral facade for all nine types; NUMERIC precision/scale reaches RowDescription and the GPU catalog; typed engine
errors, rather than message inspection, select constraint/range SQLSTATEs. All eight named client families exercise
the surface. Process-level SIGKILL recovery reopens the exact canonical WAL twice after mixed W1, General,
transaction, COPY, and sequence traffic, then appends through the same live authority.
...
The transactional-DDL boundary stages every admitted table, stored-view, index, and sequence lifecycle command in
the explicit transaction's single ordered operation stream, composes them with DML and typed resets, and carries the
resulting private catalog through Parse/Describe and the existing `engine_transaction_delta` claimant. Current
binary WAL binds each catalog ordinal; every catalog, DML, and reset statement's typed digest even when its
relational effect is empty or later shadowed; created-table OID/schema and allocator post-state; created and
referenced sequence identities; exact stored-view and index lifecycle proofs; and every mixed operation's exact
family and target before one atomic publication and deterministic replay. Sequence lifecycle opcodes 18/19 add
stable sequence OID, exact pre/post descriptor, column-default dependency closure, reset ordinal, and stable-OID
value post-state without changing opcode 4–17 records. A lifecycle or owned-reset identity must select this opcode
family; value post-state alone is rejected by both encoder and decoder. Generated SERIAL output keeps the
creator-name-to-OID binding captured by its CREATE TABLE ordinal: encoder, post-decode validation, and clone-first
apply require exact output/input equality, while stepwise replay validates every generated or existing sequence
default immediately after its CREATE TABLE before a later private rename can change the name. Final identity checks
remain stable-OID based across that rename. Current sequence rename rewrites dependent defaults and retains its
OID/comments; current drop refuses live default dependencies and drop/recreate receives a new OID. Historical
generic-SQL replay retains its acknowledged pre-boundary rename/drop behavior independently of the current
shared-index namespace epoch. Genuine V1 canonical fixtures cover that post-epoch namespace/dependency composition
through complete and split recovery rather than relying on the current encoder's V2 canonicalization. A table reset
staged before metadata-authorized index/sequence lifecycle is catalog-locally rebound to the final schema/root/
dependency proof only when the stable table and exact dependency OID closure are unchanged; its owned-sequence
reset identity remains fixed at the original statement ordinal. It adds no WAL or publication authority.
The canonical simple-query server uses this envelope for an ordinary multi-statement Query message, so a failing
statement rolls back every predecessor and suppresses every successor. Every other transactional catalog family
remains fail-closed under PRODUCT-001. In particular, role and ACL DDL inside an explicit transaction is rejected
before overlay/WAL/publication effect; a retained repeatable-read catalog generation proves old role/grant
visibility across concurrently published revoke/rename/drop instead of widening that WAL envelope. An intervening
DML/KV commit no longer forces a retry when every catalog field plus the OID/column-ID allocator high-waters
remains unchanged: READ COMMITTED rebinds the private overlay and GPU delta
to the newer publication stamp, while REPEATABLE READ validates the same content proof at COMMIT. Real catalog or
allocator drift remains a pre-WAL serialization failure. Autocommit and active-transaction `TRUNCATE` use a typed,
statement-ordered empty-root barrier with stable table/dependency guards, GPU source/empty proofs, binary WAL/replay,
table-root ledger validation, and the ADR-014 monotonic non-MVCC rewrite fence; that barrier also targets a preceding
transaction-private create. `RESTART IDENTITY` additionally carries the exact owned-sequence reset closure and
publishes its private value overlay atomically with the empty table root. Ordinary operations on an unchanged,
published sequence remain distinct from this transactional lifecycle/reset boundary: typed opcodes 20/21 encode
separately durable `nextval`, both `setval` forms, and default transitions plus exact later-envelope references.
Stable OIDs bind default rows and facade `currval`; recovery restores the durable transition/outcome index without
reconstructing session state. This inventory records those live facts; **PRODUCT-001** in PLAN remains the sole
owner of acceptance sequencing and remaining work.
...
- 352 PostgreSQL-16 psql golden SQL scenarios with 352 expected outputs under
  `tests/compat/psql-golden/`, driven by `scripts/run_psql_golden.sh`; the exact candidate passes **352/352** with
  PostgreSQL 16.14 explicitly selected and a fresh endpoint per scenario. Five expected artifacts
  (316/317/318/322/324) carry PostgreSQL 16 primary-key-first and exact `\dd` pattern corrections;
- `tests/compat/pg-dump/run.sh` and `tests/compat/pg-dumpall/run.sh`, now booting only
  `thunderbolt-db-server` for all generation, restore, metadata, privilege, and globals checks;
- the Rust SQLx and tokio-postgres integration tests in `crates/server/tests/`, now running unchanged client
  behavior against `thunderbolt-db-server`, including all nine native types, NULLs, constraint SQLSTATEs, typed
  COPY, failed-transaction recovery, and process-level durable reopen;
- application-driver smokes under `tests/compat/{node-postgres,asyncpg,psycopg,jdbc,r2dbc,pgx}/`; node-postgres,
  asyncpg, psycopg, pgx, JDBC, and R2DBC now boot the canonical binary and exercise native all-type/NULL rows,
  typed constraint errors, rollback, and reuse. R2DBC keeps its unchanged connection-time extension-autodetection
  query `SELECT oid, * FROM pg_catalog.pg_type WHERE typname IN
...
- the production TLS/SCRAM posture and live engine-backed handshake/mutation/read checks against
  `thunderbolt-db-server` in `scripts/run_connection_security_posture_preflight.sh`;
...
- `scripts/run_local_validation_preflight.sh`, which runs `scripts/check_product_ownership.sh`, selects fresh
  endpoint/catalog isolation for each golden scenario by default, and boots the canonical binary for the suite; and
- `scripts/check_product_ownership.sh`, which verifies Cargo metadata, the deleted-path set, the absence of a
  superseded executable surface, the sole facade entry, and the canonical commit/WAL/publication evidence.
