...
This is a factual inventory captured and updated on 2026-07-23 for the PRODUCT-001 consolidation. It does not
...
The transactional-DDL boundary stages every admitted `CREATE TABLE`, `CREATE [OR REPLACE] VIEW`,
`ALTER VIEW ... RENAME TO ...`, and ordered multi-target `DROP VIEW [IF EXISTS]` in the explicit transaction's
single ordered operation stream, composes them with DML and typed resets, and carries the resulting private catalog
through Parse/Describe and the existing `engine_transaction_delta` claimant. Current binary WAL binds each catalog
ordinal; every catalog, DML, and reset statement's typed digest even when its relational effect is empty or later
shadowed; created-table OID/schema and allocator post-state; created and referenced sequence identities; each view
target's exact pre/post name, stable kind/OID, definition digest, complete transitive source-relation closure, and
typed absence; and every mixed operation's exact family and target before one atomic publication and deterministic
replay. Create-only view opcodes 12/13 remain byte-for-byte stable; lifecycle opcodes 14/15 add rename/drop and
mixed-operation identity without changing opcode 4–13 records. Rename retains OID, ACL, and comments, while
drop/recreate receives a new identity. It adds no WAL or publication authority.
...
allocator drift remains a pre-WAL serialization failure. Autocommit and active-transaction `TRUNCATE ... CONTINUE
IDENTITY` use a typed, statement-ordered empty-root barrier with stable table/dependency guards, GPU source/empty
proofs, binary WAL/replay, table-root ledger validation, and the ADR-014 monotonic non-MVCC rewrite fence; that
barrier also targets a preceding transaction-private create. `RESTART IDENTITY` remains pre-effect rejected under
**PRODUCT-001**.
