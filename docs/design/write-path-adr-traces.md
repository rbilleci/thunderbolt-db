# R3-001 decision-level ACID and failure traces

This report closes the design-trace portion of the proposed
[`write-path-adr-proposal.md`](write-path-adr-proposal.md). It is reviewed against the pinned source baseline
`f701d8b6e9e0a9a1904bc990f23162632f382045` plus the verified R3-006 correction recorded in
[`write-path-adr-evidence.md`](write-path-adr-evidence.md). It does not claim that the canonical transaction,
WAL, checkpoint, or recovery implementation exists. Those graduation proofs remain with the PLAN owners named
below; this document owns no work.
...
These decision traces are exercised by the build-only model in
[`write-path-adr-controller-injections.md`](write-path-adr-controller-injections.md). That bounded evidence selects
the controller rules; real queue/device/STRATA latency and sabotage remain post-acceptance production graduation.
...
Every transition required by the proposal's design-acceptance list now has an explicit authority, visible state,
...
**closed at decision level**. This does not waive the failed Candidate-A physical-selection gate or the final
independent acceptance review. The bounded RTO argument is recorded separately, and neither report substitutes for
the post-acceptance DUR-002/R3-003 destructive fault campaign.
