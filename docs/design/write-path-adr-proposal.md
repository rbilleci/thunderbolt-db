...
facts from target rules, and are review evidence rather than implementation authority.
The independent performance review and adopted dispositions are in
[`write-path-adr-performance-audit.md`](write-path-adr-performance-audit.md).
The independent durability/resilience review and adopted dispositions are in
[`write-path-adr-durability-audit.md`](write-path-adr-durability-audit.md).
The independent transactional ACID review and adopted dispositions are in
[`write-path-adr-acid-audit.md`](write-path-adr-acid-audit.md).
The independent consistency/accuracy review and adopted dispositions are in
[`write-path-adr-consistency-audit.md`](write-path-adr-consistency-audit.md).
The measured synchronous-commit SLO and actual narrow-path footprint decision is in
[`write-path-adr-slo-footprint.md`](write-path-adr-slo-footprint.md).
The common durability-floor measurement and bounded physical Candidate-A/Candidate-B selection are in
[`write-path-adr-physical-selection.md`](write-path-adr-physical-selection.md).
The bounded cold/index/lag/skew/pressure/controller injections are in
...
and corrected the pump, resize barrier, sequence exhaustion, and cold-checkpoint boundary. The focused evidence is
...
the GPU across 8/32/128-byte rows, 1/3/6 indexes, and batch sizes 1/256/4,096. Compact append/tombstone is faster in
...
[`write-path-adr-review-matrix.md`](write-path-adr-review-matrix.md). That companion is a review index, not a work
ledger or acceptance decision. The reviewed decision-level transactional and failure schedules live in
[`write-path-adr-traces.md`](write-path-adr-traces.md). The failed Candidate-A decision evidence lives in
...
- evidence-harness injections for cold staging, index rebuild, both durable/apply lag directions, sparse-lane/global
  skew, and pressure hysteresis sufficient to validate the proposed service/admission rules without making an
...
[`write-path-adr-traces.md`](write-path-adr-traces.md) supplies the complete reviewed decision-level ACID/failure
schedules; and [`write-path-adr-slo-footprint.md`](write-path-adr-slo-footprint.md) records the current Candidate-A
base matrix and exact narrow-path bytes. That current implementation gate **failed** and remains a production
graduation failure. [`write-path-adr-physical-selection.md`](write-path-adr-physical-selection.md) isolates the
