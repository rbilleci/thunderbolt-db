...
14. For local object-bundle backup proof, export a validated archive with `Engine::export_durable_wal_archive_object_backup(...)` and restore it with `Engine::restore_durable_wal_archive_object_backup(...)`; restore verifies every manifest/segment object length and checksum, then cross-checks the manifest object bytes against the backup manifest metadata before installing the restored archive manifest.
15. Treat physical page-image base backups, production object-storage APIs, production timeline fork management, and live background cleanup scheduling as not yet implemented.
