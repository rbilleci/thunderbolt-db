...
| P2 all-schema relation describe note | 🟡 | 🟡 | ✅ | Real PostgreSQL 16 `psql \d *.*` all-schema relation description returns supported `public` session-table relation rows from catalog metadata, then reuses existing per-table describe probes for supported columns/types; unsupported schemas and broader relation kinds remain out of scope |
