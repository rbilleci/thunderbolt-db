...
| P2 information-schema views note | 🟡 | 🟡 | ✅ | Common `information_schema.views` discovery for the supported `public` schema returns no rows for the current no-SQL-view subset; view definitions, view DDL, and broader view catalog behavior remain pending |
