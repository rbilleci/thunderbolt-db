...
| Full PostgreSQL wire protocol compatibility | 🚫 | 🚫 | 🟡 | P3 first slice supports `Parse`/`Bind`/`Describe`/`Execute`/`Sync`/`Close` for text-format prepared relational `SELECT` portals with `int4`/`text` parameters, session-local statement/portal close lifecycle, strict bind parameter arity, real-client described-portal execution through psql `\bind`, and nonzero `Execute.max_rows` batching for supported relational `SELECT` portals via `PortalSuspended` plus later `Execute` resume; real PostgreSQL 16 `psql` `FETCH_COUNT` cursor flow now works for supported relational `SELECT` statements via session-local `DECLARE ... CURSOR FOR SELECT`, `FETCH FORWARD n`, and cursor `CLOSE` handling; real PostgreSQL 16 `psql \gdesc` now works for supported relational `SELECT` descriptions, including psql's follow-up `pg_catalog.format_type` query over described `int4`/`text` result OIDs; binary formats, copy/function-call flows, and advanced portal/cursor behavior remain unsupported |
