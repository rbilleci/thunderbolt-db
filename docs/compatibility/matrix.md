...
| Session-control parser no-op aliases (`RESET`/`DISCARD`/`DEALLOCATE`/`CLOSE ALL`/`LISTEN`/`UNLISTEN` incl. `UNLISTEN ALL`) | ✅ | ✅ | ✅ | PostgreSQL-style client reset/setup probes accepted without mutating state |
