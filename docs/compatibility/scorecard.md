...
1. Use test names that include the target behavior keyword, for example:
   ...
   - `relational_catalog`, `catalog_schema`, `catalog_introspection`, `pg_catalog`, `pg_type`, `pg_namespace`, `information_schema`, `type_metadata`, `column_id`, `relation_oid`, `create_index`, `catalog_index`, `view`, `drop_view`, `truncate_table`, `primary_key`, or `constraint` for the P2 catalog/schema/type bucket
