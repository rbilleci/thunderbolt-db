...
1. Use test names that include the target behavior keyword, for example:
   ...
   - `relational`, `create_table`, `insert`, `select`, `truncate`, `primary_key`, or `constraint` for the P1 relational SQL foundation bucket
   ...
   - `relational_catalog`, `catalog_schema`, `catalog_introspection`, `pg_catalog`, `pg_type`, `pg_namespace`, `information_schema`, `type_metadata`, `column_id`, `relation_oid`, `create_index`, `catalog_index`, `view`, `domain`, `drop_view`, `truncate_table`, `primary_key`, or `constraint` for the P2 catalog/schema/type bucket
