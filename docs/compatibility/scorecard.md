...
1. Use test names that include the target behavior keyword, for example:
   ...
   - `relational_catalog`, `catalog_schema`, `catalog_introspection`, `pg_catalog`, `pg_type`, `type_metadata`, `column_id`, or `relation_oid` for the P2 catalog/schema/type bucket
   - `operational_replication`, `cluster_smoke`, `deployment_preflight`, `append_entries_transport`, `read_after_apply`, or `failover` for the P6 local operational replication bucket
