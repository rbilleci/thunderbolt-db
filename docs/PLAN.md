...
1. **STRUCT-001Y — extract device write-locate ownership.** Move the write/visible-locate contracts, PTX, and
   launchers into one private execution module while preserving stable crate-root APIs and GPU behavior.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001Y write-locate extraction active; root now 20,589 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (953), `cuda_driver.rs` (883), `resident_memory.rs` (405), `resident_sort.rs` (846), `resident_count.rs` (1,066), `resident_gather.rs` (258), `resident_filter.rs` (1,221 after STRUCT-001T safety hardening), `resident_aggregate.rs` (866 after STRUCT-001V safety hardening), `resident_group.rs` (1,282), `group_input.rs` (639), `point_read_submission.rs` (442), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), `point_read_text.rs` (1,205 after STRUCT-001M/N correctness hardening), and `point_read_rows.rs` (475). STRUCT-001X replaced safe grouped raw-pointer/mode APIs with typed resident/derived/text/composite descriptors, exact logical extents, owner lifetimes, and CUDA-context identity. Host preflight now covers every indexed window and mode; PTX bounds direct and representative varlen spans and fails closed through a device error flag; timed events and default-stream errors are RAII/drain safe. The exact 116-test inventory passes 42/74. Ten grouped safety/type/NULL/overflow/two-level gates passed 30 sequential and 20 concurrent invocations without CUDA 700/716/717, plus the final focused safety gate passed three sequential invocations. Independent audit is clean. The canonical card kept grouped aggregation at 1,678.0 M elements/s, count at 0.88x/1.01x roofline, and 65,536-batch point reads at 247.4M/251.4M lookups/s with p50 138/132us in-L2/out-of-L2. **Next:** extract the cohesive write/visible-locate boundary without deciding R3 architecture. |
...
|---|---|---:|---|---|---|
| **STRUCT-001Y** | NOW | P0 | Extract `WriteLocateShard`, `WriteLocateResult`, `VisibleLocateShard`, `VisibleLocateResult`, both locate PTX programs, and `submit_multi_shard_i32_{write,visible}_locate` into a private `execution::write_locate` module below 1,500 lines. Preserve stable crate-root re-exports and method signatures, descriptor packing, index probing, duplicate/overflow semantics, MVCC visibility, pinned ownership assumptions, stream/error ordering, and GPU-only addressing. Prove normalized moved bodies and PTX byte identity, exact test inventory, workspace all-target/features checks, write/visible-locate GPU HAZARD gates 3x sequential and 2x concurrent, and independent audit. Any safety or architecture finding is promoted ahead of further decomposition; this structural move does not choose the R3 version/index/CC design. | STRUCT-001X complete | `CODE_SIZE.md`; GPU-native write charter |
