...
1. **STRUCT-001AN — isolate legacy bind/describe ownership.** Map inbound callers and move the cohesive
   parameter inference, placeholder rewriting, bind decoding, and describe-shape leaf behind the stable ready-
   loop/statement boundary. Keep this legacy compatibility logic private and do not turn it into a host product
   execution API.
...
|---|---|---:|---|---|---|
| **STRUCT-001AN** | NOW | P0 | Extract the analyzed 1,152-line/47-function legacy bind/describe cluster (current root lines 19,127–20,278): parameter-count/type inference, SQL EXECUTE argument decoding, placeholder-safe rewriting, bind literal encoding/errors, and result-column description. Exactly 15 functions have production callers outside the cluster: `bind_query_parameters`, `format_code_count_is_valid`, `expected_parameter_count`, `resolve_prepared_parameter_type_oids`, `sql_execute_parameter_type_mapping_error`, `sql_execute_argument_placeholder_index`, `is_supported_extended_dml`, `max_placeholder_index`, `contains_zero_placeholder`, `strip_sql_comments`, both bind/SQL-EXECUTE error mappers, both negative LIMIT/OFFSET errors, and `describe_query_columns`; keep only these parent-private. Co-locate focused direct-helper tests for placeholder replacement/dummy literals/describe-shape so remaining helpers stay module-private; do not blanket-export the cluster. Preserve error precedence/text, quoted/comment placeholder semantics, type OIDs, negative LIMIT/OFFSET describe handling, and stable ready-loop behavior. Run exact affected tests plus full protocol/driver/security gates and independent audit. | STRUCT-001AM | PRODUCT-001 containment; extended protocol boundary |
