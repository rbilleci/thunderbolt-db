...
1. **STRUCT-001X — make grouped aggregate inputs total.** Replace grouped raw-pointer/flag inputs with typed,
   extent-carrying borrowed descriptors; validate every fixed, text, derived, and composite indexed window
   before CUDA mutation; make text spans fail closed on-device; and harden the timed benchmark lifecycle.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001X grouped-input safety child active; root now 20,618 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `resident_sort.rs` (846), `resident_count.rs` (1,066), `resident_gather.rs` (258), `resident_filter.rs` (1,221 after STRUCT-001T safety hardening), `resident_aggregate.rs` (866 after STRUCT-001V safety hardening), `resident_group.rs` (1,207), `point_read_submission.rs` (442), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), `point_read_text.rs` (1,205 after STRUCT-001M/N correctness hardening), and `point_read_rows.rs` (475). STRUCT-001V checks every scalar aggregate maximum index with overflow-safe arithmetic before CUDA mutation. STRUCT-001W moved the grouped public result contract and both production/timed launchers byte-identically behind `resident_group`; public root APIs/re-export, all type/NULL/mask/overflow/device-compaction semantics, and benchmark event scope remain unchanged. The exact 109-test inventory and 36/73 suite remain stable. Nine grouped type/NULL/overflow/two-level gates passed 27 sequential and 36 concurrent invocations with no CUDA 700/716/717. The canonical card kept grouped aggregation at 1,677.7 M elements/s, count at 0.87x/1.00x roofline, and 65,536-batch point reads at 251.1M/257.5M lookups/s with p50 136/127us in-L2/out-of-L2. Independent audit found no extraction defect and promoted unchecked grouped fixed/text/derived/composite input extents plus timed-launcher cleanup as STRUCT-001X. **Next:** establish typed borrowed grouped descriptors and total preflight/device span validation. |
...
|---|---|---:|---|---|---|
| **STRUCT-001X** | NOW | P0 | Make all four safe grouped APIs total: `group_by_i32_count_sum_from_payload`, `group_by_i32_count_sum_minmax_from_payload`, and both benchmark entries. Replace the public raw-pointer/boolean mode soup with typed borrowed key/value descriptors carrying logical row count and exact fixed, text, derived-buffer, and composite extents; keep raw `u64` kernel arguments private. Reject nonempty `max_index >= row_count`, invalid mode combinations, zero timed runs, arithmetic overflow, or any section/descriptor extent violation before context binding, leases, or launch. Extend text/composite descriptor ABIs with byte limits and guard every current/representative span with `start <= end <= limit` on-device, reporting bounded failure without host relational fallback. Use RAII events and guaranteed error-path stream draining for the timed launcher while preserving its group-kernel-only event brackets. Preserve empty production/timed semantics, aggregate masks, dedicated NULL slots, one/two-pass behavior, dense result order, and numeric overflow. Add pure all-mode boundary tests, real-GPU reject-before-mutation/fail-closed/reuse tests, full grouped type/NULL/overflow HAZARD gates, independent audit, and the canonical card. | STRUCT-001W independent audit HIGH/MEDIUM | Safe CUDA API boundary; GPU-native aggregate charter |
