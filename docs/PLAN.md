...
1. **STRUCT-001BA — isolate session-compatibility routing and delete its dead duplicate.** Move the bounded
   pg_dump SET/RESET/LOCK controls and fixed session-introspection queries into a private owner behind delegates
   on their existing sides of DDL routing; remove only the identical post-parse branches dominated by earlier
...
|---|---|---:|---|---|---|
| **STRUCT-001BA** | NOW | P0 | Analyze and extract the bounded session-compatibility family from `execute_statement` into a private `session_compat` owner: pg_dump SET recognition, RESET search_path, ACCESS SHARE LOCK acknowledgement, fixed search-path/restrict-kind `set_config` queries, advisory unlock, recovery status, and current schemas. Preserve the two existing routing positions around DDL with two parent-private tri-state delegates and the already-computed canonical input. Prove and delete only the later SET/RESET/LOCK/set_config/advisory/recovery branches that are byte-identical to and dominated by earlier unconditional returns; retain nonduplicate behavior and exact columns/rows/tags/fallthrough. Add no session capability or host product API. Run focused pg_dump/psql/DDL tests, full protocol/driver/security gates, formatting/diff/dead-symbol checks, and independent audit. | STRUCT-001AZ, STRUCT-001AV analysis | PRODUCT-001 containment; obsolete compatibility cleanup |
