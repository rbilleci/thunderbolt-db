...
1. **STRUCT-001BH — isolate view and materialized-view DDL.** Move create/refresh/rename/drop lifecycle arms into
   a private view owner, preserving permissions/dependencies, snapshot materialization, comments/dirty state,
...
|---|---|---:|---|---|---|
| **STRUCT-001BH** | NOW | P0 | Analyze and extract the complete successfully parsed view/materialized-view DDL family from `execute_statement` into a private `view_ddl` owner: CREATE VIEW, CREATE/REFRESH MATERIALIZED VIEW, ALTER VIEW/MATERIALIZED VIEW RENAME, and DROP VIEW/MATERIALIZED VIEW. Preserve exact schema permission and name/kind/existence precedence, bounded SELECT validation and dependency checks, materialized snapshot row construction, checked OIDs, rename/drop dependency guards, comment retarget/removal, dirty old/new keys, snapshot persistence, errors/tags, and fallthrough behind one parent-private delegate. Keep function/sequence/domain commands in the root and add no view capability or host product API. Run focused view/materialized-view/comment/pg_dump/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BG, STRUCT-001AV analysis | PRODUCT-001 containment; legacy view catalog boundary |
