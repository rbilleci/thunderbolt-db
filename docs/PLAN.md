...
1. **STRUCT-001BR — isolate legacy select execution.** Move the coherent select predicate, ordering,
   projection, aggregate, materialization, and simple-query SELECT family into a private owner below the
   1,500-line target, preserving all existing consumers, ACL/view behavior, and exact result/error semantics.
...
|---|---|---:|---|---|---|
| **STRUCT-001BR** | NOW | P0 | Analyze and move the coherent legacy select-execution cluster into a private `select_execution` owner: value comparison; SELECT/DELETE predicate evaluation; grouped HAVING; table/view/materialized-view SELECT; scalar/grouped aggregates; projection/order/distinct/limit/offset; result formatting/parsing/materialization; and the parsed simple-query SELECT response arm. Preserve all existing sibling consumers through narrow parent-private exports, exact ACL/view recursion, aggregate and wire-result behavior, errors, row-description handling, and parse-error fallthrough. Keep the new owner below 1,500 lines, normalize the two inherited rustfmt exceptions absorbed by the moved cluster, and leave function-body parsing, DML, and catalog query dispatch outside. Run focused select/aggregate/view/materialization/DML-filter/ACL tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BQ, STRUCT-001AV analysis | PRODUCT-001 containment; coherent legacy query owner |
