...
|---|---|---:|---|---|---|
| **STRUCT-001AN** | NOW | P0 | Analyze and extract the legacy server's bind/describe production cluster: parameter-count/type inference, SQL EXECUTE argument decoding, placeholder-safe rewriting, bind literal encoding/errors, and result-column description. First identify exact external callers and keep only those entry points parent-private; internal helpers remain module-private. Preserve error precedence/text, quoted/comment placeholder semantics, type OIDs, negative LIMIT/OFFSET describe handling, and stable ready-loop behavior. Run exact affected tests plus full protocol/driver/security gates and independent audit. | STRUCT-001AM | PRODUCT-001 containment; extended protocol boundary |
