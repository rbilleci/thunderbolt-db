...
1. **STRUCT-001BZ — isolate column-default semantics.** Move default type checks, catalog formatting, target/
   domain preflight, supported-add checks, and runtime evaluation into a private owner, preserving all DDL/DML/
   COPY/catalog consumers and exact type/value/sequence/error behavior through narrow exports.
...
|---|---|---:|---|---|---|
| **STRUCT-001BZ** | NOW | P0 | Analyze and move the coherent column-default cluster from the server root into a private `column_defaults` owner: `column_default_matches_type`; literal/default catalog formatting; sequence-target preflight; domain resolution; ADD COLUMN supported-default checks; and runtime literal/sequence evaluation. Preserve exact SQL value formatting, type/domain checks, sequence target/value mutation and errors, dirty behavior, and all `ddl_execution`, simple-DML, COPY, and catalog-query consumers. Export only the proven helpers, keep sequence primitives in `sequence_execution`, remove obsolete root definitions/imports, and add no product API or sibling cycle. Run focused default/domain/sequence/DML/COPY/catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BY, STRUCT-001AV analysis | PRODUCT-001 containment; coherent legacy default boundary |
