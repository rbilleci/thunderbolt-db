...
1. **STRUCT-001AO — isolate legacy frontend dispatch ownership.** Move the ready loop, frontend-message state
   machine, and unsupported-message mapping into one private owner. Bootstrap calls that owner; it may invoke
   existing statement/COPY handlers but must not absorb catalog or relational execution.
...
|---|---|---:|---|---|---|
| **STRUCT-001AO** | NOW | P0 | Analyze and extract the legacy ready-loop/frontend dispatcher (`handle_ready_client`, `handle_frontend_message`, and `unsupported_frontend_message`) into a private module. Preserve `ReadyLoopState` transitions, transaction status, skip-until-Sync recovery, COPY lifecycle, flush/terminate behavior, exact errors, and the single bootstrap entry point. Dependencies point inward to existing statement/COPY/extended handlers; do not move Session/catalog/DML ownership or expose a reusable host execution API. Run exact affected lifecycle/COPY/extended tests plus full protocol/driver/security gates and independent audit. | STRUCT-001AN | PRODUCT-001 containment; frontend state-machine boundary |
