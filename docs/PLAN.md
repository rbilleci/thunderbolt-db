...
1. **STRUCT-001O — isolate equal-row device selection ownership.** Move the resident int4 equality selector,
   PTX, bounded compaction, and async/blocking result lifecycle behind a focused private module while preserving
   the crate-root facade and explicitly retaining its host-ordering debt under **RETIRE-003**.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001O structural child active; root now 26,246 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `point_read_submission.rs` (442), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), and `point_read_text.rs` (1,205 after STRUCT-001M/N correctness hardening). STRUCT-001M made zero-int4-projection framing total, validates all result vector lengths against the device-reported match count, and carries nullable int4 filter validity into PTX before raw-value comparison. STRUCT-001N carries projected text validity through the device result and both engine/example consumers so SQL NULL cannot alias `''`; the focused retained GPU differential proves NULL/empty/nonempty results and no CPU fallback. All-target/downstream checks, exact 101-test inventory, 31 non-ignored tests, nullable-text/TEXT-only/pinned-pool HAZARD gates 3× sequential + 2× concurrent with zero CUDA safety errors, and independent audit passed. The final canonical card remained green: in-L2/out-of-L2 `count_i32_compare` 0.91x/1.01x roofline and 65,536-batch point reads 245.4M/253.9M lookups/s at p50 139/131us. The GPU regressions live in a 122-line focused test module rather than expanding the existing oversized `resident_route.rs`. **Next:** isolate the resident int4 equality row selector without legitimizing its RETIRE-003-owned host ordering. |
...
|---|---|---:|---|---|---|
| **STRUCT-001O** | NOW | P0 | Extract `launch_cuda_resident_i32_equal_row_indices` and its PTX, validation, pooled buffers/stream, async pinned and blocking readback, timing, draining, and ordering contract into a focused private module below 1,500 lines. Preserve `CudaResidentDeviceMemory::match_i32_equal_row_indices_from_payload` as the stable facade; prove byte-level move equivalence, exact test inventory, 3× sequential + 2× concurrent HAZARD safety, independent audit, and the canonical report card. Do not move or bless the adjacent CPU `between` selector; the existing host sort remains explicit **RETIRE-003** debt rather than product direction. | STRUCT-001N; equality-selector ownership packet | `CODE_SIZE.md`; ADR-006/007; RETIRE-003 |
