...
1. **STRUCT-001BK — isolate domain lifecycle commands.** Move CREATE/DROP DOMAIN arms into a private owner,
   preserving permissions/type/default/existence validation, dependency checks, comments/dirty publication,
...
|---|---|---:|---|---|---|
| **STRUCT-001BK** | NOW | P0 | Analyze and extract the complete successfully parsed domain family from `execute_statement` into a private `domain_ddl` owner: CREATE DOMAIN and DROP DOMAIN. Preserve exact schema permission and name/kind/existence precedence, supported base-type/default validation, checked OID allocation, table-column dependency rejection, IF EXISTS/list behavior, comment cleanup, dirty publication, snapshot persistence, errors/tags, and fallthrough behind one parent-private delegate. Keep publication/subscription/role commands in the root and add no domain capability or host product API. Run focused domain/default/comment/pg_dump/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BJ, STRUCT-001AV analysis | PRODUCT-001 containment; legacy domain catalog boundary |
