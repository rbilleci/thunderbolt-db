...
1. **STRUCT-001AT — isolate legacy COPY state and execution.** Move COPY value encoding, COPY TO/FROM setup,
   COPY data application, and completion/error handling into one private compatibility owner. Preserve framing,
   CSV/text behavior, mutation timing, and dirty publication without creating a host product execution API.
...
|---|---|---:|---|---|---|
| **STRUCT-001AT** | NOW | P0 | Analyze and extract the cohesive legacy COPY execution cluster: text/CSV value encoding, COPY TO row framing, COPY FROM state initialization, data buffering/parsing, row application, error recovery, dirty-table publication, and completion tags. Preserve delimiter/quote/escape/header/null behavior, permission and relation/column error precedence, CopyFail/no-mutation behavior, exact messages and protocol frames. Expose only proven parent-private callers; COPY statement parsing, DML, simple-query dispatch, catalog, and relational execution remain root dependencies, with no host product execution API. Run focused COPY DML/lifecycle tests plus full protocol/driver/security gates and independent audit. | STRUCT-001AS | PRODUCT-001 containment; COPY compatibility boundary |
