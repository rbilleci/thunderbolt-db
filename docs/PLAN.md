...
1. **STRUCT-001BY — complete sequence primitive ownership.** Move sequence target validation, next-value state
   transition, and implicit sequence creation into the existing private sequence owner, preserving exact kind/
   existence/OID/overflow/state/dirty behavior and only the cross-owner aliases required by column defaults.
...
|---|---|---:|---|---|---|
| **STRUCT-001BY** | NOW | P0 | Analyze and move `sequence_target_error`, `next_sequence_value`, and `create_implicit_sequence` from the server root into the existing private `sequence_execution` owner. Preserve exact relation-kind/existence precedence, overflow and `last_value`/`is_called` transition, checked relation-OID allocation, collision behavior, inserted sequence fields, dirty publication, errors, and helper arguments. Keep the three helpers `pub(super)` only because column-default preflight/evaluation and `ddl_execution` consume them through the parent; command calls become internal, old root definitions disappear, and no product API or sibling cycle is added. Run focused sequence/default/table/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BX, STRUCT-001BJ, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy sequence owner extension |
