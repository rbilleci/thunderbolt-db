...
1. **STRUCT-001AM — split legacy server tests by invariant.** After the exact seam move, partition catalog,
   COPY/DML, extended-query, cursor, and shared support ownership into sub-3,000-line modules without production
   visibility growth.
...
|---|---|---:|---|---|---|
| **STRUCT-001AM** | NOW | P0 | Split the intermediate legacy-server `tests.rs` below the 3,000-line test envelope by invariant: shared catalog/ACL, COPY+DML, catalog/introspection, extended bind/portal, cursor/SQL PREPARE, plus a bounded shared support owner. Preserve all full test names where Rust module layout permits; otherwise record an exact old/new map. Do not widen production visibility solely for tests. Full protocol/driver/security gates and independent audit close the split. | STRUCT-001AL | `CODE_SIZE.md`; intermediate test owner |
