...
1. **STRUCT-001AS — isolate legacy extended-DML handlers.** Move extended INSERT/DELETE/UPDATE application and
   its permission/constraint/error mapping into one private compatibility owner. Preserve exact mutation order,
   row-count tags, and shared-catalog dirty publication without creating a host product execution API.
...
|---|---|---:|---|---|---|
| **STRUCT-001AS** | NOW | P0 | Analyze and extract the three cohesive legacy extended-DML handlers: INSERT, DELETE, and UPDATE permission checks, relation/column validation, constraint enforcement, mutation application, dirty-table publication, and exact row-count/error mapping. Preserve missing-relation/column precedence, not-null/unique/foreign-key behavior, atomicity at the existing session boundary, and completion tags. Expose only three proven parent-private callers; COPY, simple-query dispatch, catalog, and relational execution remain root dependencies, with no host product execution API. Run focused extended DML/COPY and lifecycle tests plus full protocol/driver/security gates and independent audit. | STRUCT-001AR | PRODUCT-001 containment; legacy DML boundary |
