...
1. **STRUCT-001BX — isolate table-constraint construction.** Move primary-key, unique, and check-constraint
   construction helpers into a private owner, preserving exact precedence, candidate validation, index/OID and
   dirty publication behavior, with only three narrow consumers exported to `ddl_execution`.
...
|---|---|---:|---|---|---|
| **STRUCT-001BX** | NOW | P0 | Analyze and move `add_primary_key_to_session`, `add_unique_constraint_to_session`, and `add_check_constraint_to_session` from the server root into a private `table_constraints` owner. Preserve exact relation/kind/existence and duplicate-name precedence, column/type checks, candidate index/check validation, checked OID allocation, primary/unique index construction, dirty flags, table publication, persistence placement, rollback/error behavior, and helper argument ownership. Export only the three helpers required by `ddl_execution`; keep foreign-key and other table mutations outside and add no product API. Run focused table/constraint/index/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BW, STRUCT-001AV analysis | PRODUCT-001 containment; coherent legacy constraint-construction boundary |
