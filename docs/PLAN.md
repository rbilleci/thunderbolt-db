...
1. **STRUCT-001W — isolate grouped aggregate ownership.** Move the production grouped-reduction launcher,
   benchmark-only event-timed launcher, and public grouped-row contract behind a focused private module while
   preserving the crate-root facade and every device/result semantic.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001W structural child active; root now 21,816 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `resident_sort.rs` (846), `resident_count.rs` (1,066), `resident_gather.rs` (258), `resident_filter.rs` (1,221 after STRUCT-001T safety hardening), `resident_aggregate.rs` (866 after STRUCT-001V safety hardening), `point_read_submission.rs` (442), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), `point_read_text.rs` (1,205 after STRUCT-001M/N correctness hardening), and `point_read_rows.rs` (475). STRUCT-001U moved all six filtered fixed-width aggregate launcher families byte-identically behind `resident_aggregate`: stable public methods, PTX ABI, index upload lifetime, pooled synchronization, signed i128 partials/carry, bounded readback, and overflow behavior are unchanged. STRUCT-001V now checks the non-empty maximum host index at the correct 4/8/16-byte width with overflow-safe arithmetic before any CUDA mutation across all nine safe aggregate APIs; engine SQL empty aggregates still return typed NULL upstream. Two pure boundary tests and one real-GPU reject-then-reuse regression raise the inventory to 109 tests, 36 active/73 ignored. Six safety/aggregate/NULL/overflow gates passed 18 sequential and 24 concurrent invocations with no CUDA 700/716/717; independent audit found no issue. The canonical card kept count at 0.93x/1.00x roofline, gather at 349.5/155.3 GB/s, grouped aggregation at 1,678.0 M elements/s, and 65,536-batch point reads at 249.4M/255.4M lookups/s with p50 136/127us in-L2/out-of-L2. **Next:** isolate grouped aggregation ownership without changing its device or bounded-readback path. |
...
|---|---|---:|---|---|---|
| **STRUCT-001W** | NOW | P0 | Extract `GroupByI32Row`, the production grouped COUNT/SUM/MIN/MAX launcher, and the benchmark-only event-timed launcher into one private module below 1,500 lines. Preserve the public crate-root row contract via an explicit re-export; root methods; all int4/int8/i128/UUID key/value modes; NULL-key and NULL-value handling; aggregate masks; one/two-pass kernels; PTX symbols/arguments/grids; index and validity lifetimes; numeric overflow; dense slot readbacks; host result assembly; and benchmark event scope. Prove normalized move equivalence, downstream facade compatibility, exact inventory, representative grouped type/NULL/overflow/concurrency HAZARD gates, independent audit, and the canonical card. | STRUCT-001V; grouped-family ownership packet | `CODE_SIZE.md`; GPU-native aggregate charter |
