...
1. **STRUCT-001BW — complete bounded table-mutation ownership.** Move foreign-key addition and column/constraint
   drop/rename mutation helpers into `ddl_execution`, preserving all candidate validation/rollback, metadata and
   comment retargeting, exact errors, and keeping the owner below 1,500 lines.
...
|---|---|---:|---|---|---|
| **STRUCT-001BW** | NOW | P0 | Analyze and move `add_foreign_key_to_session`, `drop_column_from_session`, `rename_column_in_session`, and `rename_constraint_in_session` from the server root into the existing private `ddl_execution` owner. Preserve exact kind/existence/type/column/dependency precedence, candidate-table and catalog-index validation/rollback, checked attnum behavior, implicit-sequence/ACL/index/constraint/comment removal or retargeting, dirty flags, persistence, and errors. Keep all helpers module-private, keep `ddl_execution` below 1,500 lines, remove obsolete root imports, and make no semantic rewrite. Run focused table/column/constraint/FK/comment/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BV, STRUCT-001BF/BN, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy DDL owner extension |
