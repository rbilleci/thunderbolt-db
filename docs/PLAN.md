...
1. **STRUCT-001CE — complete replication-catalog read ownership.** Route the eleven publication/subscription
   catalog query shapes through `replication_catalog` and move their private query/parser/column/row helpers,
   preserving exact catalog precedence and output behind one bounded tri-state delegate.
...
|---|---|---:|---|---|---|
| **STRUCT-001CE** | NOW | P0 | Analyze and move all eleven publication/subscription catalog query shapes from `execute_statement` into the existing private `replication_catalog` owner behind one tri-state delegate: psql publication normal/verbose and subscription listings; direct publication/publication-rel/subscription reads; schema-publication and relation-publication describe reads; and pg-dump publication/publication-rel/publication-namespace reads. Move the exclusively owned psql query literals, relation-publication OID parser, three publication column builders, and eleven row builders with them. Preserve exact dispatcher precedence, predicates, columns, ordering, OID construction, table liveness filtering, all-table namespace rows, subscription fields, and output. Keep helpers private, retain only the command executor plus new query delegate as parent-visible seams, remove obsolete root definitions/imports, add no product API/cycle, and make no catalog behavior rewrite. Run focused publication/subscription/describe/pg-dump tests and real-client scenarios, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001CD, STRUCT-001CB, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy replication-catalog owner extension |
