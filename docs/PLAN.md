...
1. **STRUCT-001M — make TEXT-only point-read framing total.** Handle the kernel-supported zero-int4-projection
   shape without `chunks_exact(0)`, validate every returned result vector exactly, and prove a TEXT-only retained
   route executes on the GPU with byte-identical ordering and NULL-safe behavior.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001M behavior child active; root now 26,238 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `point_read_submission.rs` (441), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), and `point_read_text.rs` (901). The latest structural slice owns the equal-any int4-filter/text-projection kernel and bounded row assembly while preserving PTX, compaction, bounds, pooled buffers/streams, async pinned and blocking readback, event timing, row ordering, UTF-8 validation, and root APIs. All-target/downstream checks, exact 95-test inventory, 25 non-ignored tests, text multi-warp/NULL/pinned-pool HAZARD gates 3× sequential + 2× concurrent with zero CUDA safety errors, and independent audit passed. The canonical report card remained green: in-L2/out-of-L2 `sum_i32` 1,485/1,443 GB/s, `count_i32_compare` 0.88x/1.01x roofline, and 65,536-batch point reads 248.2M/252.7M lookups/s at p50 138/132us. **Audit-defined immediate child:** the kernel accepts zero int4 projections for TEXT-only queries, but the inherited row assembler panics at `chunks_exact(0)`; STRUCT-001M owns the separate behavior fix and non-vacuous GPU route parity. |
...
|---|---|---:|---|---|---|
| **STRUCT-001M** | NOW | P0 | Replace zero-width `values.chunks_exact(projection_count)` framing with match-count-indexed assembly that supports `projection_count == 0` and validates exact lengths for values, needle indices, row indices, text starts, and text lengths before decoding. Add a non-vacuous GPU-fired TEXT-only `SELECT label ... WHERE int4 = ?` retained-route differential covering present/absent needles, ordering, empty strings, and the nullable boundary; malformed result shapes fail with typed errors rather than truncate or panic. Pass all-target/downstream checks, updated exact inventory, 3× sequential + 2× concurrent TEXT-only/NULL/pool HAZARD gates, independent audit, and the canonical report card. | STRUCT-001L audit finding | GPU-native OLTP charter; AGENTS read-path benchmark standard |
