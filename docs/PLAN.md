...
1. **STRUCT-001AY — move SQL prepared routing into its existing owner.** Move duplicate detection, pg_dump
   compatibility PREPARE/EXECUTE cases, SQL PREPARE/EXECUTE validation/result handling, and DEALLOCATE state
   mutation into `sql_prepare` behind one private tri-state delegate, preserving exact precedence and errors.
...
|---|---|---:|---|---|---|
| **STRUCT-001AY** | NOW | P0 | Move the contiguous simple-query SQL prepared family from `execute_statement` into the existing `sql_prepare` owner: duplicate-name precheck; pg_dump domain/function PREPARE cases; relational SQL PREPARE validation/type resolution/install; pg_dump domain and AddTen/function/SQL EXECUTE result paths; and DEALLOCATE ALL/named mutation. Preserve exact precedence, canonical-query use, placeholder/type validation, row-description flag, prepared-map mutations, error codes/messages, and completion tags. Move `SqlDeallocateTarget` into its owner and add one parent-private tri-state delegate; do not move later SET/LOCK/DDL/catalog behavior. Run focused cursor/prepare and extended bind/lifecycle tests, pg_dump/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001AX, STRUCT-001AV analysis | PRODUCT-001 containment; existing SQL prepare owner |
