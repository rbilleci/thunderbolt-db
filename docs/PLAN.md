...
1. **STRUCT-001F — extract the execution crate's CUDA driver/runtime facade.** Move runtime probing, synthetic
   snapshots, primitive dispatch, resident-allocation construction, and raw build helpers above `cuda_context`
   while preserving the stable crate-root API and GPU allocation semantics.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001F active; root now 31,334 lines.** Handwritten production facade with distinct routing policy, CUDA runtime contract, context/allocation, driver facade, resident-memory API, MVCC batch encoding, write locate/apply, scan/aggregate/join/sort/expression kernel, host-reference operator, and test domains. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests. History shows operator-family growth rather than one cohesive unit; stable crate-root re-exports are the facade. **Completed:** `routing.rs` (241 lines), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams with identical 94-test inventory, and `cuda_context.rs` (948) as the dependency-bottom owner of primary context, module/stream caches, pooled device/pinned-host buffers, allocation budgets, and RAII guards. Context extraction passed all-target checks, 25 non-ignored tests, NULL differential, 3× sequential + 2× concurrent real-GPU context/pool/count HAZARD probes, non-skipping pinned-pool proof, zero CUDA 700/716/717, and independent adversarial audit; excess visibility findings were adopted. **Next:** extract `CudaDriverRuntime`, probe/snapshot logic, primitive dispatch, resident allocation construction, and raw allocation builders into a sub-1,500-line driver module above `cuda_context`; preserve allocation proof fields, async copy fencing, and root API. |
...
|---|---|---:|---|---|---|
| **STRUCT-001F** | NOW | P0 | Extract `CudaDriverRuntime`, runtime probing/synthetic snapshots, primitive dispatch, resident allocation construction, and raw HtoD/DtoD/recompaction builders into `cuda_driver.rs` above `cuda_context.rs`. Keep the public type at the crate root; preserve allocation/proof bytes, async-copy fallbacks/fences, driver errors, and `GpuRuntime` routing. Pass all-target/downstream checks, exact test inventory, NULL differential, real-GPU allocation/context HAZARD gates, and independent audit. | STRUCT-001 execution-facade packet; STRUCT-001E context boundary | `docs/CODE_SIZE.md`; ARCHITECTURE GPU/control-plane boundary |
