...
1. **STRUCT-001G — extract resident-memory ownership and lifecycle.** Move resident allocation/read-view types,
   read-source contract, metadata/context access, bounded host copies, append, telemetry, header count, and Drop
   into a storage-owned module while leaving relational operator families for later independent extraction.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001G active; root now 30,466 lines.** Handwritten production facade with distinct routing policy, CUDA runtime contract, context/allocation, driver facade, resident-memory ownership, MVCC batch encoding, write locate/apply, scan/aggregate/join/sort/expression kernel, host-reference operator, and test domains. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests. History shows operator-family growth rather than one cohesive unit; stable crate-root re-exports are the facade. **Completed:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams with identical 94-test inventory, `cuda_context.rs` (948), and `cuda_driver.rs` (883). The driver module owns runtime probing/snapshots, primitive dispatch, `GpuRuntime`, resident allocation construction, async-copy fallback, raw HtoD/DtoD builders, and recompaction validation above the context layer. It passed all-target/downstream checks, exact test inventory, 25 non-ignored tests, NULL differential, 3× sequential + 2× concurrent real-GPU driver/smoke/allocation HAZARD probes with zero CUDA safety errors, and independent audit proving proof/fence/guard equivalence. **Next:** extract resident-memory types and lifecycle/copy ownership without moving relational operator wrappers; preserve root types, allocation/drop ownership, bounded copy checks, and read-source semantics. |
...
|---|---|---:|---|---|---|
| **STRUCT-001G** | NOW | P0 | Extract `CudaResidentDeviceMemory`, its read view/chunk/recompaction contracts, `CudaResidentReadSource`, Debug/Drop, metadata/context/read-view/telemetry methods, bounded DtoH copies, append, and header-count entrypoint into `resident_memory.rs`. Keep operator-family methods as separate root impls for later slices. Preserve public root paths, exact allocation ownership/drop, bounds/overflow errors, stream synchronization, and driver construction; pass all-target/downstream checks, inventory, real-GPU copy/append/NULL HAZARD gates, and independent audit. | STRUCT-001 execution-facade packet; STRUCT-001E/F layers | `docs/CODE_SIZE.md`; ARCHITECTURE residency and stable-interface contracts |
