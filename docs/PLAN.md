...
1. **STRUCT-001AR — isolate cursor lifecycle ownership.** Move DECLARE/FETCH/MOVE/CLOSE parsing, unsupported-
   form detection, and cursor execution into one private owner. Preserve transaction-local lifetime, forward-only
   positions, exact tags/errors, and the existing prepared SELECT/result bridge.
...
|---|---|---:|---|---|---|
| **STRUCT-001AR** | NOW | P0 | Analyze and extract the cohesive cursor cluster: DECLARE/FETCH/MOVE/CLOSE parsers, supported-name and forward-direction rules, unsupported-form detection, declaration execution, result ownership, position advance, row emission, and completion tags. Preserve duplicate/missing cursor errors, session/transaction-local lifetime, prepared SQL execution, parameterized extended declarations, forward-only counts/ALL semantics, and exact tags/errors. Expose only proven parent-private callers; DML, COPY, catalog, and relational dispatch remain root dependencies, with no host product execution API. Run focused cursor/prepare and extended lifecycle tests plus full protocol/driver/security gates and independent audit. | STRUCT-001AQ | PRODUCT-001 containment; cursor compatibility boundary |
