...
1. **STRUCT-001H — extract deferred point-read submission ownership.** Move batched projection result contracts,
   pooled buffer/stream guards, completion/readback assembly, and drop-without-complete draining into a focused
   module while preserving root APIs and the single bounded final readback.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001H active; root now 30,076 lines.** Handwritten production facade with distinct routing policy, CUDA runtime contract, context/allocation, driver facade, resident-memory ownership, MVCC batch encoding, point-read submissions, write locate/apply, scan/aggregate/join/sort/expression kernel, host-reference operator, and test domains. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests. History shows operator-family growth rather than one cohesive unit; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams with identical 94-test inventory, `cuda_context.rs` (948), `cuda_driver.rs` (883), and `resident_memory.rs` (405). Resident memory now owns allocation/read-view/chunk contracts, `CudaResidentReadSource`, metadata/context/read-view/telemetry methods, bounded DtoH copies, append, header-count entrypoint, Debug, and context-bound Drop; relational operator wrappers remain in the root. It passed workspace/all-target/downstream checks, exact inventory, 25 non-ignored tests, 3× sequential + 2× concurrent append/generation/submit-complete/NULL HAZARD probes with zero CUDA safety errors, and independent audit proving field-drop, bounds, Arc lifetime, and trait equivalence. **Next:** extract batched i32 projection columns/rows and `CudaI32EqualAnyProjectSubmission` completion/drop ownership into a focused module; preserve pool lease order, final readback, detach semantics, and root paths. |
...
|---|---|---:|---|---|---|
| **STRUCT-001H** | NOW | P0 | Extract `CudaI32BatchProjectionRow`, `CudaI32BatchProjectionColumns`, `CudaI32EqualAnyProjectSubmission`, text projection row contract, completion/readback assembly, owned pooled guards, and drop-without-complete drain into `point_read_submission.rs`. Keep kernel submitters separate for the next operator slice. Preserve public root paths, field/drop order, detached completion, buffer/stream reuse fences, one bounded readback, and error behavior; pass all-target/downstream checks, exact inventory, NULL + drop/reuse HAZARD gates, and independent audit. | STRUCT-001 execution-facade packet; STRUCT-001E/G ownership boundaries | `docs/CODE_SIZE.md`; ARCHITECTURE execution-result contract |
