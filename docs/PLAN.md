...
1. **STRUCT-001CG — complete sequence-list catalog ownership.** Move the normal and verbose psql sequence-list
   shapes and their private query/row helpers into `sequence_execution` behind one bounded tri-state delegate,
   preserving exact ordering, comments, and dispatcher precedence.
2. **STRUCT-001AV — map and sequence the legacy statement dispatcher.** Continue promoting concrete existing-
   owner extensions and bounded family extractions before editing each branch family; never create line shards.
...
|---|---|---:|---|---|---|
| **STRUCT-001CG** | NOW | P0 | Analyze and move the normal and verbose psql sequence listings from `execute_statement` into the existing private `sequence_execution` owner behind one tri-state delegate. Move the two exclusively owned psql query literals and their normal/verbose row builders with them. Preserve exact dispatcher position, query equality, columns, name sorting, public schema/type/owner/persistence/size constants, comment lookup, and output. Keep helpers private, retain only the existing sequence execution seams plus the query delegate as production seams and explicit `cfg(test)` query aliases, remove obsolete root definitions/imports, add no product API/cycle, and make no sequence behavior rewrite. Run focused sequence/comment/catalog/shared-catalog tests and real-client scenarios, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001CF, STRUCT-001BY, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy sequence owner extension |
| **STRUCT-001AV** | NOW | P0 | Produce a current-tree analysis packet for the remaining monolithic `execute_statement`: exact line/function/branch-family inventory; session/catalog state read/write map; calls into syntax, DDL/DML/COPY, catalog/introspection, transaction/security, and result writers; tests/goldens/source guards per family; co-change evidence; dependency direction; and safe facade seams. Promote each discovered concrete extraction child above this parent before implementation, ordered to keep leaves below owners and avoid cycles, catch-all modules, numbered shards, or new host-product execution APIs. The first promoted child must have a bounded exactness and gate plan. | STRUCT-001AU | PRODUCT-001 containment; dispatcher decomposition evidence |
