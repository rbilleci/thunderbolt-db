...
1. **STRUCT-001BL — isolate publication/subscription lifecycle commands.** Move CREATE/DROP PUBLICATION and
   SUBSCRIPTION arms into a private owner, preserving helper ordering, catalog mutation/publication, exact
   errors/tags, and successful-parse routing.
...
|---|---|---:|---|---|---|
| **STRUCT-001BL** | NOW | P0 | Analyze and extract the complete successfully parsed replication-catalog lifecycle family from `execute_statement` into a private `replication_catalog` owner: CREATE/DROP PUBLICATION and CREATE/DROP SUBSCRIPTION. Preserve exact variant order, helper validation/mutation behavior, snapshot persistence placement, propagated errors, tags, and fallthrough behind one parent-private delegate. Keep role and table/DML commands in the root and add no replication capability or host product API. Run focused publication/subscription/comment/pg_dump/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BK, STRUCT-001AV analysis | PRODUCT-001 containment; legacy replication catalog boundary |
