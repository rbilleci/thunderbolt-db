...
1. **STRUCT-001CF — complete domain catalog-read ownership.** Move the three domain list/direct catalog shapes
   and their private query/row helpers into `domain_ddl` behind one bounded tri-state delegate, preserving exact
   psql/direct rows and dispatcher precedence.
...
|---|---|---:|---|---|---|
| **STRUCT-001CF** | NOW | P0 | Analyze and move the normal and verbose psql domain listings plus the direct `pg_type` domain query from `execute_statement` into the existing private `domain_ddl` owner behind one tri-state delegate. Move the two exclusively owned psql query literals and the normal/verbose plus direct domain row builders with them. Preserve exact dispatcher position, query equality, columns, B-tree name order, base-type display/OIDs, comment lookup, null placeholders, `typtype`, and output. Keep helpers private, retain only the DDL executor plus query delegate as production seams and explicit `cfg(test)` query aliases, remove obsolete root definitions/imports, add no product API/cycle, and make no domain behavior rewrite. Run focused domain/comment/catalog tests and real-client scenarios, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001CE, STRUCT-001BK, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy domain owner extension |
