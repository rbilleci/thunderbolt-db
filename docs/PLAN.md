...
1. **STRUCT-001V — harden filtered aggregate resident windows.** Reject empty inputs and overflow/out-of-bounds
   fixed-width index windows before any CUDA context, pool, or device mutation across all nine safe filtered
   aggregate APIs. Keep reduction and SQL empty-aggregate semantics unchanged.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001V safety child active; root now 21,816 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `resident_sort.rs` (846), `resident_count.rs` (1,066), `resident_gather.rs` (258), `resident_filter.rs` (1,221 after STRUCT-001T safety hardening), `resident_aggregate.rs` (803), `point_read_submission.rs` (442), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), `point_read_text.rs` (1,205 after STRUCT-001M/N correctness hardening), and `point_read_rows.rs` (475). STRUCT-001U moved all six filtered fixed-width aggregate launcher families byte-identically behind `resident_aggregate`: stable public methods, PTX ABI, index upload lifetime, pooled synchronization, signed i128 partials/carry, bounded readback, and overflow behavior are unchanged. The 106-test inventory remains 34 active/72 ignored; five representative aggregate gates passed 15 sequential and 20 concurrent invocations with no CUDA 700/716/717. Independent audit found no extraction defect and promoted the pre-existing unchecked resident-window boundary to STRUCT-001V. The canonical card kept count at 0.88x/1.01x roofline, gather at 349.5/155.3 GB/s, grouped aggregation at 1,678.3 M elements/s, and 65,536-batch point reads at 246.2M/232.4M lookups/s with p50 140/141us in-L2/out-of-L2. **Next:** make the nine safe filtered aggregate APIs reject invalid windows before CUDA mutation. |
...
|---|---|---:|---|---|---|
| **STRUCT-001V** | NOW | P0 | Harden all nine safe filtered aggregate `*_at_indices_from_payload` APIs. For non-empty host indices, compute the maximum index once, use checked arithmetic for `(max_index + 1) * width + byte_offset` at 4/8/16-byte widths, and reject any overflow or extent beyond `allocated_bytes` before `set_current`, buffer leases, or device access. Reject empty low-level SUM inputs consistently; the engine continues mapping empty SQL aggregates to NULL before this layer. Add pure overflow/boundary tests plus real-GPU malformed-window regressions proving the context remains usable, rerun aggregate/NULL/overflow/concurrency HAZARD gates, obtain independent audit, and run the canonical card. Do not add a CPU relational fallback or scan device values on the host. | STRUCT-001U independent audit HIGH | Safe CUDA API boundary; GPU-native aggregate charter |
