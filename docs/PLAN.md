...
1. **STRUCT-001I — extract point-read submit kernels.** Move equal-project, equal-any scan submission, and unique
   index-probe submission kernels into the point-read layer, preserving PTX bytes/symbols, launch/fence behavior,
   root method APIs, and performance report-card ratios.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001I active; root now 29,647 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams with identical 94-test inventory, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), and `point_read_submission.rs` (441). Point-read submission now owns columnar/per-row result contracts, owned pooled guards, detached completion, async pinned readback, error-path drains, and drop-without-complete fencing; submit kernels remain in the root. It passed all-target/downstream checks, exact inventory, 25 non-ignored tests, 3× sequential + 2× concurrent submit/drop/pool/NULL HAZARD probes with zero CUDA safety errors, and independent audit; the audit's dense-struct documentation placement finding was adopted. **Next:** extract equal-project, equal-any scan, and unique-index point-read submit kernels/PTX into a sub-1,500-line module; preserve symbols, launch dimensions, guard construction, fences, root wrappers, and report-card ratios. |
...
|---|---|---:|---|---|---|
| **STRUCT-001I** | NOW | P0 | Extract `launch_cuda_resident_i32_equal_project`, `submit_cuda_resident_i32_equal_any_project`, and `submit_cuda_resident_i32_index_probe` with their PTX/launch setup into `point_read_submit.rs`. Preserve private generic read-source support, public resident/read-view wrappers, PTX bytes and entry names, allocation/stream guard construction, async enqueue and index pinning. Pass all-target/downstream checks, exact inventory, NULL + scan/index/drop HAZARD gates, independent audit, and the canonical report card with no material ratio regression in either cache regime/layer. | STRUCT-001 execution-facade packet; STRUCT-001H result contract | `docs/CODE_SIZE.md`; AGENTS read-path benchmark standard |
