...
1. **STRUCT-001AZ — isolate legacy DDL mutation routing.** Move the contiguous TRUNCATE, DROP TABLE, and ALTER
   TABLE DROP CONSTRAINT execution families into a private `ddl_execution` owner behind one tri-state delegate,
   preserving catalog/session mutation, rollback, dirty publication, exact errors, and tags.
...
|---|---|---:|---|---|---|
| **STRUCT-001AZ** | NOW | P0 | Analyze and extract the contiguous legacy DDL mutation family from `execute_statement` into a private `ddl_execution` owner: TRUNCATE (including restart-identity sequence state), DROP TABLE dependency/catalog cleanup, and ALTER TABLE DROP CONSTRAINT/index side effects. Preserve branch order after SET/RESET/LOCK, relation-kind and existence precedence, permission/dependency checks, candidate validation/rollback, shared-catalog dirty publication, exact errors/tags, and fallthrough. Reuse `ddl_syntax` inputs through a one-way dependency and expose one parent-private tri-state delegate; do not move later catalog-query or parsed-command behavior. Run focused COPY/DDL/shared-catalog tests, pg_dump/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001AY, STRUCT-001AV analysis | PRODUCT-001 containment; legacy DDL execution boundary |
