...
1. **STRUCT-001BE — isolate cluster-object DDL commands.** Move database and tablespace create/drop/rename arms
   into a private owner, preserving existence/duplicate/dependency precedence, OID and ACL/comment state,
   rename retargeting, dirty publication, exact errors/tags, and successful-parse routing.
...
|---|---|---:|---|---|---|
| **STRUCT-001BE** | NOW | P0 | Analyze and extract the successfully parsed cluster-object DDL arms from `execute_statement` into a private `cluster_ddl` owner: CREATE/DROP/ALTER DATABASE RENAME and CREATE/DROP/ALTER TABLESPACE RENAME. Preserve exact duplicate-name and duplicate-target ordering, bootstrap/current database protections, role/dependency and IF EXISTS behavior, checked OID allocation, ACL/comment removal or retargeting, dirty old/new keys, snapshot persistence, errors/tags, and fallthrough behind one parent-private delegate. Keep general schema/relation DDL in their existing owners/root and add no cluster capability or host product API. Run focused database/tablespace/role/pg_dumpall/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BD, STRUCT-001AV analysis | PRODUCT-001 containment; legacy cluster catalog boundary |
