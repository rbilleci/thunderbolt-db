...
| **R3-002** | NEXT | P0 | Extend the GPU-native write/read fast path beyond int4-PK: numeric/UUID, bool, wider fixed-width types, then variable-width text and compound keys. Each graduated shape has GPU-fired differentials, recovery parity, and mixed read/write coverage. | R3-001 | Type-coverage evidence in archived handovers; `docs/design/non-int4-index-design-inputs.md` |
...
| **R3-004** | BLOCKED | P0 | Delete the host write/commit/MVCC tuple-store relational data path. Recovery reconstructs device-native state without acknowledged-commit loss; production and tests contain no host relational execution. | R3-002, R3-003, DUR-002 | ADR-006/007 |
...
| **PRODUCT-002** | NEXT | P1 | Close PostgreSQL type/protocol/catalog breadth: text+binary codecs, OID/typmod, typed NULL parameters, catalog/function execution, large NUMERIC, and checked aggregate overflow. Every type graduates to a GPU route. | R3-002 for write-capable types | ARCHITECTURE §14 |
