...
1. **STRUCT-001BD — isolate bootstrap catalog DDL commands.** Move the bounded plpgsql extension and public-
   schema create/drop command arms into a private owner, preserving existence/IF EXISTS semantics, dependency
   and ACL/comment cleanup, dirty publication, exact errors/tags, and successful-parse routing.
...
|---|---|---:|---|---|---|
| **STRUCT-001BD** | NOW | P0 | Analyze and extract the successfully parsed bootstrap-catalog DDL arms from `execute_statement` into a private `bootstrap_ddl` owner: CREATE/DROP EXTENSION for the bounded bootstrap plpgsql object and CREATE/DROP SCHEMA for the bounded public namespace. Preserve exact name/schema restrictions, IF EXISTS/IF NOT EXISTS precedence, dependency checks, schema ACL/default ACL/comment cleanup, dirty flags, snapshot persistence, errors/tags, and fallthrough behind one parent-private tri-state delegate. Keep all general relation/database/tablespace DDL in the root; add no new catalog capability or host product API. Run focused extension/schema/pg_dump/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BC, STRUCT-001AV analysis | PRODUCT-001 containment; legacy bootstrap catalog boundary |
