...
1. **STRUCT-001N — preserve nullable text identity on the GPU point-read path.** Carry projected text validity
   through the device result so SQL NULL never aliases an empty string, without a CPU relational fallback.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001N behavior child active; root now 26,242 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `point_read_submission.rs` (441), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), and `point_read_text.rs` (1,066 after STRUCT-001M correctness hardening). STRUCT-001M made zero-int4-projection framing total, validates all result vector lengths against the device-reported match count, and carries the nullable int4 filter validity bitmap into PTX before raw-value comparison. The GPU retained differential covers present/absent, ordered, empty-string, real-zero, and NULL-key sabotage without CPU fallback. All-target/downstream checks, exact 99-test inventory (four intentional framing tests), 29 non-ignored tests, TEXT-only/filter-NULL/pinned-pool HAZARD gates 3× sequential + 2× concurrent with zero CUDA safety errors, and independent audit passed after both findings and its non-vacuity finding were adopted. The final canonical card remained green: in-L2/out-of-L2 `count_i32_compare` 0.96x/1.01x roofline and 65,536-batch point reads 247.3M/252.2M lookups/s at p50 139/132us. The new GPU regression lives in a 74-line focused test module rather than expanding the existing oversized `resident_route.rs`. **Next:** preserve projected nullable text validity device-to-result so SQL NULL never aliases `''`. |
...
|---|---|---:|---|---|---|
| **STRUCT-001N** | NOW | P0 | Extend compact int4-filter/text projection so a projected nullable text column carries device-read validity alongside text bytes; `NULL` and `''` remain distinct in execution results and pgwire-facing `SqlValue`s. Preserve bounded device compaction, captured snapshot semantics, stable ordering, and zero host relational fallback. Update the execution result contract and every engine/example consumer intentionally; add GPU-fired NULL/empty/non-empty differentials, exact inventory, HAZARD, independent audit, and the canonical report card. | STRUCT-001M; pre-existing nullable-text boundary found while implementing it | GPU-native OLTP charter; STRATA null bitmap contract |
