...
1. **STRUCT-001BS — complete bounded-function ownership.** Move the remaining bounded SQL-function result and
   body parser/evaluator helpers into the existing private `function_execution` owner, preserving permission,
   supported-body, type/value, error, and result semantics with no new host execution surface.
...
|---|---|---:|---|---|---|
| **STRUCT-001BS** | NOW | P0 | Analyze and move the remaining bounded SQL-function execution internals from the server root into the existing private `function_execution` owner: function lookup/schema and function ACL checks, bounded SQL body parsing, keyword/quote scanning, supported text literal parsing, return-type value construction, and exact unsupported-body errors. Preserve command behavior and result rows through internal calls, keep syntax/function DDL outside, remove obsolete root imports/exports, and add no generalized host execution API. Run focused function/ACL/error/recovery tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BR, STRUCT-001BI, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy function owner extension |
