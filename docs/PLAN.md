...
1. **STRUCT-001CB — complete replication-catalog ownership.** Move publication target validation and publication/
   subscription create/drop mutations into the existing private owner, preserving exact duplicate/dependency/
   error/dirty behavior with all helpers internal to the command module.
...
|---|---|---:|---|---|---|
| **STRUCT-001CB** | NOW | P0 | Analyze and move `publication_table_target_error`, `create_publication`, `drop_publication`, `create_subscription`, and `drop_subscription` from the server root into the existing private `replication_catalog` owner. Preserve exact target kind/existence, duplicate publication tables/names, publication/subscription dependency checks, duplicate-list/IF EXISTS behavior, constructed fields, comment cleanup, dirty keys, helper arguments, errors, and success-only command persistence/tags. Keep all five helpers module-private, remove obsolete root imports, add no product API/cycle, and make no replication behavior rewrite. Run focused publication/subscription/comment/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001CA, STRUCT-001BL, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy replication-catalog owner extension |
