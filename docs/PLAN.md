...
1. **STRUCT-001BM — isolate role lifecycle commands.** Move CREATE/DROP/RENAME ROLE arms into a private owner,
   preserving reserved/existence/dependency checks, ACL/current-role retargeting, comments/dirty publication,
   exact errors/tags, and successful-parse routing.
...
|---|---|---:|---|---|---|
| **STRUCT-001BM** | NOW | P0 | Analyze and extract the complete successfully parsed role lifecycle family from `execute_statement` into a private `role_ddl` owner: CREATE ROLE, DROP ROLE, and ALTER ROLE RENAME. Preserve exact reserved/bootstrap/current-role and existence/duplicate/dependency precedence, OID allocation, role membership and every database/tablespace/schema/default-table/relation/function ACL cleanup or retarget, comment cleanup/retarget, dirty old/new keys, current-role rename, snapshot persistence, errors/tags, and fallthrough behind one parent-private delegate. Keep table/drop/alter/DML commands in the root and add no role capability or host product API. Run focused role/ACL/comment/pg_dumpall/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BL, STRUCT-001AV analysis | PRODUCT-001 containment; legacy role catalog boundary |
