...
1. **STRUCT-001BO — isolate catalog comment execution.** Move the complete parsed `COMMENT ON` family into a
   private catalog-comment owner, preserving target kind/existence precedence, shared-catalog checks, column
   attnums, constraint lookup, comment insert/removal, dirty publication, persistence, and exact errors/tags.
...
|---|---|---:|---|---|---|
| **STRUCT-001BO** | NOW | P0 | Analyze and move the complete successfully parsed `COMMENT ON` family from `execute_statement` into a private `catalog_comments` owner. Preserve exact per-target kind/existence/error precedence for database, role, schema, tablespace, table, column/attnum, index, view/materialized view, function, extension, sequence, domain, publication, subscription, and constraint targets; preserve shared-catalog visibility checks, comment insert/removal, dirty publication, snapshot persistence, command tag, and parse-error fallthrough behind one parent-private delegate. Keep ACL and DML commands in the root. Run focused comment/shared-catalog/catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BN, STRUCT-001AV analysis | PRODUCT-001 containment; coherent legacy catalog-metadata owner |
