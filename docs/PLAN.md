...
1. **STRUCT-001BV — isolate relational integrity validation.** Move shared unique-index, check-constraint, and
   foreign-key validation plus exact violation constructors into a private owner, preserving validation order,
   predicate/type behavior, errors, and all DDL/DML/COPY consumers through three narrow exports.
...
|---|---|---:|---|---|---|
| **STRUCT-001BV** | NOW | P0 | Analyze and move unique-index, check-constraint, and foreign-key validation and their exact violation constructors from the server root into a private `integrity_validation` owner. Preserve row/key comparison, constraint predicate/type/error behavior, referenced-table/column mapping and null treatment, validation order, exact codes/messages, and all root, table-DDL, index-DDL, simple-DML, COPY, and test consumers through only the three required `pub(super)` validators. Keep catalog index-definition formatting and mutation logic outside; remove obsolete root definitions/imports and add no product execution API. Run focused DML/COPY/constraint/index/FK tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BU, STRUCT-001AV analysis | PRODUCT-001 containment; coherent legacy integrity boundary |
