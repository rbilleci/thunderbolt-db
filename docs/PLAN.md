...
1. **STRUCT-001E — extract the execution crate's shared CUDA context substrate.** Move primary-context ownership,
   module/stream caches, pooled device/pinned-host buffers, and allocation-budget leases into a dependency-bottom
   module before separating resident-memory and driver-runtime layers; run the real-GPU safety gates.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001E active; root now 32,251 lines.** Handwritten production facade with distinct routing policy, CUDA runtime contract, context/allocation, resident-memory API, MVCC batch encoding, write locate/apply, scan/aggregate/join/sort/expression kernel, host-reference operator, and test domains. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; CUDA implementation domains share primary-context, pool, lease, error, PTX-symbol, and launch boundaries. History shows operator-family growth rather than one cohesive unit. Stable crate-root re-exports are the facade. **Completed:** `routing.rs` (241 lines), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), and `runtime_contract.rs` own independent contracts. The former 7,177-line inline tests now retain identical full test names through `include!` seams in `tests/{write_index,sort_join,cuda_paths,context_aggregate}.rs`; `tests/mod.rs` and every seam are below 3,000 lines. Exact 94-test inventory diff, 25 non-ignored tests, downstream checks, unique-definition search, and diff whitespace pass. **Next:** extract the dependency-bottom primary-context/cache/pool/allocation-budget substrate; preserve unsafe ownership/drop order, CUDA symbols, and process-lifetime registry semantics, then run HAZARD gates. |
...
|---|---|---:|---|---|---|
| **STRUCT-001E** | NOW | P0 | Extract `GpuPrimaryContext`, driver-symbol ownership, module/stream caches, pooled device/pinned-host buffers, allocation tracking/scopes, leases, pending-copy transport, and the per-GPU registry into a sub-1,500-line `cuda_context.rs`. Expose only the parent-private surface required by resident/driver code; preserve drop/drain order and symbol lookup exactly. Pass compile/non-ignored tests, then real-GPU context/pool/count smoke 3× sequential and 2× concurrent with zero CUDA 700/716/717. | STRUCT-001 execution-facade packet | `docs/CODE_SIZE.md`; CHARTER GPU safety and lease-lifetime rules |
