...
1. **STRUCT-001J — isolate dense index point-read ownership.** Move the single- and multi-shard dense unique-index
   submission kernels, shard descriptor contract, completion, and drop fence into one point-read module while
   preserving the stable crate-root API, PTX bytes/symbols, device-resident routing, and report-card ratios.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001J active; root now 28,353 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams with identical 94-test inventory, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `point_read_submission.rs` (441), and `point_read_submit.rs` (1,308). The latest slice owns equal-project launch plus equal-any scan and unique-index atomic submission/PTX, while preserving the root wrappers, generic read source, allocation guards, stream/event ordering, fallbacks, and index lifetime. All-target/downstream checks, exact inventory, 25 non-ignored tests, NULL/scan/index/drop HAZARD probes 3× sequential + 2× concurrent with zero CUDA safety errors, and independent byte-level audit passed. The canonical report card remained green in both layers/cache regimes: in-L2/out-of-L2 `sum_i32` 1,471/1,453 GB/s, `count_i32_compare` 0.89x/1.00x roofline, and 65,536-batch point reads 246.1M/253.3M lookups/s at p50 138/132us. **Next:** isolate the single- and multi-shard dense unique-index descriptor, submit/PTX, completion, and drop-fence ownership in a sub-1,500-line point-read module without changing device routing or public APIs. |
...
|---|---|---:|---|---|---|
| **STRUCT-001J** | NOW | P0 | Extract `CudaI32IndexProbeDenseSubmission`, `MultiShardProbeShard`, the single- and multi-shard dense unique-index submit/PTX paths, completion, and drop fence into `point_read_dense.rs`. Preserve crate-root type/API paths, private generic read-source support, descriptor bytes, zone/MVCC device routing, PTX bytes and entry names, allocations, guards, stream/event ordering, status semantics, and telemetry. Pass all-target/downstream checks, exact inventory, dense single/multi-shard/NULL/drop HAZARD gates, independent audit, and the canonical report card with no material ratio regression in either cache regime/layer. | STRUCT-001 execution-facade packet; STRUCT-001H/I point-read contracts | `docs/CODE_SIZE.md`; AGENTS read-path benchmark standard |
