...
1. **STRUCT-001AK — bound unauthenticated frontend frame allocation.** Add explicit operational maxima for
   startup and tagged frames, reject over-limit declarations before allocation, and construct each accepted
   frame in one buffer. Preserve PostgreSQL framing and make EOF/minimum/exact-limit/over-limit behavior explicit.
...
|---|---|---:|---|---|---|
| **STRUCT-001AK** | NOW | P0 | Bound attacker-controlled pre-authentication allocation in both frontend readers. Define one reviewed maximum for startup packets and one for tagged messages; validate the declared PostgreSQL length (including its four-byte length field) before allocation; construct tag/length/payload or length/payload directly into one exact buffer rather than allocating and copying a second payload vector. Focused tests cover EOF, partial prefix/payload, below-minimum, ordinary reconstruction, exact maximum via allocation-free validation, and one-byte-over rejection. Full protocol/driver/security gates and independent security audit must pass. | STRUCT-001AJ | Inherited finding from STRUCT-001AJ audit |
