...
1. **STRUCT-001CC — complete catalog-comment lookup ownership.** Move the five shared-catalog target lookup
   helpers into `catalog_comments`, preserving global-lock and liveness/constraint predicates with all helpers
   module-private and no new API.
...
|---|---|---:|---|---|---|
| **STRUCT-001CC** | NOW | P0 | Analyze and move `shared_catalog_contains_table`, `shared_catalog_contains_view`, `shared_catalog_contains_sequence`, `shared_catalog_contains_live_index`, and `shared_catalog_contains_table_constraint` from the server root into the existing private `catalog_comments` owner. Preserve exact shared-catalog mutex access, table/view/sequence membership, live-index table predicate, primary/unique/check constraint detection, and all comment target kind/existence/error ordering. Keep all five helpers module-private, import only the shared catalog accessor, remove obsolete root definitions/imports, and add no product API or cycle. Run focused comment/shared-catalog/catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001CB, STRUCT-001BO, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy catalog-comment owner extension |
