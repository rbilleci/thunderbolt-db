...
1. **STRUCT-001AB — make device write-apply inputs total.** Replace arbitrary device addresses with typed,
   owned same-context regions and exact extents; validate index geometry and every fixed/text fold span before
   CUDA work; and drain every launched error before pooled resources can recycle.
2. **MULTI-002 — partition device-locate work by CUDA context.** This newly promoted GPU-native boundary is
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001AB write-apply safety child active; root now 19,020 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (953), `cuda_driver.rs` (883), `resident_memory.rs` (405), `resident_sort.rs` (846), `resident_count.rs` (1,066), `resident_gather.rs` (258), `resident_filter.rs` (1,221 after STRUCT-001T safety hardening), `resident_aggregate.rs` (866 after STRUCT-001V safety hardening), `resident_group.rs` (1,282), `group_input.rs` (639), `write_locate.rs` (865 after STRUCT-001Z safety hardening), `write_apply.rs` (873), `point_read_submission.rs` (442), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), `point_read_text.rs` (1,205 after STRUCT-001M/N correctness hardening), and `point_read_rows.rs` (475). STRUCT-001Z made the extracted write/visible-locate boundary total: typed same-context owners, exact index/version extents, on-device packed-slot bounds, reserved fail-closed count sentinels, and guaranteed launched-error draining. The exact execution inventory is 118 tests with 43 active and 75 GPU-ignored; five affected GPU gates passed 15 sequential and 10 concurrent invocations, and independent audit is clean. The remaining cross-context orchestration boundary is promoted as **MULTI-002** rather than hidden behind host probing. STRUCT-001AA isolated fused append publication, incremental int4 index insertion, and device compound-key folding with exact PTX and stable facade; its extraction audit promoted raw address/context/extent/geometry and error-drain gaps as **STRUCT-001AB**. **Next:** make that GPU write boundary total without deciding **R3-001** implicitly. |
...
|---|---|---:|---|---|---|
| **STRUCT-001AB** | NOW | P0 | Make all three safe device write-apply APIs total. Replace `FusedApplyRequest` raw destination/header/index addresses with owned typed regions plus exact byte offsets/extents; require every owner to share the submission primary context; validate staging/row arithmetic, writable destinations, index mask/shift/allocation/base-row capacity, and compound fixed-width/text offset/blob spans before descriptor allocation, context selection, or CUDA mutation. Bind compound folding to its exact resident owner rather than an arbitrary base pointer. Guarantee a best-effort default-stream drain after every launched error, including DtoH failure, before any pooled lease or owner drops. Add pure boundary/geometry tests and retained-GPU malformed/cross-owner/fail-closed/context-reuse coverage; rerun fused/index/fixed/wide/UUID/text/streaming HAZARD gates 3x sequential and 2x concurrent and obtain independent audit. Preserve PTX algorithms, staging ABI, same-key version-twin advancement, 256-probe decline, compound hash parity, stamp-before-publication order, and GPU-only key derivation; do not introduce host validation scans or select **R3-001**. | STRUCT-001AA independent audit HIGH | Safe CUDA write boundary; ADR-013 ownership invariant |
