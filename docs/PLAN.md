...
1. **STRUCT-001BP — isolate ACL mutation execution.** Move the complete parsed GRANT/REVOKE family into a
   private ACL owner, preserving helper arguments/order, mutation errors, dirty publication performed by the
   helpers, persistence placement, exact tags, and successful-parse routing.
...
|---|---|---:|---|---|---|
| **STRUCT-001BP** | NOW | P0 | Analyze and move the complete successfully parsed ACL mutation family from `execute_statement` into a private `acl_execution` owner: table/relation, schema, database, tablespace, function, and default-table-privilege GRANT/REVOKE commands. Preserve exact helper selection, arguments and ownership/order, propagated errors, helper-owned dirty publication, snapshot-persistence placement, command tags, and parse-error fallthrough behind one parent-private delegate. Keep relational DML in the root. Run focused ACL/permission/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BO, STRUCT-001AV analysis | PRODUCT-001 containment; coherent legacy ACL owner |
