...
1. **STRUCT-001K — isolate device Bloom point-read pruning.** Move the chunk-Bloom descriptor and probe kernel
   into a focused point-read module while preserving device hashing, mask/output layout, allocation lifetimes,
   stable crate-root APIs, NULL boundaries, and report-card ratios.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001K active; root now 27,280 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `point_read_submission.rs` (441), `point_read_submit.rs` (1,308), and `point_read_dense.rs` (1,086). The latest slice owns the dense unique-index result/lifecycle contract plus single- and multi-shard submit/PTX and descriptors. It preserves device-only hash probing, MVCC birth/death gates, row bounds, zone pruning, binary/linear shard routing, root APIs, guards, events, fallback/drain behavior, and cancellation fencing. All-target/downstream checks, the exact 95-test inventory (one intentional dense-drop safety regression), 25 non-ignored tests, five dense/MVCC/binary/NULL/drop HAZARD gates 3× sequential + 2× concurrent with zero CUDA safety errors, and independent audit passed. The canonical report card remained green: in-L2/out-of-L2 `sum_i32` 1,483/1,443 GB/s, `count_i32_compare` 0.86x/1.00x roofline, and 65,536-batch point reads 247.1M/252.3M lookups/s at p50 139/132us. **Next:** isolate chunk-Bloom device pruning in a focused sub-1,500-line point-read module; preserve hash/mask/output semantics, device guards, root wrappers, NULL boundary, and report-card ratios. |
...
|---|---|---:|---|---|---|
| **STRUCT-001K** | NOW | P0 | Extract `ChunkBloomProbeShard` and `probe_cuda_chunk_blooms` with its PTX/launch setup into `point_read_bloom.rs`. Preserve the crate-root descriptor API, two-hash bit addressing, per-needle/chunk output layout, input bounds, buffer sizes, resident Bloom guards, stream launch/synchronization, and host readback only of the bounded prune result. Pass all-target/downstream checks, exact inventory, Bloom false-negative + NULL HAZARD gates, independent audit, and the canonical report card with no material ratio regression in either cache regime/layer. | STRUCT-001 execution-facade packet; STRUCT-001I/J point-read modules | `docs/CODE_SIZE.md`; AGENTS read-path benchmark standard |
