...
1. **STRUCT-001BF — move parsed table-definition DDL into its existing owner.** Extend `ddl_execution` with
   CREATE TABLE, ADD/DROP/RENAME constraints, and RENAME TABLE parsed-command routing, preserving default/
   sequence preflight, rollback, indexes/comments/ACLs, dirty publication, exact errors/tags, and parse order.
...
|---|---|---:|---|---|---|
| **STRUCT-001BF** | NOW | P0 | Analyze and move the contiguous successfully parsed table-definition DDL family from `execute_statement` into the existing private `ddl_execution` owner: CREATE TABLE; ADD PRIMARY KEY/UNIQUE/CHECK/FOREIGN KEY; DROP/RENAME CONSTRAINT; and RENAME TABLE. Preserve exact branch order, schema permissions, type/domain/default and implicit-sequence preflight, checked OIDs, candidate validation/rollback, index/constraint/comment/ACL retargeting and cleanup, dirty publication, snapshot persistence, errors/tags, and fallthrough behind one parent-private delegate. Keep CREATE/RENAME INDEX and view/function/sequence/domain commands in the root. Run focused table/constraint/default/rename/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BE, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy DDL owner extension |
