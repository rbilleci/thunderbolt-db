...
1. **STRUCT-001S — isolate typed predicate-filter ownership.** Move int8/i128/text/UUID/bool resident predicate
   launchers and their nullable-mask compactor behind a focused private module while preserving root APIs,
   three-valued logic, device compaction, and typed comparison semantics.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001S structural child active; root now 23,648 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `resident_sort.rs` (846), `resident_count.rs` (1,066), `resident_gather.rs` (258), `point_read_submission.rs` (442), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), `point_read_text.rs` (1,205 after STRUCT-001M/N correctness hardening), and `point_read_rows.rs` (475). STRUCT-001R moved the shared fixed-width gather lifecycle and int4/bool/int8/i128 wrappers byte-for-byte behind the private gather module. Bounds-before-device-access, PTX entry/arguments/grid, row-index H2D, pooled stream/scratch, launch synchronization, one bounded result D2H, bitmap addressing, endian/output layout, and public APIs are unchanged. All-target/downstream checks, exact unchanged 101-test inventory, 31 non-ignored tests, and width/bool/pool HAZARD gates passed 3× sequential + 2× concurrent with zero CUDA safety errors. Independent audit's seam and stale public bool-doc findings were adopted. The canonical card remained green: gather kernel-only in-L2/out-of-L2 349.5/155.3 GB/s, `count_i32_compare` 0.85x/1.01x roofline, and 65,536-batch point reads 246.1M/251.1M lookups/s at p50 140/131us. **Next:** isolate GPU-native typed predicate filtering and nullable compaction. |
...
|---|---|---:|---|---|---|
| **STRUCT-001S** | NOW | P0 | Extract resident int8 scalar/column, i128 scalar/column, text equality/ordering/LIKE, UUID scalar/column, and bool-to-mask filter launchers plus `compact_mask_with_validity` into one private module below 1,500 lines. Preserve public methods, PTX paths/arguments/grids, scalar limb encoding, text/UUID ordering, bool bitmap expansion, per-column validity AND, device compaction, bounded index readback, and no CPU predicate fallback. Prove byte-level equivalence across non-contiguous late functions, exact inventory, representative type/NULL/concurrency HAZARD gates, independent audit, and the canonical report card. | STRUCT-001R; typed-filter ownership packet | `CODE_SIZE.md`; GPU-native predicate charter |
