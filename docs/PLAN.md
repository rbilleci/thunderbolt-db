...
1. **STRUCT-001BU — localize catalog rename mutations.** Move table, index, sequence, and function rename helpers
   into their sole existing command owners, preserving exact metadata/ACL/comment retargeting, dependency/error
   precedence, dirty publication, persistence, and direct-test access through test-only aliases.
...
|---|---|---:|---|---|---|
| **STRUCT-001BU** | NOW | P0 | Analyze and move the four sole-production-consumer catalog rename helpers from the server root into their existing private owners: `rename_table_in_session` to `ddl_execution`, `rename_index_in_session` to `index_ddl`, `rename_sequence_in_session` to `sequence_execution`, and `rename_function_in_session` to `function_execution`. Preserve exact kind/existence/dependency precedence, object/foreign-key/ACL/currval/comment retargeting, dirty state, persistence, errors, and no-op behavior. Keep each helper private to its owner except `pub(super)` visibility required by existing direct tests, exposed from the root only under `cfg(test)`; remove obsolete production aliases and add no sibling cycle or product API. Run focused rename/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BT, STRUCT-001BF/BG/BI/BJ, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy command-owner extensions |
