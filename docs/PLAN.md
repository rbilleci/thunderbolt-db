...
1. **STRUCT-001BB — isolate pg_dump catalog compatibility routing.** Move the contiguous subscription/role,
   pg_dump/pg_dumpall discovery, comment/ACL/dependency/security-label, and sequence restore/readback prelude into
   a private `pg_dump_compat` owner behind one tri-state delegate before parsed-command execution.
...
|---|---|---:|---|---|---|
| **STRUCT-001BB** | NOW | P0 | Analyze and extract the contiguous pg_dump/pg_dumpall catalog compatibility prelude from `execute_statement` into a private `pg_dump_compat` owner, from subscription/role discovery through extension/language/namespace and relation/type/database/sequence metadata, shared/local comments and ACLs, indexes/constraints/views/defaults/publications, empty catalog families, dependencies/security labels/memberships, and pg_dump sequence setval/last-value traffic. Preserve exact branch order before `parse_command`, canonical predicates, session reads and sequence mutation/dirty persistence, rows/columns/nulls/errors, and fallthrough behind one parent-private tri-state delegate. Keep metadata row builders in their current owner for this slice, use explicit dependencies, and add no catalog capability or host product API. Run focused pg_dump/pg_dumpall/psql tests, full protocol/driver/security gates, formatting/diff/dead-symbol checks, and independent audit. | STRUCT-001BA, STRUCT-001AV analysis | PRODUCT-001 containment; legacy dump compatibility boundary |
