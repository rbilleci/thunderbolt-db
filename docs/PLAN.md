...
1. **STRUCT-001U — isolate filtered aggregate ownership.** Move fixed-width SUM/MIN/MAX and i128 partial
   aggregate launchers behind a focused private module while preserving root APIs, device reduction semantics,
   overflow representation, and pooled lifecycle invariants.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001U structural child active; root now 22,608 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `resident_sort.rs` (846), `resident_count.rs` (1,066), `resident_gather.rs` (258), `resident_filter.rs` (1,221 after STRUCT-001T safety hardening), `point_read_submission.rs` (442), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), `point_read_text.rs` (1,205 after STRUCT-001M/N correctness hardening), and `point_read_rows.rs` (475). STRUCT-001T checks every int8/i128/UUID fixed window, bool/validity bitmap, text-offset array, and explicit text-byte extent before CUDA mutation. Equality/order/LIKE PTX rejects `start > end` or `end > bytes_len` before any byte read; invalid rows fail closed. Engine callers pass the resident descriptor's exact byte extent. Three pure overflow/OOB tests and two real-GPU safety regressions raise the inventory to 106 tests, 34 non-ignored. Nine safety/type/NULL HAZARD gates passed 3× sequential + 2× concurrent with zero CUDA safety errors. Independent audit's malformed-`<>` HIGH and indentation LOW findings were adopted. The canonical card remained green: i64/i128 filters 165.6/285.6 GB/s in-L2 and 362.4/591.9 GB/s out-of-L2, `count_i32_compare` 0.86x/1.00x roofline, and 65,536-batch point reads 247.8M/251.9M lookups/s at p50 138/132us. **Next:** isolate filtered fixed-width aggregate reduction ownership. |
...
|---|---|---:|---|---|---|
| **STRUCT-001U** | NOW | P0 | Extract filtered int4 SUM, int8 SUM-to-i128, int4/int8 MIN/MAX, and i128 SUM/MIN/MAX partial launchers into one private module below 1,500 lines. Preserve every public aggregate method, PTX symbols/arguments/grids, index H2D, pooled stream/scratch, signed low/high partial representation, device reduction, final bounded readback, and empty-input contract. Prove byte-level move equivalence, exact inventory, representative width/overflow/NULL/concurrency HAZARD gates, independent audit, and the canonical report card. | STRUCT-001T; aggregate-family ownership packet | `CODE_SIZE.md`; GPU-native aggregate charter |
