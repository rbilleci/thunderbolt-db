...
1. **STRUCT-001BJ — isolate sequence lifecycle and value functions.** Move CREATE/RENAME/DROP SEQUENCE plus
   nextval/currval/setval command arms into a private owner, preserving permissions/range/state, comments/dirty
   publication, exact rows/errors/tags, and successful-parse routing.
...
|---|---|---:|---|---|---|
| **STRUCT-001BJ** | NOW | P0 | Analyze and extract the complete successfully parsed sequence family from `execute_statement` into a private `sequence_execution` owner: CREATE SEQUENCE, nextval/currval/setval, ALTER SEQUENCE RENAME, and DROP SEQUENCE. Preserve exact schema permission and name/kind/existence precedence, checked OID and sequence option/range validation, last-value/is-called/currval state, overflow/error ordering, default/dependency drop guards, comment retarget/removal, dirty old/new keys, snapshot persistence, returned columns/rows, tags, and fallthrough behind one parent-private delegate. Keep domain commands in the root and add no sequence capability or host product API. Run focused sequence/default/comment/pg_dump/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BI, STRUCT-001AV analysis | PRODUCT-001 containment; legacy sequence boundary |
