...
1. **STRUCT-001AQ — isolate SQL PREPARE/EXECUTE compatibility ownership.** Move the SQL-level
   PREPARE/EXECUTE/DEALLOCATE parsers, argument decoder, describe bridge, and prepared-result executor into one
   private owner. Keep cursor parsing/execution, DML, catalog, and relational dispatch outside the module.
...
|---|---|---:|---|---|---|
| **STRUCT-001AQ** | NOW | P0 | Analyze and extract the cohesive SQL-level PREPARE/EXECUTE/DEALLOCATE cluster: statement parsers, type OID mapping, SQL literal/cast decoding, describe-time parameter binding, result-column/error bridge, and prepared-result execution. Preserve comment/quote/dollar-quote handling, argument normalization and NULL semantics, replacement/deallocation behavior, type validation and exact errors/tags. Expose only proven parent-private callers; cursor parsing/execution, DML, catalog, and relational dispatch remain root dependencies, with no host product execution API. Run focused cursor/prepare and extended bind/lifecycle tests plus full protocol/driver/security gates and independent audit. | STRUCT-001AP | PRODUCT-001 containment; SQL compatibility boundary |
