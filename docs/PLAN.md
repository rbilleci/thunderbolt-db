...
1. **STRUCT-001BN — move remaining parsed table DDL into its existing owner.** Extend `ddl_execution` with DROP
   TABLE and column default/add/rename/drop arms, preserving dependencies, default/type validation, rollback,
   ACL/comments/dirty publication, exact errors/tags, and successful-parse routing.
...
|---|---|---:|---|---|---|
| **STRUCT-001BN** | NOW | P0 | Analyze and move the remaining successfully parsed table-definition mutation family from `execute_statement` into the existing private `ddl_execution` owner: DROP TABLE, ALTER COLUMN SET/DROP DEFAULT, ADD COLUMN, RENAME COLUMN, and DROP COLUMN. Preserve exact name/kind/existence/permission/dependency precedence, default target and domain/type validation, checked attnum and implicit-sequence behavior, candidate-table validation/rollback, index/constraint/ACL/comment cleanup or retargeting, dirty publication, snapshot persistence, errors/tags, and fallthrough behind one parent-private delegate. Keep COMMENT and DML commands in the root. Run focused table/column/default/constraint/comment/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BM, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy DDL owner extension |
