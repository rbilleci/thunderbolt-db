...
1. **STRUCT-001L — isolate text point-read projection.** Move the equal-any int4-filter/text-projection kernel
   and its bounded row assembly into one point-read module while preserving device compaction, UTF-8/text bounds,
   async result staging, stable crate-root APIs, NULL semantics, and report-card ratios.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001L active; root now 27,128 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `point_read_submission.rs` (441), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), and `point_read_bloom.rs` (206). The latest slice owns chunk-Bloom device pruning while preserving two-hash/three-probe bit addressing, descriptor/output layout, bounds, pooled buffers, resident guards, synchronous launch/readback, and root API. All-target/downstream checks, exact 95-test inventory, 25 non-ignored tests, Bloom false-negative and NULL HAZARD gates 3× sequential + 2× concurrent with zero CUDA safety errors, and independent audit passed; its sole formatting finding was adopted. The canonical report card remained green: in-L2/out-of-L2 `sum_i32` 1,479/1,444 GB/s, `count_i32_compare` 0.89x/1.00x roofline, and 65,536-batch point reads 246.3M/253.5M lookups/s at p50 139/131us. **Next:** isolate the equal-any int4-filter/text-projection kernel and bounded text row assembly in a sub-1,500-line point-read module, leaving shared pinned-result transfer helpers available to all read families. |
...
|---|---|---:|---|---|---|
| **STRUCT-001L** | NOW | P0 | Extract `launch_cuda_resident_i32_equal_any_project_text` and `assemble_i32_text_batch_projection_rows` into `point_read_text.rs`; retain shared `stage_result_dtoh_async`/`copy_pinned_into` helpers for all read families. Preserve generic resident/read-view wrappers, PTX bytes and symbols, int4 filtering, text offset/length bounds, device compaction, stream/event ordering, pinned/fallback readback, row ordering, UTF-8 validation, and root APIs. Pass all-target/downstream checks, exact inventory, text + NULL + pool HAZARD gates, independent audit, and the canonical report card with no material ratio regression in either cache regime/layer. | STRUCT-001 execution-facade packet; STRUCT-001H-K point-read modules | `docs/CODE_SIZE.md`; AGENTS read-path benchmark standard |
