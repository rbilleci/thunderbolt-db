...
1. **STRUCT-001CA — complete ACL policy ownership.** Move role/object/schema permission checks, dependency checks,
   ACL target validation, and all GRANT/REVOKE mutations into the existing private `acl_execution` owner,
   preserving exact precedence/publication and exporting only proven cross-owner policy helpers.
...
|---|---|---:|---|---|---|
| **STRUCT-001CA** | NOW | P0 | Analyze and move the coherent ACL policy/mutation cluster from the server root into the existing private `acl_execution` owner: relation-kind and role/database/tablespace existence; grantee/current-role and role privilege evaluation; relation/schema/function/object permission errors; role dependency checks; ACL target validation; and relation/function/schema/default-table/database/tablespace GRANT/REVOKE helpers. Preserve exact kind/existence/permission/error precedence, PUBLIC/current-role behavior, dependency scans, ACL set mutations and empty-entry cleanup, dirty publication, helper arguments, and command persistence/tags. Export only `role_exists`, `database_exists`, `tablespace_exists`, `schema_permission_error`, `schema_usage_permission_error`, `object_access_permission_error`, `function_access_permission_error`, and `role_has_dependencies` for proven sibling consumers; command mutations become internal. Keep the owner below 1,500 lines and add no product API or cycle. Run focused ACL/permission/role/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BZ, STRUCT-001BP, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy ACL owner extension |
