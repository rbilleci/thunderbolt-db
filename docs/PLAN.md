...
1. **STRUCT-001AX — move simple cursor routing into its existing owner.** Move DECLARE/FETCH/MOVE/CLOSE routing,
   unsupported-form errors, and CLOSE state mutation from `execute_statement` into `cursor` behind one private
   tri-state delegate, preserving exact precedence, tags, errors, and position/lifetime behavior.
...
|---|---|---:|---|---|---|
| **STRUCT-001AX** | NOW | P0 | Move the ordered simple-query cursor routing family from `execute_statement` into the existing `cursor` owner: DECLARE, unsupported DECLARE, FETCH, unsupported FETCH, MOVE, unsupported MOVE, then CLOSE. Preserve `execute_declare_cursor` error consumption, forward counts/ALL and position mutation, CLOSE ALL/named behavior, missing-cursor `34000`, unsupported-form `0A000` messages, completion tags, and exact handled/unhandled ordering. Add one parent-private `try_execute_cursor_statement`; keep cursor parsing/execution helpers private to their existing boundary and do not move prepared/other statement behavior. Run focused cursor/prepare and lifecycle tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001AW, STRUCT-001AV analysis | PRODUCT-001 containment; existing cursor owner |
