...
1. **STRUCT-001BT — complete view dependency ownership.** Move recursive session-view dependency and dependent
   checks into the existing private `view_ddl` owner, preserving cycle guards, replacement/rename/drop behavior,
   and exact errors without widening visibility.
...
|---|---|---:|---|---|---|
| **STRUCT-001BT** | NOW | P0 | Analyze and move `session_view_depends_on`, its cycle-guarded recursive helper, and `session_view_has_dependents` from the server root into the existing private `view_ddl` owner. Preserve exact graph traversal, cycle termination, self exclusion, and CREATE OR REPLACE/rename/drop dependency behavior and errors. Keep all helpers module-private, remove obsolete root imports, and make no view semantic rewrite. Run focused view/materialized-view/dependency/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BS, STRUCT-001BH, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy view owner extension |
