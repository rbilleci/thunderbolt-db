...
1. **STRUCT-001AP — isolate legacy extended-query handlers.** Move Parse/Bind/Describe/Execute/Close dispatch,
   portal batching, and bind format decoding into one private owner. Keep cursor parsing, SQL statement execution,
   catalog, and relational behavior outside the module.
...
|---|---|---:|---|---|---|
| **STRUCT-001AP** | NOW | P0 | Analyze and extract the cohesive legacy extended-query handler cluster: `handle_parse`, `handle_bind`, `handle_describe`, `handle_execute`, `handle_close`, `execute_portal_batch`, format-code selection, bind decoding, and binary-format errors. Preserve statement/portal replacement rules, skip-until-Sync signaling, parameter/result format arity, binary int4/text decoding, row-description timing, suspension/resume positions, exact errors/tags, and five private dispatcher entry points. Cursor parsing/execution and SQL/catalog/relational work remain root dependencies; no host execution API. Run focused extended lifecycle/bind/cursor tests plus full protocol/driver/security gates and independent audit. | STRUCT-001AO | PRODUCT-001 containment; extended protocol boundary |
