...
1. **STRUCT-001BI — isolate bounded function lifecycle and invocation.** Move CREATE/RENAME/DROP FUNCTION and
   SELECT-function command arms into a private owner, preserving permissions/body validation, ACL/comments,
   result framing, dirty publication, exact errors/tags, and successful-parse routing.
...
|---|---|---:|---|---|---|
| **STRUCT-001BI** | NOW | P0 | Analyze and extract the complete successfully parsed bounded-function family from `execute_statement` into a private `function_execution` owner: CREATE FUNCTION, ALTER FUNCTION RENAME, DROP FUNCTION, and SELECT function invocation. Preserve exact schema/function permission and name/signature/existence precedence, supported body/return-type validation, OID allocation, ACL/comment cleanup or retargeting, dirty old/new keys, snapshot persistence, invocation result/error semantics, include-row-description behavior, tags, and fallthrough behind one parent-private delegate. Keep sequence/domain commands in the root and add no function capability or host product API. Run focused function/ACL/comment/pg_dump/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BH, STRUCT-001AV analysis | PRODUCT-001 containment; legacy bounded-function boundary |
