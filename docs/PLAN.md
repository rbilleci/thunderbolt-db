...
1. **STRUCT-001BQ — isolate simple-query relational DML.** Move the parsed INSERT/DELETE/UPDATE family into a
   private simple-query DML owner, preserving its distinct default, permission, candidate validation/rollback,
   dirty publication, persistence, exact errors/tags, and successful-parse routing.
...
|---|---|---:|---|---|---|
| **STRUCT-001BQ** | NOW | P0 | Analyze and move the complete successfully parsed simple-query INSERT/DELETE/UPDATE family from `execute_statement` into a private `simple_dml` owner. Preserve exact relation existence and ACL precedence, column mapping/type/default evaluation, row/filter/assignment validation, candidate-table construction, unique/check/foreign-key validation and rollback, counts, dirty publication, persistence, errors/tags, and parse-error fallthrough behind one parent-private delegate. Keep this owner separate from `extended_dml`, whose current semantics differ, and make no behavior merge in this structural slice. Keep SELECT in the root. Run focused DML/default/constraint/ACL tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BP, STRUCT-001AV analysis | PRODUCT-001 containment; coherent legacy simple-query DML owner |
