...
1. **STRUCT-001BC — isolate parsed session and transaction commands.** Move SET ROLE, BEGIN, COMMIT, ROLLBACK,
   and role-reset handling into a private session-command owner after successful parsing, preserving cursor and
   transaction state transitions, role validation, exact errors/tags, and ResetAll fallthrough.
...
|---|---|---:|---|---|---|
| **STRUCT-001BC** | NOW | P0 | Analyze and extract successfully parsed session/transaction commands from `execute_statement` into a private `session_commands` owner: SET ROLE/RESET ROLE validation and state, BEGIN, COMMIT [AND CHAIN], and ROLLBACK [AND CHAIN] including cursor cleanup. Preserve parse-error precedence, exact role lookup/error, transaction booleans, cursor mutation, tags, and the non-role `ResetAll` fallthrough. Use one parent-private tri-state delegate over a borrowed `Command`; any required role clone is allocation-only and must not change state/error ordering. Leave Flush/Truncate/KV and all relational/catalog commands in the root. Run focused role/transaction/cursor tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BB, STRUCT-001AV analysis | PRODUCT-001 containment; legacy session state boundary |
