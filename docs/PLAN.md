...
1. **STRUCT-001T — harden every typed-filter device input window.** Reject overflow/OOB fixed-width, bitmap,
   validity, and text-offset descriptors before launch, and bound device-read text spans so safe Rust APIs cannot
   trigger a CUDA illegal access from malformed offsets or resident bytes.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001T behavior child active; root now 22,601 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (948), `cuda_driver.rs` (883), `resident_memory.rs` (405), `resident_sort.rs` (846), `resident_count.rs` (1,066), `resident_gather.rs` (258), `resident_filter.rs` (1,064), `point_read_submission.rs` (442), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), `point_read_text.rs` (1,205 after STRUCT-001M/N correctness hardening), and `point_read_rows.rs` (475). STRUCT-001S moved int8/i128/text/UUID/bool predicate launchers and nullable-mask compaction byte-for-byte behind the private filter module. PTX, scalar limbs/signedness, text/UUID ordering, LIKE token lifetime, bool bitmap expansion, validity AND, device compaction, bounded index readback, and public APIs are unchanged. All-target/downstream checks, exact unchanged 101-test inventory, 31 non-ignored tests, and seven scalar/column/text/LIKE/UUID-NULL/bool HAZARD gates passed 3× sequential + 2× concurrent with zero CUDA safety errors. Its independent audit found a pre-existing safe-API device-bounds defect and promoted STRUCT-001T immediately. The canonical card remained green: i64/i128 1%-selectivity filters 165.6/286.0 GB/s in-L2 and 363.7/594.6 GB/s out-of-L2, `count_i32_compare` 0.87x/1.00x roofline, and 65,536-batch point reads 248.0M/250.9M lookups/s at p50 138/131us. **Next:** close the safe typed-filter OOB boundary before further extraction. |
...
|---|---|---:|---|---|---|
| **STRUCT-001T** | NOW | P0 | Harden all safe typed-filter entry points before any CUDA context/pool mutation: checked `offset + row_count * width` for int8/i128/UUID; checked `offset + ceil(row_count/32)*4` for bool and every validity bitmap; checked `(row_count+1)*8` text-offset arrays and text-bytes base. Pass the allocation-relative text-byte limit into equality/order/LIKE PTX and reject/skip malformed `start > end` or `end > limit` before any byte load. Add pure non-GPU overflow/OOB tests plus GPU malformed-text and full type/NULL differentials; correct the misplaced UUID/helper and stale bool-nullability docs; require HAZARD, independent audit, and the canonical card. No CPU validation scan or relational fallback. | STRUCT-001S; HIGH pre-existing safe-API OOB finding from its audit | GPU-native safety boundary; CUDA shared-context discipline |
