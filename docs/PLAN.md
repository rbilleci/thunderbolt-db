...
1. **STRUCT-001BG — isolate parsed index DDL.** Move CREATE INDEX, RENAME INDEX, and DROP INDEX command arms into
   a private index owner, preserving permissions/existence precedence, validation, comments/constraint guards,
   dirty publication, exact errors/tags, and successful-parse routing.
...
|---|---|---:|---|---|---|
| **STRUCT-001BG** | NOW | P0 | Analyze and extract the complete successfully parsed index DDL family from `execute_statement` into a private `index_ddl` owner: CREATE INDEX, ALTER INDEX RENAME, and DROP INDEX. Preserve exact schema permission and relation-kind/name/existence precedence, column/type checks, unique validation, checked OID allocation, primary/unique-constraint drop guards, duplicate-list handling, comment cleanup/retargeting, dirty index/comment publication, snapshot persistence, errors/tags, and fallthrough behind one parent-private delegate. Keep table constraints in `ddl_execution` and view/function/sequence/domain commands in the root. Run focused index/constraint/comment/pg_dump/shared-catalog tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001BF, STRUCT-001AV analysis | PRODUCT-001 containment; legacy index catalog boundary |
