...
1. **STRUCT-001CD — complete session-compatibility fallback ownership.** Move the final canonical session/golden
   compatibility match into `session_compat`, preserving exact match order, state mutations, rows, errors, and
   the terminal unsupported-shape response behind one bounded delegate.
...
|---|---|---:|---|---|---|
| **STRUCT-001CD** | NOW | P0 | Analyze and move the final `canonical.as_str()` session/golden compatibility match from `execute_statement` into the existing private `session_compat` owner behind one parent-visible delegate. Preserve exact branch order; transaction flags; `DEALLOCATE ALL` retention of extended prepared statements; golden prepared-state insert/lookup/removal; command tags; result columns/rows; missing-relation position; and the terminal unsupported-shape error. Keep parsing/catalog routing precedence unchanged, remove obsolete root code/imports, add no product API or cycle, and make no behavior cleanup during the move. Run focused session/prepare/golden/error tests, full protocol/driver/security gates, formatting/diff checks, and independent audit. | STRUCT-001CC, STRUCT-001BA, STRUCT-001AV analysis | PRODUCT-001 containment; existing legacy session-compatibility owner extension |
