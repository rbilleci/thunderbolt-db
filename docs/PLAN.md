...
1. **STRUCT-001Z — make device write-locate inputs total.** Bind every index and MVCC region to an owned,
   same-context typed descriptor with exact geometry; bound packed slots on-device; and drain every launched
   error path before owners or pooled buffers can drop.
...
|---:|---|---|
| 40,308 | `crates/execution/src/lib.rs` | **DECOMPOSE — STRUCT-001Z write-locate safety child active; root now 19,881 lines.** Handwritten production facade with distinct routing policy, CUDA runtime/context/driver layers, resident-memory ownership, MVCC batch encoding, point-read submission/result ownership, write locate/apply, and scan/aggregate/join/sort/expression kernel families. Public root symbols are consumed by engine, planner, metrics, observability, examples, and server tests; stable crate-root re-exports are the facade. **Completed modules:** `routing.rs` (241), RETIRE-001 `reference_operators.rs` (391), `mvcc_batch.rs` (208), `runtime_contract.rs`, sub-3,000-line test seams, `cuda_context.rs` (953), `cuda_driver.rs` (883), `resident_memory.rs` (405), `resident_sort.rs` (846), `resident_count.rs` (1,066), `resident_gather.rs` (258), `resident_filter.rs` (1,221 after STRUCT-001T safety hardening), `resident_aggregate.rs` (866 after STRUCT-001V safety hardening), `resident_group.rs` (1,282), `group_input.rs` (639), `write_locate.rs` (721), `point_read_submission.rs` (442), `point_read_submit.rs` (1,308), `point_read_dense.rs` (1,086), `point_read_bloom.rs` (206), `point_read_text.rs` (1,205 after STRUCT-001M/N correctness hardening), and `point_read_rows.rs` (475). STRUCT-001Y moved the four device write/visible-locate contracts, both byte-identical ASCII PTX programs, and both byte-identical launchers behind the private `write_locate` boundary with stable root re-exports. The exact 116-test inventory and 42/74 suite remain stable; write locate, visible DELETE/UPDATE, and sharded-duplicate gates passed 12 sequential and 8 concurrent invocations without CUDA 700/716/717. Independent audit found no extraction defect and promoted unsafe cross-context/index-geometry/raw-region/slot-boundary inputs plus post-launch error draining as STRUCT-001Z. **Next:** make that safe GPU boundary total before further extraction. |
...
|---|---|---:|---|---|---|
| **STRUCT-001Z** | NOW | P0 | Make both safe device-locate APIs total. Replace unowned raw version pointers with owned typed GPU-region descriptors; carry exact logical row/index geometry; reject cross-context owners, non-power-of-two or out-of-allocation table masks, incoherent hash shifts, region under-capacity, and count/size overflow before CUDA allocation or launch. Extend both descriptor/PTX paths with logical row bounds, reject any packed index slot outside its shard before version loads or output, and report a bounded device error that leaves the context reusable. Add guaranteed default-stream draining after every launched error, including DtoH failures. Update engine/streaming callers without host probing or fallback, add pure geometry tests plus retained-GPU malformed/cross-owner/fail-closed/reuse coverage, run write/visible-locate and lane DELETE/UPDATE/sharded HAZARD gates 3x sequential and 2x concurrent, and obtain independent audit. Preserve duplicate advancement, overflow sentinel, unsigned MVCC visibility, output order, and GPU-only key addressing. | STRUCT-001Y independent audit HIGH/MEDIUM | Safe CUDA API boundary; GPU-native write charter |
