...
- Timeline ancestry must be preserved for auditability. The local proof records an optional parent timeline id plus the fork transaction and timestamp boundary, then validates the registered sidecar and branch archive before a named timeline can be selected for recovery; production timeline management and GC remain future work.
