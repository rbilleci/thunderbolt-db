...
- Timeline ancestry must be preserved for auditability. The local proof records an optional parent timeline id plus the fork transaction and timestamp boundary, validates the registered sidecar and branch archive before a named timeline can be selected for recovery, and can prune a checked registry to one retained target plus ancestors while removing only unreferenced local branch artifacts. Automated production timeline failover orchestration remains future work.
