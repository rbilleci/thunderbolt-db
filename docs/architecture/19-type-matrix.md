...
- **`AVG(int4)` — DONE**: `SELECT AVG(col) FROM t WHERE <pred>` = the GPU SUM / the count, as numeric
  (PG). NO new kernel -- reuses the SUM gather-reduce; the final scalar `sum/count` reuses the existing
