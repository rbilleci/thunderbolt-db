...
2. WAL replay restores catalog plus table data to the committed boundary. The file-backed P5 slice reuses the existing durable-prefix replay path, so recovery applies only flushed records and rebuilds volatile relational indexes from committed WAL contents.
...
5. Storage/recovery docs and runbooks match the implemented behavior. First P5 docs reconciliation landed on 2026-05-11 in `docs/architecture/05-storage-and-recovery.md`, `docs/operations/runbooks.md`, README, compatibility matrix, and this roadmap; the file-backed WAL segment update now documents the durable segment boundary and remaining checkpoint/control-metadata gap.
