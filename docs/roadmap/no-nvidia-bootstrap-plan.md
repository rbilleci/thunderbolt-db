...
- Next obvious extension boundary: extend the new frame-bundle surface beyond the current exact-membership, exact full-path equality, ordered subpath matching, and path-summary controls into richer quantified bundle predicates without weakening the explicit fallback contract.
