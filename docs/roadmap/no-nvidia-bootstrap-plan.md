...
- Support text parameters and PostgreSQL-compatible text parameter/result format-code counts for the first relational SQL subset.
- Keep unsupported non-SELECT and broader unsupported parameterized SELECT extended statements, binary formats, copy, function call, and advanced portal behavior explicitly classified. Real `psql` parameterized extended INSERT, parameterized JOIN SELECT, and COPY SQL traffic are now pinned as explicit unsupported errors with post-error session recovery; frontend COPY data/function-call frames remain unsupported wire paths with named unsupported-feature errors, raw missing `Describe` / `Execute` targets, raw binary parameter/result `Bind` format rejection, mismatched raw `Bind` parameter/result format-code counts, and raw frontend `CopyData`/`CopyDone`/`CopyFail`/`FunctionCall` now have helper-level coverage for skip-until-`Sync` recovery and skipped side effects, while raw missing `Close` targets complete successfully per PostgreSQL protocol semantics.
...
2026-05-15 follow-up: scenario 215 extends the explicit unsupported COPY boundary to real PostgreSQL 16 `psql \copy ... TO PROGRAM` client-side copy-out traffic, proving the generated `COPY ... TO STDOUT` request receives the stable unsupported COPY error before frontend COPY data flow and later supported SELECT traffic recovers.
