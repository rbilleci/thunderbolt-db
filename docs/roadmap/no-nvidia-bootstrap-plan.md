...
- Current bootstrap truth: the engine-facing MVCC slice now supports full scans + single-key lookups + key-batch fan-in lookups + value→key reference expansion + value→key-prefix expansion + source-preserving value→key→prefix expansion with snapshot visibility, prefix/value/composite/range filters, key/value ordering, projection, optional source provenance on join-adjacent rows, and post-order limit under an explicit `GpuMvccReadParityGap` fallback contract.
- Next obvious extension boundary: widen the current join-adjacent/source-preserving slice into richer relational composition (for example explicit join-side projections or deeper multi-join chaining) without weakening the explicit fallback contract.
