...
4. **Provenance and filter expansion parity**
   ...
   - Progress on 2026-05-10: gate 4 has started with the first named provenance-aware predicate category. `CudaMvccExecutionBackend` now supports single-frame `ProvenanceKeyPrefix` and `ProvenanceValueEquals` filters, including nested `All` / `Any` composition with existing supported predicates, over CPU-resolved provenance rows from value-chain sources. This keeps source resolution and composition movement out of scope for gate 4 while proving CUDA predicate masks against CPU output. Broader provenance bundle/path predicates, provenance ordering/projection movement, and native GPU source resolution for join/composition shapes remain open.
