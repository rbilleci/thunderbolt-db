...
- Remaining MVCC CUDA gaps: provenance ordering/projection movement, native GPU source resolution for join/composition shapes, and composed source shapes still need to be ported behind the existing backend boundary before broader shapes may report GPU execution.
