...
- Keep unsupported non-SELECT and broader unsupported parameterized SELECT extended statements, binary formats, copy, function call, and advanced portal behavior explicitly classified. Real `psql` parameterized extended INSERT, parameterized JOIN SELECT, and COPY SQL traffic are now pinned as explicit unsupported errors with post-error session recovery; frontend COPY data/function-call frames remain unsupported wire paths with named unsupported-feature errors, and raw missing `Describe` / `Close` / `Execute` targets, raw binary parameter/result `Bind` format rejection, mismatched raw `Bind` parameter/result format-code counts, and raw frontend `CopyData`/`CopyDone`/`CopyFail`/`FunctionCall` now have helper-level coverage for skip-until-`Sync` recovery and skipped side effects.
...
2026-05-15 follow-up: scenario 198 pins real PostgreSQL 16 `psql` `FETCH_COUNT` over SQL-prepared execution, proving `DECLARE ... CURSOR FOR EXECUTE <prepared>(...)` is supported for the current relational SELECT subset and uses the same SQL prepared result path as direct `EXECUTE`. Scenario 199 extends that real-client cursor path to quoted zero-parameter SQL prepared names, and helper-level raw wire coverage now accepts `Parse`/`Bind`/`Describe`/`Execute` for `DECLARE ... CURSOR FOR EXECUTE <prepared>($1)` by inferring outer bind metadata from SQL prepared state while describing the cursor-declaration portal as `NoData`.
