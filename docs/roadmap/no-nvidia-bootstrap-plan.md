...
- Keep unsupported non-SELECT and broader unsupported parameterized SELECT extended statements, binary formats, copy, function call, and advanced portal behavior explicitly classified. Real `psql` parameterized extended INSERT, parameterized JOIN SELECT, and COPY SQL traffic are now pinned as explicit unsupported errors with post-error session recovery; frontend COPY data frames and FunctionCall frames remain unsupported wire paths with named unsupported-feature errors, and raw frontend `CopyData`/`FunctionCall` now have helper-level coverage for skip-until-`Sync` recovery and skipped side effects.
