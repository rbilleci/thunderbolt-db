...
- Next obvious extension boundary: extend the frame-bundle surface from exact occurrence distances toward broader relative-occurrence constraints (for example ranged occurrence distances or mixed-subpath relationships) without weakening the explicit fallback contract.
