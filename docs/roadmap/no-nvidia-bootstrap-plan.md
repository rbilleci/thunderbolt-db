...
- Keep unsupported binary formats, copy, function call, and advanced portal behavior explicitly classified. Real `psql` COPY SQL traffic is now pinned as an explicit unsupported error with post-error session recovery; frontend COPY data frames and FunctionCall frames remain unsupported wire paths with named unsupported-feature errors.
