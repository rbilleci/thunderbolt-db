...
- Progress on 2026-05-10: source-relative and branch-label filters (`SourceKeyPrefix`, `SourceValueEquals`, `BranchLabelEquals`) are now CUDA-eligible over CPU-resolved rows via the existing CUDA mask bridge, with classifier, CPU parity, and local ignored CUDA coverage. Remaining post-milestone gaps are native GPU source resolution for join/composition shapes, composition beyond native-source `Concat`, ordering beyond key asc/desc and provenance ordering, and non-provenance source/branch ordering.
