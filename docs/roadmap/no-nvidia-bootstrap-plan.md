...
4. **Provenance and filter expansion parity**
   ...
   - Closeout on 2026-05-10: gate 4 is closed for the current filter enum surface. All provenance filter categories are CUDA-eligible over CPU-resolved provenance rows with CPU parity coverage; the remaining filter-shaped fallbacks are now explicitly classified as `source_relative_filter` (`SourceKeyPrefix`, `SourceValueEquals`) or `branch_label_filter` (`BranchLabelEquals`), which depend on broader source/composition movement and belong to gate 5 rather than missing predicate plumbing.
...
- Progress on 2026-05-10: CPU-resolved provenance sources can now keep CUDA execution when they request provenance ordering and provenance projection, provided they also contain a supported provenance predicate for CUDA mask execution. This closes the first post-milestone provenance order/projection routing gap without changing `MvccReadRow`: CUDA still performs source/visibility/filter masking, then the existing host row contract performs provenance sort/project materialization. Native GPU source resolution for join/composition shapes, composition beyond native-source `Concat`, source-relative filters, and branch-label filters remain explicit post-milestone `GpuMvccReadParityGap` work.
- Progress on 2026-05-10: supported CUDA paths now apply post-filter/post-order `limit` handling without changing `MvccReadRow`. Single-source CUDA execution truncates the matched resolved rows after optional ordering and before projection; native `KeyBatchLookup` and `Concat` truncate after request-order/source-order fan-in. CPU parity coverage and local ignored CUDA regressions cover ordered full-scan limits plus concat/key-batch fan-in limits. Remaining post-milestone gaps are native GPU source resolution for join/composition shapes, composition beyond native-source `Concat`, ordering beyond key asc/desc and provenance ordering, non-provenance source/branch ordering, and source-relative/branch-label filter semantics.
...
- The first MVCC CUDA integration is intentionally narrow: supported filterless full-scan/key-lookup/key-batch reads, key ascending/descending order for full-scan/key-lookup reads, post-filter/post-order limits, CUDA-backed `Concat` over those native child sources, and reads with exact numeric or byte/string `ValueEquals` filters, key-prefix filters, general bytewise key ranges, single-frame provenance key-prefix/value-equality filters, provenance bundle key/value/key-value equality, counted membership, key-prefix filters, stable bundle path predicates (`PathEquals`, `PathContains`, `PathCountAtLeast`, `PathPairAtDistance`, path prefix/suffix/slice/segment equality, and bundle length), same-subpath occurrence path predicates, and mixed-subpath occurrence path predicates over CPU-resolved provenance rows, and nested `All`/`Any` combinations of supported predicates can now use CUDA mask primitives plus device-side MVCC source/visibility masking where applicable and report `executed_target = gpu(...)` on local NVIDIA hardware. Supported full scans, key lookups, and first key-batch lookups feed all stored MVCC versions into the CUDA row batch before source/visibility masking; broader source/order/projection shapes continue to fall back under `GpuMvccReadParityGap`.
- Remaining MVCC CUDA gaps: native GPU source resolution for join/composition shapes, composition beyond native-source `Concat`, ordering beyond key asc/desc on native full-scan/key-lookup shapes, non-provenance source/branch ordering, and source-relative/branch-label filter semantics still need to be ported behind the existing backend boundary before broader shapes may report GPU execution.
