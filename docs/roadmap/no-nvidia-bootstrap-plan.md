...
- Current bootstrap truth: the engine-facing MVCC slice now supports full scans + key lookups with snapshot visibility, prefix/value/composite/range filters, key/value ordering, projection, and post-order limit under an explicit `GpuMvccReadParityGap` fallback contract.
- Next obvious extension boundary: join-adjacent or other multi-source shapes without weakening the explicit fallback contract.
