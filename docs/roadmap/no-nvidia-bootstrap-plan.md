...
- Next obvious extension boundary: extend the new frame-bundle surface beyond the current exact-membership, exact full-path equality, and path-summary controls into richer positional or quantified bundle predicates without weakening the explicit fallback contract.
