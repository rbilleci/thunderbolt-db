...
Priority: highest
Status: active highest-priority queue item. Current focus: richer resume/catch-up/snapshot-install stress paths now that `status_snapshot()` publishes live progress, durable progress, and recovery-gap alignment directly; keep tightening rejected/accepted follower transitions, durable term/alignment and durable-never-ahead status invariants, and stale/snapshot-frontier validation so snapshot identity cannot drift away from the served frontier and impossible higher-frontier/lower-term installs are ignored, including a hard same-frontier snapshot-id alignment invariant and later role/term changes that discard speculative tail.
