...
2026-05-15 follow-up: scenario 198 pins real PostgreSQL 16 `psql` `FETCH_COUNT` over SQL-prepared execution, proving `DECLARE ... CURSOR FOR EXECUTE <prepared>(...)` is supported for the current relational SELECT subset and uses the same SQL prepared result path as direct `EXECUTE`.
