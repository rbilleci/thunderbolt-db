...
- Current bootstrap truth: the engine-facing MVCC slice now supports full scans + single-key lookups + key-batch fan-in lookups + value→key reference expansion + value→key-prefix expansion + source-preserving value→key→prefix expansion + source-preserving value→key→value→key chaining + source-preserving value→key→value→prefix chained fan-out with snapshot visibility, prefix/value/composite/range filters, key/value ordering, target-key plus seed-side value projection for join-adjacent rows, optional source provenance on join-adjacent rows, and post-order limit under an explicit `GpuMvccReadParityGap` fallback contract.
- Next obvious extension boundary: widen the current join-adjacent/source-preserving slice beyond chained fan-out plus seed-side value projection (for example source-aware filters/order clauses or additional mixed result-shape controls) without weakening the explicit fallback contract.
