...
- Current bootstrap truth: the engine-facing MVCC slice now supports full scans + single-key lookups + key-batch fan-in lookups + value→key reference expansion with snapshot visibility, prefix/value/composite/range filters, key/value ordering, projection, and post-order limit under an explicit `GpuMvccReadParityGap` fallback contract.
- Next obvious extension boundary: widen the new join-adjacent slice toward source-preserving joins or prefix-driven foreign-key expansion without weakening the explicit fallback contract.
