...
2026-05-15 follow-up: scenario 198 pins real PostgreSQL 16 `psql` `FETCH_COUNT` over SQL-prepared execution, proving `DECLARE ... CURSOR FOR EXECUTE <prepared>(...)` is supported for the current relational SELECT subset and uses the same SQL prepared result path as direct `EXECUTE`. Scenario 199 extends that real-client cursor path to quoted zero-parameter SQL prepared names, and helper-level raw wire coverage now accepts `Parse`/`Bind`/`Describe`/`Execute` for `DECLARE ... CURSOR FOR EXECUTE <prepared>($1)` by inferring outer bind metadata from SQL prepared state while describing the cursor-declaration portal as `NoData`. Scenario 205 adds the corresponding real-client `psql \bind` cursor-declaration path for SQL-prepared outer placeholders, proving `NoData` description and later cursor installation/fetch/close recovery. Scenario 208 extends that cursor path to negative SQL-prepared `LIMIT` binds, proving metadata-only `NoData` Describe does not execute or install the cursor before the later execution error.
