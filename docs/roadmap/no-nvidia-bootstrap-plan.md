...
- Next obvious extension boundary: extend the new frame-bundle surface beyond the current key/value prefix-equality and path-summary controls into richer exact-membership or mixed key/value bundle predicates without weakening the explicit fallback contract.
