...
- Connection-security posture evidence includes the local/dev trust-auth, no-TLS boundary, the opt-in production security profile v1 TLS+SCRAM verifier gate, the local/test plaintext bootstrap boundary, and explicit remaining security non-claims.
...
- Production profile v1 requires explicit TLS certificate/key, auth user, and exactly one credential source. A SCRAM-SHA-256 verifier or verifier file is the production-oriented source; plaintext password input is retained only as a local/test bootstrap path and is rejected when combined with verifier material.
...
- `scripts/run_replication_channel_security_preflight.sh` generates a local CA plus server/client certificates, proves a rustls mTLS AppendEntries request/response, rejects missing certificate/key material before serving, and rejects a TLS client that omits client certificate material. The plain TCP AppendEntries service/container/Compose smokes remain the dev/test transport profile.
...
- `scripts/run_replication_deployment_preflight.sh` aggregates the packaged follower-service smoke, systemd contract verification, Kubernetes manifest verification, Docker Compose restart smoke, and channel-security preflight into one bounded local deployment preflight.
...
- Channel-security preflight output includes `operational_replication_channel_security_smoke=passed`, `replication_channel_security_transport=mtls_append_entries`, `replication_channel_security_missing_material_rejection=passed`, `replication_channel_security_missing_client_cert_rejection=passed`, `replication_channel_security_plain_transport_profile=dev_test_only`, and explicit gaps for certificate lifecycle and production trust distribution.
