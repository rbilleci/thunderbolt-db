# ARCHIVED — Full decision chronology before documentation unification (2026-07-12)

> Historical ADR detail and experimental chronology. It is not an executable plan. The concise current ADR
> ledger is `docs/DECISIONS.md`; current work is `docs/PLAN.md`.
...
[CHARTER.md](../../CHARTER.md); the *design* in [ARCHITECTURE.md](../../ARCHITECTURE.md). Words such as `NEXT`, `OPEN`,
`PARKED`, or `DEFERRED` inside an ADR describe the state when that decision was recorded; they never create
current work. The only executable ledger is [PLAN.md](../../PLAN.md).
...
- **Context:** Read-path assessment findings D3/D4 (`docs/archive/reviews/read-query-path-assessment.md` §3 + the
...
- **Decision:** Build a **streaming executor** for out-of-core execution. A query whose working set exceeds the GPU
  ...
  charter-compliant (ADR-006/007) and enabled the now-completed production S10d host-read deletion without losing
  over-VRAM coverage.
...
- **Consequences:** Extends ADR-010. The current sharded read path (recompact ALL shards into one unified resident
  ...
  S10d** alongside S-F; both are now closed. Work item: PLAN §2 S-E (reframed from "multi-GPU spill" to the
  streaming executor + cross-shard combine).
...
- **Consequences:** Renames `RelationalResidentPartition → RelationalResidentShard` (**landed: S-A, 2026-06-27**).
  The commit-triggered producer landed as S-B and **S-F flipped it production-default ON on 2026-07-12** after
  incremental writes, dense/index reads, bounded streaming, and the mixed gate closed the old hold. Recovery now
  replays first and bulk-admits once. Production derives an 80%-VRAM default budget. Mandatory replacements allocate
  before a two-phase deterministic unified/shard eviction, and one allocation transaction caps actual payload,
  version/identity-region, rollover, and lazy device-index allocations. Effective defaults also feed secondary-GPU
  eligibility. The streaming
  executor and multi-GPU partial scheduler serve over-budget reads; production host SELECT fallback is deleted.
...
- **First measurement (2026-06-27, RTX PRO 6000; `engine/examples/oltp_auto_admit_ab` +
  ...
  the persistent-kernel wave engine (ADR-009) with on-GPU result slots. **Historical decision:** S-F stayed OFF at
  this measurement. It was superseded by the 2026-07-12 flip after the production dense/index route, incremental
  writes, bounded streaming, and mixed gate eliminated the measured blockers.
...
- **R2.2 FOLLOW-UP REVIEW correctness gates C1/C2/C3 (2026-06-28, commit `6b28832e`; `docs/archive/reviews/r2.2-wave-port-followup-review.md`).**
...
- **Consequences:** The GPU-native read-path campaign (S1–S10c) executed this for the read path; production S10d is
  now closed after STRATA S-F. The remaining CPU oracle is `cfg(test)` only and is an ADR-007 test-infrastructure debt.
...
- **Decision:** No CPU-only / GPU-absent / hybrid steady-state mode. Any CPU relational execution is interim
  ...
  re-implementation.
- **Consequences:** STRATA S-F is now default and the **production** CPU relational read/execute path is retired:
  `finalize_relational_select`, CPU-pinned SELECT, `CpuMvccExecutionBackend`, and backend-chain fallback compile only
  under `cfg(test)`. Non-test decline/fault policy is fail-loud GPU-required; catalog/materialized-view reads use
  transient GPU relations. Published residency retains no decoded host-row shadow, and bounded SQL-function literal
  results also materialize through a transient GPU relation. `FirstCudaSliceParityBackend` and the CPU oracle remain
  test-only infrastructure pending ADR-007's stricter GPU-native-oracle cleanup.
