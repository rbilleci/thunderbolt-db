# ARCHIVED — Non-int4 point-lookup index proposal (2026-06-30)

> Historical proposal containing obsolete branches, file:line pointers, measurements, and phasing. The durable
> representation/correctness constraints were retained in `docs/design/non-int4-index-design-inputs.md`.
...
> The onboarding notes below describe the 2026-06-30 tree. Reconfirm entry points before using them under
> **READ-002** or **R3-002**.
...
~30ms tail is fixed. At that date this was proposed as read-path key-type breadth, independent of the R3 write
program. Current priority and scope are **READ-002** and **R3-002** in PLAN; the branch and performance numbers
...
**Current authorities:** `docs/PLAN.md`, `docs/STATUS.md`, and `docs/DECISIONS.md`. The result-path arc
`11M->14.6M->44.1M->78.5M->121.6M` below is historical measurement context.
...
3. **Result path** — reuse the lpb arc machinery (batched result + columnar drain + O(n) scatter), with a
   ...
   `docs/archive/optimizations/cross-kernel-result-path-transfer.md`.
...
2. **text — second.** Very common natural key; hash build+probe already exists (`hash_join_build_text` /
   ...
   (Historical aside: the same index was considered for `docs/archive/future/redis/`.)
