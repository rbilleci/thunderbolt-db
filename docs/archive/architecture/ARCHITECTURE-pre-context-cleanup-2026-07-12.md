# ARCHIVED — Architecture before context cleanup (2026-07-12)

> Historical mixed current/target design. It includes retired wave-engine and superseded implementation
> descriptions. Current architecture is `docs/ARCHITECTURE.md`; work is owned only by `docs/PLAN.md`.
...
authoritative on which is which. Target language and open design questions in this document are not scheduled
work; only PLAN task IDs can activate them.
...
- **Cache-manager state machine** (`RelationalResidentCache`): Absent → Admitting → Valid → Invalidated → Refreshing
  → Evicting. **Admission** (`admit_relational_residency_snapshot`): per-GPU **byte budget** (explicit, or 80% of
  physical VRAM by production default); fits → admit; needs room → deterministic eviction across unified and shard
  tables. The complete mandatory replacement is allocated before a two-phase eviction prefix is published, so an
  allocation or fit failure cannot damage the existing resident set. A shared allocation transaction enforces the
  cap over actual payload, version/identity-region, rollover, and lazy device-index allocations; optional indexes
  decline to resident GPU scans at the cap. **A working set larger than the byte budget is served
  by the streaming executor (§13, ADR-012): shards are admitted on demand and evicted under budget, with host/NVMe as
  the cold STORAGE tier — the host is never an execution tier.** **Invalidation**
...
- **The admission producer _(built; production default)_:** a **commit-triggered**, post-`publish_committed_seq`,
  best-effort step (via the `&self` + held-catalog-guard seam) that scans the new generation and lays it down as
  shards. Recovery suppresses per-record admission, replays the full durable store, and bulk-admits once at the
  settled boundary. `auto_admit_on_commit` remains a kill switch but defaults ON.
- **Read path:** the resident route reads device memory directly for one shard; for N shards it either recompacts
  fixed-width/text/bool/NULL sections on-device (including text offset rebasing) or uses the streaming push-down +
  cross-shard combine model (§13). Version/identity sidecars are carried with the same published descriptor.
  `RelationalResidencyEntry` publishes only that device descriptor: decoded rows used by admission are discarded
  after upload, and commit append maintenance never clones or extends a host row mirror.
...
- **Transient relations:** catalog/materialized-view rows and bounded SQL-function literal results are uploaded as
  short-lived device relations and use the general projection executor. Parsing or synthesizing those rows is host
  control-plane work; result materialization is not.
...
- **Scalar functions & functional indexes (e.g. `lower()`) — DESIGN SPACE, NOT DECIDED; the ONLY evidence is int4.** R1
  ...
  **NOT O(1) for text** (a hash hit needs a full-string compare O(W), and variable-length keys need a key-arena
  ...
  - **DON'T-ASSUME (so a future agent doesn't ship a bad algorithm):** no O(1) text probe (O(W) on collision); no cheap
    ...
    for text. Measurement and any implementation are owned by **READ-002**: build cost + index memory vs
    `(N, W)`, probe cost vs `W`, the frequency crossover against a fused scan, and the VRAM/chunking ceiling.
    Until measured, only the per-row fused floor is safe to rely on.
...
- **Isolation:** SI for autocommit _(built)_; RR / RC / **SSI** are target work under **R3-003**. For deterministic GPU waves,
...
- **Streaming executor — out-of-core _(built, ADR-012)_:** a query whose working set exceeds
  ...
  charter-compliant (ADR-006/007) and allowed the now-completed production S10d host-read deletion without losing
  over-VRAM coverage.
...
- **Multi-GPU partial execution _(built; ≥2-GPU hardware gate pending)_:** scalar, projection, and grouped chunks
  round-robin across physical GPUs that are healthy and carry at least the coordinator's effective query budget
  (explicit override or the production 80%-of-physical default). Full relation
  payloads never move device-to-device; only final framing/associative partials return to the default-GPU combine.
  Ordered/JOIN/window folds remain coordinator-local. Cache-aware replication of critical shards remains target work.
- **Cross-shard combine (the shared mechanism — used by both the streaming executor and multi-GPU spill):** push the query fragment to each shard (partial agg / local
  ...
  Same-GPU shards combine via `cuMemcpyDtoD`; cross-GPU fold inputs are independently uploaded from the cold bytes and
  only their small framed partials return to the coordinator—never a full relation cross-GPU copy.
...
view. The open-loop offered-rate latency evidence gate is **BENCH-001** in PLAN.
