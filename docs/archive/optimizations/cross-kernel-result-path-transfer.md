# ARCHIVED — Cross-kernel result-path transfer analysis
...
other kernels. Historical companion `read-path-lpb-levers.md` is not retained in this tree;
[`scan-kernel-levers.md`](scan-kernel-levers.md) is retained, while `small-batch-latency-levers.md` is not.
