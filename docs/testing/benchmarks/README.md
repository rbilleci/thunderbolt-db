...
SQL-visible bulk MVCC COPY admission throughput; the latest bounded endpoint
probe also projects the 64,424,510-row 10% load outside the approved 6h budget.
...
- `engine_sql_visible_mvcc_bulk_copy_admission_required`: default PostgreSQL
  ...
  needs a defended SQL-visible bulk MVCC COPY admission path before the 10%
  single-load default/tuned/GPU curves can fit the approved worker budget.
