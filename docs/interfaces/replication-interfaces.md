...
- Helper APIs: `ReplicationWatermarks::has_backlog_blocker(bit)` and typed helpers via `BacklogBlocker` (`bit()`, `as_str()`, `ReplicationWatermarks::has_blocker_kind(kind)`, `ReplicationWatermarks::backlog_blockers()`) for downstream readiness/admission automation without manual bit arithmetic.
