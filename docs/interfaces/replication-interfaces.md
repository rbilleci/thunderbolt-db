...
- **Newer-leader acceptance repair stability:** if a newer leader successfully catches a follower up after discarding stale speculative tail, `status_snapshot()` reflects only the fresh term's committed/apply gap and surviving speculative tail; stale pre-step-down entries do not leak into the new epoch's live/durable delta.
