...
- **Advanced-frontier identity handoff stability:** if that advanced-frontier snapshot arrives while speculative tail still exists, later newer-leader rejection/repair and restart-resume handoffs preserve the exact advanced snapshot identity while only the speculative tail and commit/apply lag evolve.
