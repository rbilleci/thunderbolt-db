...
- Next obvious Q2 extension is widening the new explicit source-composition surface beyond ordered `Concat` (for example deduplicating/intersection semantics or deeper nested join-composition helpers) without weakening the explicit GPU fallback contract.
