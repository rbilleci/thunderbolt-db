...
- `FollowValueKeyRefValueKeyRefValueKeyRefPrefixes { keys }` pushes that deeper chain through one more referenced-row hop before the final prefix expansion, proving the contract still holds for a distinctly more nested fan-out landing shape.
...
- Next obvious Q2 extension is widening the deeper source-preserving join-composition surface beyond the new deeper nested fan-out helper with a more composable nested-join mechanism instead of indefinite one-off variants, without weakening the explicit GPU fallback contract.
