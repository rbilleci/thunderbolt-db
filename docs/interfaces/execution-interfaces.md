...
- Supported sources:
  ...
  - `FollowValueChainBranches { keys, plans }` (generic source-preserving branch helper; for each visible seed row in request order, resolves each listed linear value-chain plan in branch-list order before moving to the next seed)
...
- `FollowValueChainBranches { keys, plans }` is the first explicit branch-form nested-join surface; unlike `Concat` over multiple linear chains, it groups branch results by seed key first, then by branch-list order, so one query can keep related branch expansions adjacent without another result-shape rewrite.
...
- Next obvious Q2 extension is widening the new nested-join surface again with explicit fan-in policies or richer provenance controls without weakening the explicit GPU fallback contract.
