...
- CUDA runtime availability: `gpu_db_execution::CudaDriverRuntime::probe()` dynamically loads `libcuda`, initializes the driver, and reports a `CudaRuntimeSnapshot { driver_available, driver_version, device_count, devices }` for device routing. `CudaDriverRuntime::launch_smoke_add_one(...)` proves module load, device allocation, kernel launch, synchronization, and D2H copy on local NVIDIA hardware. `CudaDriverRuntime::filter_equal_u32_mask(...)` is the first device-side predicate primitive: it copies a fixed-width batch to the GPU, writes an equality mask in a CUDA kernel, and copies the mask back for CPU parity checks. MVCC reads still need row encoding plus backend integration before they may report `executed_target = gpu(...)`.
- CUDA backend attachment: `Engine::execute_mvcc_query_with_cuda_driver_probe(...)` routes through `CudaMvccExecutionBackend` and then the existing CPU fallback chain. It reports `GpuUnavailable` when the driver/device cannot be used and `GpuMvccReadParityGap` when hardware is present but the first-slice MVCC kernels are not ported yet.
