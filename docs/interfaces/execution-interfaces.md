...
- CUDA runtime availability: `gpu_db_execution::CudaDriverRuntime::probe()` dynamically loads `libcuda`, initializes the driver, and reports a `CudaRuntimeSnapshot { driver_available, driver_version, device_count, devices }` for device routing. `CudaDriverRuntime::launch_smoke_add_one(...)` also proves module load, device allocation, kernel launch, synchronization, and D2H copy on local NVIDIA hardware. This is hardware/launch eligibility only; MVCC kernels still need to land behind the backend boundary before MVCC reads may report `executed_target = gpu(...)`.
