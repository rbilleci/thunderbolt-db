...
- Supported sources:
  ...
  - `FollowValueChainBranches { keys, plans, fan_in }` (generic source-preserving branch helper; for each visible seed row in request order, resolves listed linear value-chain plans under an explicit fan-in policy before moving to the next seed)
...
- `FollowValueChainBranches { keys, plans, fan_in }` widens that branch-form nested-join surface with explicit per-seed fan-in policy; `AllBranches` preserves the original seed-major branch grouping, while `FirstNonEmptyBranch` turns branch lists into deterministic fallback/alternative expansion plans without changing the row contract.
...
- Next obvious Q2 extension is richer source-provenance controls on top of the now-explicit branch fan-in surface, without weakening the explicit GPU fallback contract.
