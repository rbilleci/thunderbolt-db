...
- CUDA backend attachment: `Engine::execute_mvcc_query_with_cuda_driver_probe(...)` routes through `CudaMvccExecutionBackend` and then the existing CPU fallback chain. On CUDA-capable hardware it may execute the first narrow MVCC slice with `executed_target = gpu(...)` for supported filterless full-scan/key-lookup queries and queries whose filter tree is composed from numeric/string `MvccReadFilter::ValueEquals`, `MvccReadFilter::KeyPrefix`, `MvccReadFilter::KeyRange`, and non-empty nested `All`/`Any` combinations of those predicates. Supported full scans now feed every stored MVCC version into the CUDA row batch and use the device visibility mask for snapshot row selection; fallback re-resolves the CPU-visible rows before running the reference backend. Key lookup source resolution and projection still use the existing engine/storage contract. It reports `GpuUnavailable` when the driver/device cannot be used and `GpuMvccReadParityGap` for unsupported source/order/projection/limit/filter shapes.
