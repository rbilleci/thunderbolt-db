...
- CUDA runtime availability: `gpu_db_execution::CudaDriverRuntime::probe()` dynamically loads `libcuda`, initializes the driver, and reports a `CudaRuntimeSnapshot { driver_available, driver_version, device_count, devices }` for device routing. `CudaDriverRuntime::launch_smoke_add_one(...)` proves module load, device allocation, kernel launch, synchronization, and D2H copy on local NVIDIA hardware. `CudaDriverRuntime::filter_equal_u32_mask(...)` and `filter_equal_bytes_mask(...)` are the first device-side predicate primitives: they copy fixed-width or byte/string batches to the GPU, write an equality mask in a CUDA kernel, and copy the mask back for CPU parity checks.
- CUDA backend attachment: `Engine::execute_mvcc_query_with_cuda_driver_probe(...)` routes through `CudaMvccExecutionBackend` and then the existing CPU fallback chain. On CUDA-capable hardware it may execute the first narrow MVCC slice with `executed_target = gpu(...)` for supported full-scan/key-lookup queries whose exact filter is numeric/string `MvccReadFilter::ValueEquals` or `MvccReadFilter::KeyPrefix`; row source resolution, snapshot visibility, and projection still use the existing engine/storage contract. It reports `GpuUnavailable` when the driver/device cannot be used and `GpuMvccReadParityGap` for unsupported filter shapes or any remaining MVCC row/key-range predicate gap.
