...
- STRUCT-001JP is closed. The exact 17-test SQL scalar predicate type matrix plus its one helper now lives in the
  rustfmt-clean 1,459-line private `tests/resident_expr/sql_scalar_predicates.rs` child; the 8,921-line test root
...
- STRUCT-001JR is closed. The exact two-test/seven-helper checked-int4 arithmetic family now lives in the
  rustfmt-clean 177-line private `tests/resident_expr/checked_arithmetic.rs` child; the 8,166-line test root
...
- STRUCT-001JT is closed. The exact five-test direct-`ResidentExpr` predicate matrix now lives in the
  rustfmt-clean 398-line private `tests/resident_expr/programmatic_predicates.rs` child; the 6,665-line test root
  remains PLAN-owned. Twenty-five focused actual-GPU executions, both 505/487 modes, the complete 992 suite,
  static/source/cleanup/inventory gates, and independent audit pass. The actionable inventory remains 13;
  STRUCT-001JU owns the exact nullable-grouping family at current root lines 92–569.
- STRUCT-001JU is closed. The exact nine-test nullable grouping contract now lives in the rustfmt-clean 482-line
  private `tests/resident_expr/nullable_grouping.rs` child; the 6,187-line test root remains PLAN-owned.
...
- STRUCT-001JW is closed. The exact 23-test composite grouping matrix and paired bare-bigint sentinel now live in
  the rustfmt-clean 974-line private `tests/resident_expr/composite_grouping.rs` child; the 4,684-line test root is
  below the critical threshold but remains PLAN-owned. One hundred fifteen focused actual-GPU executions, both
  505/487 modes, the complete 992 suite, static/source/cleanup/inventory gates, and independent audit pass. The
  actionable inventory remains 13; STRUCT-001JX owns the exact COUNT(DISTINCT) matrix at current root lines
  352–1138.
- STRUCT-001JX is closed. The exact 23-test grouped/scalar COUNT(DISTINCT) matrix and mixed-width companion now
  live in the rustfmt-clean 791-line private `tests/resident_expr/count_distinct.rs` child; the 3,897-line test
...
1. **STRUCT-001KW:** isolate exact sharded reductions current `tests/resident_route.rs` lines 1033–1939 as mapped in
   `PLAN.md`, preserving every shard/chunk/query/aggregate/decline contract and proving all five paths serially and
   concurrently on actual CUDA hardware.
