...
## >>> THE ONE NEXT ACTION: M1 BAKEOFF DONE — verdict B (wave-time batched validation) WINS. The charter-pure device locate now runs 40-42k @32w (from 3.5k per-item), p99<1ms, correct + sabotage-verified. Full stack pushed (`e2da8b46`): write-locate kernel + index_insert kernel (lock-free incremental device index, the fix for the 305us/wave O(rows) rebuild) + wave-batched validation. RESIDUAL to the host oracle (76k): ~40us synchronous locate launch/wave + wave-batch orchestration host cost (~10us/item unexplained — INSTRUMENT next). NEXT LEVERS: (1) async-overlap the launch / reduce orchestration allocs; (2) then M3 deletes the host cache; (3) A (coalescer) NOT worth building (shares the device-index win but adds p99-hostile batching delay). All flags default OFF. Memory: `m1-locate-bakeoff-plan` (the result), `charter-governance-ruling` (BINDING), `scalability-ledger` #20/#24. <<<
