...
## >>> THE ONE NEXT ACTION: M1's charter-pure device WRITE-LOCATE kernel + wiring are DONE + PROVEN (`492b0dcd`+`a0d01670`; PTX sabotage-verified, differential vs the host-probe oracle). PERF IS A REAL FORK (measured: per-item 3.5k vs host 76k — 20x launch latency; the INSERT unique locate fires OFF-LOCK PER-WRITER, NOT wave-batchable trivially). NEXT = M1's performance slice: (B, recommended) WAVE-TIME BATCHED VALIDATION — defer the off-lock uniqueness check, the sequencer batches the wave's PK needles into ONE device locate (coalescer-free; changes the validation model -> its own differential+audit, do NOT rush at session end) OR (A) a locate coalescer. THEN M3 deletes the host cache. Also: the i64-section flip (`6f22b71a`, pushed) — confirm its blast-radius audit verdict. Memory: `charter-governance-ruling` (BINDING), `type-coverage-14` (the M1 fork). <<<
