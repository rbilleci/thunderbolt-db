...
- STRUCT-001GT is closed. The exact two production DISTINCT/select bridges now live in the rustfmt-clean 91-line private
  `engine_expr/select_bridge.rs` leaf. The normalized moved source hash is identical; DISTINCT synthesis, source and
  visibility forwarding, schema/row framing, errors, dispatch, and consumers are unchanged. Thirteen focused GPU/route
  tests, both 505/487 engine modes, the complete 992-test GPU suite, static/scoped gates, and independent audit pass;
  15 GiB of generated residue was removed. The expression root is 7,141 lines; HAZARD/report card were not applicable.
...
1. **STRUCT-001GU:** isolate the exact two test-only GROUP BY benchmark helpers behind a test-gated private module;
   preserve their stable inherent paths and keep production execution and JOIN ownership separate.
